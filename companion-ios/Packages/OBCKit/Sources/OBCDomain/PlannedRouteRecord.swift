...
/// The device's copy of a planned route, as the phone can prove it. "Up to
/// date" means the current upload payload's CRC-32 equals the one the device
/// committed — the same whole-object CRC the transfer verified.
...
/// A planned route as the **phone's library** keeps it: the canonical parsed
/// `ImportedRoute`, the list `summary` derived from it at save time, the
...
/// `RouteBlob` payload is re-encoded from `route` whenever it's needed.
public struct PlannedRouteRecord: Identifiable, Equatable, Sendable {
    /// The list-row summary under the route's library id. Carries the display
    /// name — renames land here, never in `route`.
    ...
    /// match across the BLE boundary). `nil` until an upload commits, or a route
    /// never pushed; a device-side delete clears it again at reconcile.
    ...
    /// The detail screen's data, derived from the canonical geometry — the
    /// device never had this route, so `routeDetail` can't answer for it. Same
    /// `RouteStats` path the import landing used, so reopening shows exactly
    /// what was saved.
