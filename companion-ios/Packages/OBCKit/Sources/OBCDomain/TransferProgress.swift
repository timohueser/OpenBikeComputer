...
///
/// There is deliberately no resume offset: an interrupted transfer restarts, it does not
/// resume, so after a drop the counter starts over from 0.
...
/// How a bulk transfer ended: the terminal state `TransferHandle.outcome` resolves to, so a
/// screen never infers completion from byte counts. A drop is not terminal: a dropped transfer
/// stays unresolved until it is retried to completion, canceled, or fails for good.
