...
/// The companion UI's tracked design-token authority: colours, chrome metrics, radii and spacing.
...
public enum OBCTheme {
    ...
    /// `--dev-header`: the device UI's rust title bar.
    public static let deviceHeader = Color(hex: 0xAA5500)
    /// `--dev-header-text`: cream text on the title bar.
    ...
    // MARK: Radii
    /// `--radius-sm`: badges, pills and small controls.
    public static let radiusSmall: CGFloat = 7
    /// `--radius-md`: inputs, the segmented track and banners.
    ...
    /// `--radius-lg`: route cards.
    public static let radiusCard: CGFloat = 16
    /// `--radius-xl`: large feature panels.
...
extension Color {
    /// A 24-bit sRGB literal, `0xRRGGBB`: how the token hex values are transcribed.
    init(hex: UInt32) {
        self.init(
