...
@MainActor @Suite struct RouteRetentionReconcileTests {
    ...
    @Test func connectStampsTheClockAndReportsCapability() async throws {
        let (model, control, _) = makeModel()
        model.start()
        try await waitFor("the clock stamp") { !control.setClockSamples.isEmpty }
    ...
    @Test func oldFirmwareDropsCapabilityAndPushesNoRetention() async throws {
        ...
        model.start()
        try await waitFor("the (unsupported) stamp") { !control.setClockSamples.isEmpty }
        try await waitFor("the lists") { model.loadState == .loaded && model.connection == .connected }
    ...
    /// lands on the device.
    @Test func aDivergingDesiredLevelPushesOnce() async throws {
        let (model, control, library) = makeModel()
        if var r = record(library, objectID: 7) { r.retention = .twoMonths; library.savePlannedRoute(r) }

        model.start()
        try await waitFor("the retention push") { control.routeRetention(for: DeviceObjectID(7)) == .twoMonths }
    ...
    @Test func aNilDesiredLevelNeverPushes() async throws {
        let (model, control, library) = makeModel()
        ...
        model.start()
        try await waitFor("the route-7 push (reconcile ran)") {
    ...
    @Test func aMatchingDesiredLevelIsANoOp() async throws {
        let (model, control, library) = makeModel()
    ...
    @Test func anUploadOptsIntoTheDefaultAndPushesIt() async throws {
        let (model, control, library) = makeModel()
        model.start()
        try await waitFor("the lists") { model.loadState == .loaded && model.connectedScope != nil }
        ...
        try await waitFor("the default retention push") {
    ...
    @Test func anUploadKeepsAnExplicitLevel() async throws {
        let (model, control, library) = makeModel()
        ...
        try await waitFor("the explicit-level push") {
