...
struct TripDayEditsTests {
    ...
    func removeDayEndJoinsTheDayToTheNextOne() {
        ...
        #expect(removed)
        #expect(trip.dayEnds.map(\.title) == ["A", "B"], "the joined day keeps the first day's name")
        var unnamed = threeDays()
        unnamed.renameDay(0, to: nil)
        unnamed.removeDayEnd(0)
        #expect(unnamed.dayEnds.map(\.title) == ["B", "C"], "without one, it keeps the next day's")
