...
/// The provisional `Config` blob codec: round-trip and malformed-input behavior. These tests are
/// the single place that must move when the layout is re-pinned.
final class ConfigCodecTests: XCTestCase {
    ...
    /// A name past the u16 range must not wrap the length field: encode caps it at the 48-byte
    /// limit, so the blob stays well-formed and the trailing `units` survive.
    ...
    /// The cap lands on a Character boundary, never mid-sequence: 17 three-byte scalars truncate
    /// to the 16 that fit in 48 bytes, still valid UTF-8.
