...
/// The signed lateral offset the placement fixes at import: the magnitude is to the track point
/// that won the placement, and the sign says which side of the direction of travel the waypoint
/// fell on, positive to the right. The firmware converter derives it from the same rule.
struct WaypointOffsetTests {
    /// A dead-straight eastward track at 47 degrees north: three points, about 152 m apart.
    ...
    /// Riding east, north is left (negative) and south is right (positive); 0.001 degrees of
    ...
    /// The winning point can be the track's first, which has no incoming segment; the side then
    /// comes from the outgoing one.
    ...
    /// Placement is also where the source symbol becomes a category, and where an unmapped one
    /// degrades to generic without dropping the waypoint.
