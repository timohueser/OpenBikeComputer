...
/// The mock side of the bond seam: scenario presets carry the bond bit, and `MockBondStore` is a
/// live view onto `MockControl`.
final class MockBondStoreTests: XCTestCase {
    ...
    func testStoreIsALiveViewOntoTheControl() {
        ...
        XCTAssertTrue(control.bonded)
        // The saved name is served back: the bond record holds the desired name, which diverges
        // from `deviceInfo` when a rename's config write failed. The reconcile pass closes it.
    ...
    /// Scenario boots never `save`: the store serves the control's live identity, and `apply()`
    /// drops any saved name with the rest of the knobs.
