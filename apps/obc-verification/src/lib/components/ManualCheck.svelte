...
<div class="eyebrow">{requirement.id} · {requirement.group || 'Ungrouped'} · Manual verification</div><h2 class="requirement-title">{test.title}</h2>{#if requirement.todo}<p class="warning small">This requirement is marked To do. Its definition must be completed in a new candidate before publication.</p>{/if}<p class="muted small">Version {candidate.version} · <code>{candidate.sourceSha.slice(0,10)}</code> · requirements r{candidate.revision.id}</p>
