...
<div class="criteria">
  {#each plan.criteria.filter(c => touched.has(c.id)) as criterion (criterion.id)}
    ...
    <CoverageCriterion {criterion} {requirement} {catalog} previous={old} tag={old ? 'Changed' : 'New'} proposed />
  {/each}
...
{#if unchanged.length}<details class="small unchanged"><summary>{unchanged.length} unchanged {unchanged.length === 1 ? 'criterion' : 'criteria'}</summary><div class="criteria">{#each unchanged as criterion (criterion.id)}<CoverageCriterion {criterion} {requirement} {catalog} proposed />{/each}</div></details>{/if}
<style>
