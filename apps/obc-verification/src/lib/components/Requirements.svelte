...
<div class="workbench"><aside>
  ...
  {#each sections as section}
    ...
    {#each section.requirements as r}<button class="entry" class:selected={selected === r.id} on:click={() => select(r.id)}><span class="eyebrow">{r.id} {#if !r.active}· inactive{/if}</span><strong>{r.title || 'Untitled requirement'}</strong><span class="small muted">{r.tests.length} {r.tests.length === 1 ? 'check' : 'checks'}{r.active && !r.tests.length ? ' · verification missing' : ''}</span></button>{/each}
...
{#if requirement}
  <div class="row"><span class="eyebrow">{requirement.id} · {groupName(requirement) || 'Ungrouped'}</span><div class="actions"><span class:warning={!requirement.active} class="badge">{requirement.active ? 'Active for future candidates' : 'Inactive draft'}</span>{#if !edit && !testDraft && !picker}<button on:click={() => edit = true}>Edit requirement</button>{/if}</div></div>
