<script lang="ts">
  ...
  $: filtered = requirements.filter(r => `${r.id} ${r.title} ${r.statement}`.toLowerCase().includes(query.toLowerCase()));
  ...
  function create() {
    ...
    const r: Requirement = { id: `SYS-${String(n).padStart(3, '0')}`, title: '', statement: '', active: false, tests: [] };
    requirements = [...requirements, r]; selected = r.id; edit = true;
  }
  ...
  async function refresh() {
    ...
    busy = true; error = '';
    try { const versions = await api<Revision[]>('/api/revisions'); const latest = versions.sort((a,b) => b.id - a.id)[0]; if (latest) { revision = latest; requirements = clone(latest.requirements); selected = requirements[0]?.id || ''; testDraft = null; edit = false; undo = null; onsaved(latest); } }
    catch (e) { error = message(e); } finally { busy = false; }
  }
...
{#if history !== null}
  <section class="panel"><div class="row"><h2>Revision history</h2><button on:click={() => { history = null; historical = null; }}>Close</button></div>
    ...
    {#if historical}<div class="section"><p class="eyebrow">Read-only revision r{historical.id}</p>{#each historical.requirements as r}<details class="test"><summary>{r.id} · {r.title} {r.active ? '' : '(inactive)'}</summary><Markdown text={r.statement} />{#each r.tests as t}<div class="inset"><h3>{t.title} <span class="muted small">· {t.kind}</span></h3>{#if t.kind === 'manual'}<Markdown text={t.steps || ''} /><Markdown text={t.expected || ''} /><Files files={t.inputs} label="Input files" />{:else}<code class="wrap">{t.caseId}</code>{/if}</div>{/each}</details>{/each}</div>{/if}
  </section>
{/if}
<div class="workbench"><aside><label class="search-label">Find a requirement<input type="search" bind:value={query} placeholder="Search requirements…" /></label><div class="muted small sidebar-caption">{requirements.length} requirements · r{revision.id}</div>{#each filtered as r}<button class="entry" class:selected={selected === r.id} on:click={() => select(r.id)}><span class="eyebrow">{r.id} {#if !r.active}· inactive{/if}</span><strong>{r.title || 'Untitled requirement'}</strong><span class="small muted">{r.tests.length} {r.tests.length === 1 ? 'check' : 'checks'}{r.active && !r.tests.length ? ' · verification missing' : ''}</span></button>{:else}<p class="muted small">{query ? 'No matching requirements.' : 'Start with one important product promise.'}</p>{/each}</aside>
<section class="detail">
{#if requirement}
  <div class="row"><span class="eyebrow">{requirement.id}</span><div class="actions"><span class:warning={!requirement.active} class="badge">{requirement.active ? 'Active for future candidates' : 'Inactive draft'}</span>{#if !edit && !testDraft && !picker}<button on:click={() => edit = true}>Edit requirement</button>{/if}</div></div>
  {#if edit}<div class="section"><label>Title<input bind:value={requirement.title} on:input={() => requirements = [...requirements]} placeholder="A clear product promise" /></label><MarkdownField label="Requirement" bind:value={requirement.statement} /><label class="check"><input type="checkbox" bind:checked={requirement.active} />Include in future release candidates</label><p class="muted small">Inactive requirements remain visible but do not block a release. Existing candidates never change.</p><button on:click={() => edit = false}>Done editing</button></div>
...
{:else}<div class="empty"><h2>Make the important promises explicit.</h2><p class="muted">Write a measurable requirement, then define the checks that provide evidence.</p><button class="primary" on:click={create}>Create requirement</button></div>{/if}
</section></div>
