<script lang="ts">
  ...
  function removeRequirement() {
    ...
    selected = requirements.find(r => matches(r, groupFilter, query))?.id || '';
  ...
  function restoreRequirement() {
    ...
    deleted = deleted.slice(0, -1); groupFilter = 'all'; query = ''; selected = item.requirement.id;
  }
  ...
  async function refresh() {
    ...
    busy = true; error = '';
    try { const versions = await api<Revision[]>('/api/revisions'); const latest = versions.sort((a,b) => b.id - a.id)[0]; if (latest) { revision = latest; requirements = clone(latest.requirements); groupFilter = 'all'; query = ''; selected = requirements[0]?.id || ''; testDraft = null; edit = false; undo = null; deleted = []; deleting = false; onsaved(latest); } }
    catch (e) { error = message(e); } finally { busy = false; }
  }
...
<div class="workbench"><aside>
  ...
  {#each sections as section}
    ...
    {#each section.requirements as r}<button class="entry" class:selected={selected === r.id} on:click={() => select(r.id)}><span class="eyebrow">{r.id}</span><strong>{r.title || 'Untitled requirement'}</strong><RequirementLabels requirement={r} /><span class="small muted">{r.tests.length} {r.tests.length === 1 ? 'check' : 'checks'}{r.active && !r.tests.length ? ' · verification missing' : ''}</span></button>{/each}
