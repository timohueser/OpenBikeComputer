<script lang="ts">
  ...
  function state(r: Requirement): State {
    ...
    if (r.todo || r.implementationNeeded || !r.tests.length || coverageIssues(r).length) return 'blocked';
