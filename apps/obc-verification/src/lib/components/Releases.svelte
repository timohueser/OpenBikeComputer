<script lang="ts">
  import { coverageIssues, coverageStatus } from '$lib/coverage';
  ...
  function state(r: Requirement): State {
    ...
    const outcomes = r.tests.map(t => status(r, t));
    if (outcomes.some(o => o === 'fail' || o === 'error' || o === 'blocked')) return 'failing';
    if (r.todo || r.implementationNeeded || !r.tests.length || coverageIssues(r, candidate.sourceSha).length) return 'blocked';
