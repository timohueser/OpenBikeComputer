<script lang="ts">
  ...
  function back() { if (exceptionBusy) return false; if (dirty && !confirm('Discard the unsaved test result or exception reason?')) return false; manualDirty = false; exceptionDrafts = {}; editorEpoch++; check = null; savedRun = ''; review = false; preparing = false; return true; }
  ...
  /** The requirement a readiness item belongs to, if any. */
  function owner(item: string) { return candidate?.revision.requirements.find(r => r.active && (item.startsWith(`${r.id}: `) || item.startsWith(`${r.id} / `))); }
  /** One summary line per kind of blocker; an empty line is left out. */
  $: blockers = (() => {
    const missing = readiness?.missing || [];
    const files = missing.filter(item => item.startsWith('Missing release asset: ')).length;
    const build = [candidate && candidate.ciStatus !== 'success' ? `automated checks ${candidate.ciStatus}` : '', files ? `${count(files, 'file')} missing` : ''].filter(Boolean);
    let run = 0, failing = 0, noResult = 0, blocked = 0;
    for (const r of candidate?.revision.requirements.filter(r => r.active) || []) {
      if (!outstanding(state(r))) continue;
      const tests = citedTests(r);
      if (r.todo || r.implementationNeeded || !tests.length || coverageIssues(r).length) blocked++;
      for (const t of tests) {
        const o = status(r, t);
        if (o === 'fail' || o === 'error' || o === 'blocked') failing++;
        else if (o !== 'pass') t.kind === 'manual' ? run++ : noResult++;
      }
    }
    const requirements = [missing.includes('No active requirements.') ? 'no active requirements' : '', run ? `${count(run, 'manual check')} to run` : '', failing ? `${count(failing, 'test')} failing` : '', noResult ? `${count(noResult, 'automated result')} missing` : '', blocked ? `${count(blocked, 'requirement')} not ready for verification` : ''].filter(Boolean);
    return { build, requirements };
  })();
  /** Opens the requirement's group, scrolls to its row and focuses its first control. */
  async function jump(r: Requirement) {
    openGroups = { ...openGroups, [r.group?.trim() || '']: true };
    await tick();
    const row = document.getElementById(`evidence-${r.id}`);
    row?.scrollIntoView({ block: 'start', behavior: 'smooth' });
    (row?.querySelector<HTMLElement>('button:not(:disabled)') || row)?.focus({ preventScroll: true });
  }
  /** CI can succeed while the candidate still waits for evidence; the server then keeps `running`. */
  const statusLabel = (c: Candidate) => c.status === 'running' && c.ciStatus === 'success' ? 'not ready' : c.status;
</script>
...
<div class="workbench"><aside><div class="eyebrow sidebar-caption">Release candidates</div>{#each candidates as c}<button class="entry" disabled={exceptionBusy} class:selected={!preparing && candidate?.id === c.id} on:click={() => load(c.id)}><strong>{c.version}</strong><span class="small muted">{statusLabel(c)} · {c.sourceSha.slice(0,8)}</span><span class="small muted">{date(c.createdAt)}</span></button>{:else}<p class="muted small">Prepared candidates will appear here, including failed attempts.</p>{/each}</aside><section class="detail">
{#if preparing}
  <button class="back" on:click={() => preparing = false}>← Back to releases</button><h2 class="requirement-title">Prepare a release candidate</h2><p class="muted">Choose the code revision and the requirements revision. GitHub Actions then builds the candidate and runs verification.</p>
...
{:else if candidate && check}
  {#key candidate.id + check.test.id}<ManualCheck {candidate} requirement={check.requirement} test={check.test} bind:dirty={manualDirty} onback={() => check = null} onsaved={(c) => { const r = check!.requirement; savedRun = r.id; candidate = c; remember(c); load(c.id, true); jump(r); }} />{/key}
