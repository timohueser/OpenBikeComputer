...
export function coveragePlan(value: unknown, requirement: Requirement, catalog: Catalog, procedures: VerificationTest[] = []): CoveragePlan {
  ...
  const criteria: AcceptanceCriterion[] = plan.criteria.map((entry: any, index: number) => within(
    `criterion ${index + 1}`,
    { criterion: index + 1, ...(typeof entry?.id === 'string' && entry.id.trim() ? { criterionId: entry.id.trim() } : {}) },
    () => {
...
export function draftCoverage(requirements: Requirement[], raw: unknown, catalog: Catalog, id: () => string): Requirement[] {
  return requirements.map((requirement, index) => within(`${requirement.id} · ${requirement.title}`, { requirementId: requirement.id }, () => {
