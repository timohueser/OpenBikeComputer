...
export function report(candidate: Candidate): string {
  ...
  const outcome = !state.ready ? 'Incomplete' : state.excepted ? 'Accepted with exceptions' : 'Verified';
...
<h2>${outcome}</h2><p>${state.verified}/${state.total} requirements verified · ${state.excepted} accepted with exception</p>
...
${candidate.revision.requirements.map(r => {
    ...
    const issues = requirementIssues(candidate, r, results);
    return `<article><h2>${escape(r.id)} — ${escape(r.title)}${r.active ? '' : ' (inactive)'}</h2>
${r.group ? `<p><small>Group: ${escape(r.group)}</small></p>` : ''}
${r.todo ? '<p class="todo">To do — requirement definition is incomplete.</p>' : ''}
