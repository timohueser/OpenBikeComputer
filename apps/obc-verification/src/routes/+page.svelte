<script lang="ts">
  ...
  function guard(event: BeforeUnloadEvent) { if (requirementsDirty || runDirty || accountDirty) { event.preventDefault(); event.returnValue = ''; } }
  onMount(load);
</script>
