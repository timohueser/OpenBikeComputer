...
db.clearHistory(db.latestRevision().id, 'Local demo setup', true);
db.saveRevision(db.latestRevision().id, 'Local demo setup', requirements);
for (let i = 0; i < plans.length; i++) await propose(i);
db.db.close();
...
console.log(`\nLocal coverage demo: ${process.env.ORIGIN}\nUsername: demo\nPassword: ${password}\nDisposable database: ${directory}\nOpen Review proposals, then inspect Releases → v0.0.0-coverage-demo. Results are simulated.\n`);
