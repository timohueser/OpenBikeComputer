...
export function candidate(): Candidate {
  return { id: 'candidate', version: '0.1.0', sourceRef: 'develop', sourceSha: 'a'.repeat(40), createdAt: '', status: 'running', ciStatus: 'success',
    revision: { id: 1, author: 'owner', createdAt: '', requirements: [{ id: 'REQ-1', title: 'Recover data', statement: '**Required**', active: true, coverage: { sourceSha: 'a'.repeat(40), conclusion: 'complete', rationale: 'Both recovery obligations have evidence.', criteria: [{ id: 'recover', statement: 'Data is retained through a power cut.', evidence: [{ caseId: 'suite::class::recovery', rationale: 'Automated recovery path.' }, { testId: 'manual', rationale: 'Physical power cut.' }], gap: '' }], review: { author: 'owner', createdAt: '', proposalId: 'review' } }, tests: [{ id: 'auto', title: 'Recovery', kind: 'automated', caseId: 'suite::class::recovery', inputs: [] }, { id: 'manual', title: 'Power cut', kind: 'manual', steps: 'Cut power', expected: 'Data retained', inputs: [] }] }] },
