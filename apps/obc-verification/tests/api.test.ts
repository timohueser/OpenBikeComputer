...
test('requirement suggestions are recorded, superseded, decided once, and cleared with history', async () => {
  const owner: Actor = { name: 'owner', role: 'owner' };
  const agent: Actor = { name: 'agent', role: 'agent' };
  const base = store().latestRevision();
  // A second requirement carries the change suggestions, so the example requirement stays untouched.
  ...
  store().saveRevision(edited.id, 'owner', edited.requirements.map(r => r.id === 'EXAMPLE-002' ? { ...r, statement: `${r.statement} It shall also keep the zoom level.` } : r));
  const stale = (await listed()).find((s: any) => s.id === second.id);
  assert.match(stale.stale, /EXAMPLE-002 changed in r\d+, after this suggestion\./);
