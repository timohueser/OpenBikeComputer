...
import { authenticate, changePassword, createSession, githubLogin, localLogin, logout, requireAdmin } from '../src/lib/server/auth.ts';
...
test('persistent local credentials rotate sessions without resetting requirements or accepting stale environment hashes', async () => {
  ...
  const request = new Request('https://verification.example');
  {
    ...
    const agent = authenticate(new Request(request, { headers: { authorization: 'Bearer agent-token' } }), cookies)!;
