...
//!
//! Nothing here knows about the browser — that is the point. `cargo test` runs this natively, so the
//! phase mapping, the error vocabulary and the byte-for-byte determinism pin are covered by the
...
//!
//! [`obcm_assemble::assemble_full`] takes two host-supplied seams — a [`Clock`] and a [`MapStore`] —
...
//!   `t_poi`, `t_nav`, `t_plan`, then the write and verify timings). Ticks 1–5 therefore name the
//!   phases `open → poi → nav → plan → write`, and `tests/determinism.rs`'s
...
//! * Once the write starts, the store's own calls are unambiguous and take over:
//!   [`MapStore::write`] means bytes of geometry are moving and [`MapStore::source`] means the §4.8
//!   verify pass is about to read the sealed map back.
//! * The verify pass itself is observed one level deeper. [`MapStore::source`] is called **once** and
//!   everything after it happens inside the engine, so a bar driven by store calls alone would sit
//!   still through §4.8 — which is **43 % of a measured region-scale run**, 11.4 s of
//!   baden-württemberg's 26.2 s (#1116's phase-D harness). [`VerifySource`] therefore wraps the
//!   sealed map the engine reads back and reports from [`ByteSource::read_at`], which is the
//!   read-back's own inner loop.
//!
//! # Abort granularity
//!
...
//!   `verify_map` turns any read failure into `Error::Verify` and a cancelled run must never look
//!   like a broken assembler).
//! * **Inside the nav rewrite it is not.** §4.6 makes no store calls at all, so an abort requested
//!   during it is honoured when the phase ends. That is the only uninterruptible stretch, and at a
//!   measured 20 % of the run it is the shorter of the two the bridge used to be blind to. Making it
//!   finer needs a seam inside the engine.
//!
...
//!
//! Half a browser's peak is the **input**: [`CellBytes`] carries a whole downloaded cell into linear
//! memory and it stays there for the run — ~795 MB at country scale, resident from the first
//! `addCell` to the last byte written. It does not have to be there. Everything the engine does with
//! a cell it does through [`ByteSource::read_at`], so a cell can live anywhere the host can serve a
//! byte range from — OPFS, in the browser's case.
//!
...
//!
//! The other half is the **output**, and it is the half that decides whether a country can be
//! assembled in a tab at all. D1 was built because "the core shard cannot be split — one nav graph,
//! one file", so a country-scale core could not be a `Vec<u8>` in a 4 GiB address space. One file
//! makes that *more* true, not less: a DACH map is now a single ~9 GiB object, larger than the
//! address space it would have to live in, and writing it through the host's own handle is the only
//! way it assembles at all. The engine's write seam is what makes a map that size expressible; this
//! seam is what makes it reachable.
//!
//! [`MapWrites`] is the write-side twin of [`CellReads`], and it is the same three ideas: the calls
//! are synchronous so they can be made from inside the one blocking assembly, the bytes cross as a
//! view rather than a copy, and everything the engine reads back comes through a [`BlockCache`]. In
//! the browser it is one OPFS `FileSystemSyncAccessHandle`, opened in the assembly worker.
//!
//! What changes inside this module is small on purpose, because the format knowledge must not move:
//!
//! * [`MapStore::write`] forwards to the host instead of extending a vector.
//! * [`MapStore::source`] hands §4.8 a [`VerifySource`] whose body is a *file*, so the read-back
//!   genuinely re-reads what was written — the property the whole verify pass exists for, and one
//!   that an in-memory store can only assert. `a_map_the_sink_corrupts_on_disk_fails_verify` is the
//!   proof: flip a byte behind the driver's back and §4.8 must reject the map.
//! * The sealed, verified map is reported by identity ([`SealedMap`]) rather than handed over as
//!   bytes — [`Hooks::map_sealed`] — because the host already has the file. Nothing map-sized is
//!   ever in linear memory, so there is nothing to hand over and nothing to `postMessage`.
//!
//! The buffered path stays exactly as it was, to the byte: no sink means [`MapBody::Buffered`], a
//! `Vec<u8>`, and [`Outcome::bytes`].
...
/// How a host takes the bytes of the map — and gives them back (#1116 D1).
///
/// The write-side twin of [`CellReads`], and the reason a country-scale map does not have to be a
/// `Vec<u8>` in a 4 GiB address space. There is one file, so nothing here is indexed.
///
/// Every method is called from **inside** the synchronous assembly, with the engine blocked behind
/// it. In the browser that is a `FileSystemSyncAccessHandle`, opened in the assembly worker before
/// the run starts (it cannot be opened during it — the opener is async and the run cannot await),
/// which is why the seam is blocking calls rather than futures.
///
/// The lifecycle is fixed and the engine's own: `create` once, `write` many times in order, `seal`
/// once, then any number of [`MapWrites::read_at`] calls while §4.8 reads the map back.
/// `Err(message)` from any of them fails the run as [`ErrorCode::Io`] carrying that message — never
/// as a §4.8 [`ErrorCode::Verify`] defect, which would tell a rider the assembler is broken because
/// their disk filled up.
pub trait MapWrites {
...
/// The map the host wrote itself, once §4.8 has read it back and passed it (#1116 D1) — an
/// **identity instead of bytes**, because the host already has the file.
///
/// The digest is this crate's own arithmetic, taken from the bytes as they crossed into the sink,
/// and it is cross-checked against the engine's before the caller is told anything (see
/// [`check_sealed_identity`]). A map reported here has no [`Outcome::bytes`].
...
/// [`obcm_assemble::ScratchStore`], crossed the way [`CellReads`] and [`MapWrites`] cross).
///
...
/// cannot await — the same constraint that shaped [`MapWrites`]), so `create` can refuse when the
...
/// plus the one knob that belongs to this crate rather than to the engine.
///
/// The omission is deliberate and is the issue's own requirement: OBCA §4.8 makes verification a
/// **precondition of writing a map**, and this bridge exists to hand bytes to a device. There is no
...
impl BridgeOptions {
    ...
    /// [`Options::default`]. Read by hand rather than with `serde`'s derive because four fields are
...
pub enum Phase {
    ...
    Nav,
    /// Laying the map out: every region's offset computed before a byte is written (§5).
    Plan,
    /// Writing the file, which is where the verbatim geometry graft happens (§4.4) and where the
    /// §1.3 raster is spliced into the tail.
    Write,
    /// Reading the sealed map back through the real reader (§4.8).
    Verify,
...
impl Phase {
    ...
    /// engine (macOS arm64, release, published v12 catalog, 256 MiB merge budget). D3 made the nav
    /// merge external-sort-shaped and D4 made the write phase stream the section out of scratch, so
    /// both grew relative to verify — each recalibration this file has carried moved because the
    /// engine did.
    ///
    ...
    ///
    /// The streaming engine made the split **scale-free**: the two regions agree to half a point on
    /// every phase, where every earlier calibration had them merely bracketing each other. That is
    /// the property that lets one weight table serve a corridor and a country.
...
/// The map's slot in its own read-back cache. There is one file, so there is one slot; the number
/// exists because [`BlockCache`] is shared with the input seam, where slots are how a cell is named.
...
struct CachedBlock {
    ...
    slot: usize,
    /// Which block of it — a **file**-relative index, so `u64`: a 64 GiB file has more blocks than
    /// a wasm32 `usize` would be embarrassed by, but the arithmetic that produces it is a file
    /// offset divided by the block size and it stays in the file's width throughout.
    index: u64,
    /// The block's bytes. Exactly `len` of them are valid — the last block of a source is short.
    data: Vec<u8>,
    len: usize,
...
/// of [`MapWrites`] for the output (#1116 D1) — so one [`BlockCache`] serves either, and the
/// argument for the cache (module header) is made once rather than twice.
trait SlotReads {
...
/// …and the sealed map, which the §4.8 pass reads back through the same handle it was written
/// through. One file, so the slot is always [`MAP_SLOT`] and is not passed on.
...
/// of blocks shared by **every** slot, so residency is a constant rather than a per-source cost.
///
...
///   and never a §4.8 "the assembler wrote a map the reader cannot read".
struct BlockCache<'r> {
    reads: &'r dyn SlotReads,
    block: usize,
    /// Per slot, how a message names that source. Set once, before the run — the input cells' from
    /// the catalog, the map's because there is only one — so the read path never formats.
    labels: Vec<String>,
...
impl<'r> BlockCache<'r> {
    ...
    ///
    /// There is no `u32::try_from` here: since FS7.5-seam a sunk map is a **file** in the host's
    /// storage, which may be larger than this address space. The buffer is still `usize`-bounded,
    /// and correctly so — it lives in wasm memory.
...
/// [`assemble`] refuses first) or a sunk map exists without a sink (which cannot be constructed).
struct NoReads;
...
pub trait Hooks {
    ...
    /// The map the host's own [`MapWrites`] sink wrote has passed its §4.8 read-back (#1116 D1).
    ///
    /// Called once, after the engine returns, and **only** when a sink was wired: bytes that never
    /// entered linear memory cannot be handed over, so what the caller gets is what it has. Without
    /// a sink the bytes are in [`Outcome::bytes`] and there is nothing to report.
    ///
    /// `Err(message)` fails the run as [`ErrorCode::Io`] with that message, because a map whose
    /// identity the caller could not record must not be reported as finished — the file exists and
    /// the digest is the only thing that says which bytes are in it.
...
pub enum ErrorCode {
    ...
    Format,
    /// A ceiling: `obcm_assemble::FILE_CEILING` (the interior an `Offset Scale` of 4 addresses,
    /// 64 GiB), the `HoursRef` pool, the `uint32` index space. **Coverage must be reduced** — one
    /// file has one remedy, and per §5.7 this bridge never "solves" it by dropping any.
    Capacity,
    /// The §4.8 verify pass rejected the output: the engine wrote a map the real reader cannot read.
...
/// What an assembly produced: one map file, the engine's `SHOULD`-report warnings, and the same
/// summary document `obcm-assemble --json` prints.
///
/// The file has no name here. The engine names nothing (naming is a host's job — a card's 8.3
/// filename, a browser's save dialog), so what crosses is an identity and, when the run buffered it,
/// the bytes.
pub struct Outcome {
    ...
    /// §4.5.2's dropped duplicate POIs, `OBCM_Spec.md` §8.3's degree-cap truncations). A caller that
    /// ignores this ships the same bytes; a caller that shows it tells the rider what the spec
    /// wanted them told.
...
/// Hand-written so a failing assertion prints the map's *shape* rather than a megabyte of hex.
...
struct Progress<'h> {
    ...
    aborted: bool,
    /// The sink path's own failure, if it raised one. The engine's store contract has a single
    /// refusal (`Error::Io`) which says nothing about *why*, so the driver keeps the real one here
    /// and [`map_error`] prefers it.
...
impl Progress<'_> {
    ...
    /// The write/verify span's bar. Two independent terms over one shared span, because a write
    /// counter alone stops moving the moment §4.8 starts — and §4.8 is two fifths of the run.
    ///
    ...
    /// the store's own calls, and `done` lands on exactly 1.0.
...
impl Clock for HookedClock<'_, '_> {
    fn now_us(&self) -> u64 {
        ...
        // next phase. From tick 6 the engine is inside the write and verify passes, where the
        // store's own calls say which of the two is running, so the clock stops naming phases.
...
/// Where the map's bytes actually are.
///
/// The two are the same file to the engine and to the format — the difference is only whether this
/// address space is holding it. That is why the enum is here rather than two stores: everything
/// about §4.8, the digest and the hand-off moment is shared, and only `read_at` and `len` branch.
enum MapBody<'a> {
    /// In wasm memory: no sink, or a native caller that wants the bytes.
    Buffered(Vec<u8>),
    /// In the host's own storage (#1116 D1) — how many bytes were handed over, and the cache the
    /// §4.8 read-back pulls them back through.
    Sunk { len: u64, cache: &'a BlockCache<'a> },
}

impl MapBody<'_> {
    /// The map's length, exactly. A **buffered** map is bounded by this address space; a **sunk**
    /// one is a host file that may be larger than it, which is the case the `u64` read seam exists
    /// for.
    fn len(&self) -> u64 {
        match self {
...
/// The sealed map as the §4.8 verify pass reads it back — the bytes, wherever they are, plus the
...
/// [`MapStore::source`] hands the engine a `&dyn ByteSource` and everything §4.8 does happens behind
/// it, so `read_at` is the only place a browser can learn that verify is moving — or tell it to
/// stop.
///
    ...
    fn read_at(&self, offset: u64, buf: &mut [u8]) -> Result<(), obc_formats::io::Error> {
        {
            ...
            if p.aborted {
                // The read-back's own error channel is all there is down here. `verify_map` turns
...
///
/// A sealed map has to be **randomly addressable**: OBCA §4.8 requires it to be read back through
/// the real reader before it is handed on. Where it is addressable *from* is this store's one degree
/// of freedom, and both answers are here — wasm memory, or the host's own storage through a
/// [`MapWrites`] sink (#1116 D1). The second is what a country-scale map needs, because it is one
/// file and a browser cannot hold it.
struct HookedStore<'a, 'h> {
    ...
    /// because a second SHA-256 pass over a gigabyte-scale file is real time to spend on a digest
    /// the engine already computes and a buffered caller already has the bytes for.
    hasher: Sha256,
    /// Lowercase hex, once sealed. Empty for a run with no sink.
    sha256: String,
    /// Where the map's bytes go, when they do not go into this address space (#1116 D1). `None` is
    /// the buffered store, unchanged to the byte.
...
/// the engine's when the run unwinds.
...
impl HookedStore<'_, '_> {
    ...
    /// engine's store contract has; [`assemble`] turns it back into [`ErrorCode::Aborted`] because
    /// it knows why it was raised.
...
impl MapStore for HookedStore<'_, '_> {
    fn begin(&mut self) -> obcm_assemble::Result<()> {
        ...
        match self.sink {
            // The host takes the bytes. Nothing map-sized is reserved here, which is the whole
            // point: §5 may have just planned a 9 GiB file.
            Some(sink) => {
            ...
            None => {
                ...
                // The map comes out about the size of the cells that went in — measured 1.00 on
                // both of #1116's harness regions — so reserving the projection up front gets the
                // browser its buffer in one allocation rather than through a doubling ladder whose
                // last step transiently holds 1.5× a gigabyte-scale file, memory the estimate does
                // not model and a tab may not have contiguously. The engine's exact figure is not
                // reachable here (`MapStore::begin` takes no plan), so this is a projection and
                // being a little wrong costs one growth. `try_` because a refusal is recoverable:
                // the write path then grows the vector itself and fails (or not) exactly as it
                // would have, where a plain `reserve_exact` would abort the whole module.
    ...
    fn write(&mut self, buf: &[u8]) -> obcm_assemble::Result<()> {
        ...
        self.check_abort()?;
        // The engine hashes the same bytes on its way through `emit::write`, but it keeps that
        // digest to itself until the run ends. Hashing here is what lets the end-of-run comparison
        // against the engine's figure be a real check that the bytes the *host* saved are the bytes
        // the engine wrote — the host never sees them, so nothing else could say so.
    ...
    fn seal(&mut self) -> obcm_assemble::Result<()> {
        if let Some(sink) = self.sink {
            ...
            // this file back, so a host that buffers has to have flushed by now.
    ...
    fn source(&self) -> obcm_assemble::Result<&dyn ByteSource> {
        // The engine asks for the sealed map for exactly one reason: the §4.8 verify pass. This is
...
/// equality against a checked-in file the CLI produced, and `builder/app/src/lib/assemble/
...
/// rider's selection covers no terrain object, and the map is written with an empty §1.3 region — a
...
/// memory (#1116 B2), or one that would rather the map did not either (#1116 D1), builds a
/// [`Wiring`] instead.
...
/// output than have this address space hold it, where the map goes.
///
...
/// the output: §4.6.5 renumbers by content and §5 lays the file out by ladder level, which is what
/// makes an assembly deterministic however the downloads finished.
#[derive(Default)]
pub struct Wiring<'r> {
    ...
    /// Where the map is written (#1116 D1). `None` keeps it in wasm memory, which is what every
    /// native caller wants and what the browser falls back to. With one, the file is never resident
    /// and [`Outcome::bytes`] is `None`: its identity is reported to [`Hooks::map_sealed`] once
    /// §4.8 has passed.
...
pub fn assemble(
    ...
    // …and the same on the write side, for the §4.8 read-back of a map the host wrote itself. One
    // file, so its label is known before the run rather than derived during it.
    ...
    let summary = match assemble_full(inputs, known_empty, job, &schema, &skin, &options, &mut store, &clock, scratch) {
        ...
        Err(e) => {
            ...
            // blame the catalog for the browser's storage. The host's own message wins. A map that
    ...
    // The host saved these bytes without ever seeing them, so this equality is the only thing
    // standing between a mislabelled file and a card. It runs **before** the caller is told
    // anything, which is what makes it a gate rather than a tripwire.
    if sink.is_some() {
...
///
/// It exists because the two digests come from the same bytes by different paths: the engine's from
/// `emit::write`'s own pass, this one from the buffers that crossed into the host's file. Being
/// right is a correctness claim rather than a convenience — a caller records this digest against a
/// file it can no longer inspect — and a failure is [`ErrorCode::Internal`] because the only way to
/// reach it is a defect here.
...
/// channel it was standing in — the store's `Error::Io` from a write, but `Error::Verify` from a
/// [`VerifySource::read_at`] refusal, because `verify_map` reports every read failure as a §4.8
/// defect. A cancelled run must never be reported as "the assembler wrote a map the reader cannot
/// read": that is the one code a caller is told never to retry past, and it would turn a cancel
/// button into a bug report.
///
/// `failure` is the sink path's own, raised through the same one-shaped `Error::Io` channel and
/// therefore just as unreadable without it. It is read next; it cannot coexist with an abort,
/// because every store entry point checks the abort flag before it touches the sink.
...
/// the CLI's version names an output *path*, which does not exist in a browser, and because the
/// §4.8 report is a field here where the CLI prints it to a terminal — everything else is the same
/// document, deliberately, so the two hosts report one thing.
...
mod tests {
    ...
    /// read-back comes back as `Error::Verify`, because `verify_map` reports *any* read failure as
    ...
    /// writing a map, and this bridge exists to hand bytes to a device. The knob is ignored, not
    ...
    /// The sink's identity gate fires on a digest disagreement, and says which two.
    #[test]
