...
- `--script TOKENS` applies device input before a headless render. `d`/`u` step, `p` selects, `h`
  ...
  The context drawer uses `c` for no backdrop, `D` for the production Down+Back chord with
  palette-LUT dimming, `S` for a stippled scrim, `C` for fullscreen, and `A` to advance one quarter through its rise (`w`
  ...
  The top quick-drawer uses `q` (plain) or `Q` (the production dimmed Up+Select chord); in the GUI
  the physical upper pair, Up+Select, opens it (`Left Arrow`+`Enter` on the keyboard). Its
  brightness and power-confirmation pages use the same controls.
