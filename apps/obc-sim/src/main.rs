...
struct Args {
    ...
    sensors_screen: bool,
    /// Headless `--png` only: consume a recorded create-route request without starting it, so the
    ...
    /// Headless `--png` only: consume a recorded **detour-plan** request without starting it (#882), so the
...
fn main() {
    ...
    if let Some(path) = &args.png {
        ...
        if let Some(script) = &args.script {
            ...
            // `--nav-hold` / `--inject-nav-fail` consume the request without starting it so the planning
            // screen stays up (for its own snapshot, or for the injected answer to land in).
            let hold = PlanHold::new(
                args.nav_hold || args.inject_nav_fail.is_some(),
                args.detour_hold || args.inject_detour_fail.is_some(),
            );
