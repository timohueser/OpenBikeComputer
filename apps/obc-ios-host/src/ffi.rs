...
//!
//! **Every function is main-thread only.** The display link, the touch areas and the CoreLocation
//! and CoreMotion delegates all run there, and nothing here synchronises. A NULL host is a no-op —
//! `false`, NULL, or a failure code — so a shell that outlives its host cannot take the app down.
//! A path that is NULL or not UTF-8 fails through [`obc_ios_last_error`].
//!
...
// The module contract above *is* the safety contract of every entry point, and C cannot call an
// `unsafe fn` any differently; marking each one would only copy that one sentence into sixteen
// `# Safety` sections.
#![allow(clippy::not_unsafe_ptr_arg_deref)]

use crate::{import_map, Host, FRAME_H, FRAME_W};
use obc_app::Screen;
...
///
/// # Safety
/// `host` is NULL or a handle from [`obc_ios_open`] that is not closed yet.
...
#[no_mangle]
pub extern "C" fn obc_ios_import_map(card_path: *const c_char, obcm_path: *const c_char) -> i32 {
    install_panic_hook();
    // SAFETY: both are NULL or NUL-terminated strings the caller keeps alive across the call.
    let paths = unsafe { (as_path(card_path, "card path"), as_path(obcm_path, "map path")) };
    let (Some(card), Some(obcm)) = paths else {
        return -1;
    };
...
#[no_mangle]
pub extern "C" fn obc_ios_open(
    card_path: *const c_char,
    settings_path: *const c_char,
    exports_dir: *const c_char,
) -> *mut Host {
    install_panic_hook();
    // SAFETY: each is NULL or a NUL-terminated string the caller keeps alive across the call.
    let paths = unsafe {
        (
            as_path(card_path, "card path"),
            as_path(settings_path, "settings path"),
            as_path(exports_dir, "exports directory"),
        )
    };
    let (Some(card), Some(settings), Some(exports)) = paths else {
        return ptr::null_mut();
    };
...
#[no_mangle]
pub extern "C" fn obc_ios_close(host: *mut Host) {
    if !host.is_null() {
        // SAFETY: the handle came from `obc_ios_open`'s `Box::into_raw` and C closes it once.
...
#[no_mangle]
pub extern "C" fn obc_ios_tick(host: *mut Host, now_ms: f64) -> bool {
    // SAFETY: an open handle, or NULL.
    unsafe { with(host, false, |host| host.tick(now_ms)) }
}
...
#[no_mangle]
pub extern "C" fn obc_ios_frame(host: *const Host) -> *const u8 {
    // SAFETY: an open handle, or NULL.
    unsafe { read(host, ptr::null(), |host| host.frame().as_ptr()) }
}
...
/// no UTC stamp.
#[no_mangle]
pub extern "C" fn obc_ios_push_fix(
    host: *mut Host,
    lat_udeg: i32,
    lon_udeg: i32,
    course_deg: f32,
    ...
    // SAFETY: an open handle, or NULL.
    unsafe { with(host, (), |host| host.sensors().push_fix(fix, (unix_secs != 0).then_some(unix_secs))) }
}

/// `CLHeading.trueHeading` in degrees clockwise from north.
#[no_mangle]
pub extern "C" fn obc_ios_push_heading(host: *mut Host, degrees: f32) {
    // SAFETY: an open handle, or NULL.
    unsafe { with(host, (), |host| host.sensors().push_heading(degrees)) }
}

/// `CMAltimeter` absolute altitude in metres.
#[no_mangle]
pub extern "C" fn obc_ios_push_altitude(host: *mut Host, metres: f32) {
    // SAFETY: an open handle, or NULL.
    unsafe { with(host, (), |host| host.sensors().push_altitude(metres)) }
}

/// `UIDevice.batteryLevel` as whole percent.
#[no_mangle]
pub extern "C" fn obc_ios_push_battery(host: *mut Host, percent: u8) {
    // SAFETY: an open handle, or NULL.
    unsafe { with(host, (), |host| host.sensors().push_battery(percent)) }
}
...
#[no_mangle]
pub extern "C" fn obc_ios_button(host: *mut Host, button: u32, down: bool) {
    ...
    // SAFETY: an open handle, or NULL.
    unsafe { with(host, (), |host| host.push_button(button, down)) }
}

/// Import one `.obcr` or `.gpx` route into the card. 0 on success; [`obc_ios_last_error`] says why
/// otherwise.
#[no_mangle]
pub extern "C" fn obc_ios_import_route(host: *mut Host, path: *const c_char) -> i32 {
    // SAFETY: an open handle, or NULL.
    let Some(host) = (unsafe { host.as_mut() }) else {
        return fail("no host is open".to_string(), -1);
    };
    // SAFETY: NULL or a NUL-terminated string the caller keeps alive across the call.
    let Some(route) = (unsafe { as_path(path, "route path") }) else {
        return -1;
    };
...
#[no_mangle]
pub extern "C" fn obc_ios_screen(host: *const Host) -> *const c_char {
    // SAFETY: an open handle, or NULL.
    unsafe { read(host, ptr::null(), |host| screen_name(host.screen())) }
}
...
#[no_mangle]
pub extern "C" fn obc_ios_recording(host: *const Host) -> bool {
    // SAFETY: an open handle, or NULL.
    unsafe { read(host, false, Host::recording) }
}
