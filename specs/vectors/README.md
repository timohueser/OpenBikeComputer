...
> **Pending FS7.5b/c (OBCM v14, [#1420](https://github.com/timohueser/OpenBikeComputer/issues/1420)).**
> Five files in the table below are already known to be moving, and this note exists so that no
> number in it is read as current when it is not:
>
> - `version-read.bin` and `version-read-features.bin` carry `obcm_version = 13`, self-sourced from
>   `obc_formats::obcm::VERSION`. `OBCM_Spec.md` is now v14 and the constant follows in **FS7.5b**,
>   which re-cuts both files with `obcm_version = 14`. The bytes here are the v13 bytes and stay
>   valid until then — the spec moved first on purpose, and there is no codec yet that could produce
>   the new ones. **Regenerates with FS7.5b.**
...
>   **Deleted with the set machinery in FS7.5b/c**, together with `obc-formats/src/obcs.rs` and the
...
| `transfer-set-shard.bin` (**dies in FS7.5b/c**) | `transferControl` §4.2 | a **volume-set shard** upload (§4.1, #1039): type `17` `mapShard`, and the one `object_id` on this wire that is not an object id — the packed part `(shard_count << 8) \| index` = `0x0802`, shard 2 of 8. Count in the *high* byte; the fixture exists because that is the half of the rule three implementations would otherwise each re-derive from prose. `total_len`/`crc32` are `route-waypoints.obcr`'s, as the other descriptors' are |
| `transfer-set-terrain.bin` (**dies in FS7.5b/c**) | `transferControl` §4.2 | the set's **terrain shard** upload (#1044): type `19` `terrainShard`, new-only (`0xFFFF`) because there is at most one per set. `total_len`/`crc32` are `terrain-shard.obcd`'s own, so the descriptor and the file it announces are one decodable pair. Sent after every `mapShard` and before the `mapSet` |
| `transfer-set-manifest.bin` (**dies in FS7.5b/c**) | `transferControl` §4.2 | the **set manifest** upload: type `18` `mapSet`, new-only (`0xFFFF`), and written **last** (`OBCA_Spec.md` §5.4). `total_len` = `72 + 64 × 9` = 648 — the *record* count, not the shard count: eight OBCM shards **plus** the terrain record above, because §5.2's `Shard Count` counts every record. The record is 64 bytes since manifest **v3** (#1389) gave every record its member's `ObjectId`; it was 56 before. A device checks it at the announce, against what it actually received (#1044) |
...
would be a lie three implementations agreed on. (That example is the v12→v13 bump; the **v13→v14**
one is live right now — `OBCM_Spec.md` is v14, the constant is not yet, and the note at the top of
this file is what stops the gap from being silent while the two are apart.) So an OBCM format bump
fails
