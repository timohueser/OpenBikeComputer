...
| `status-command-result-ack.bin` | `status` msg 3 §4.3 | a `commandResult` that carries a `detail` byte: `ok`, `detail` 3. Its `cmd` is a retired id, so the file pins the four-byte layout and the `detail` field, not a live command |
