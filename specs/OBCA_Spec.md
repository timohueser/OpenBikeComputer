...
editor with no re-bake and no server. Styles may change drawing order freely.
...
| 12 | Max Lat | 4 | `int32` | |
| 16 | Max Lon | 4 | `int32` | |
| 20 | Bytes | 4 | `uint32` | Shard size in bytes. **This field counted bytes, not units**, so it did not widen with v14's offsets and did not widen with the read seam either — which made it, for a while, the **narrowest** wall in the format and the one that bound every file a producer wrote (§5.5, §5.7). Every OBCM file this section describes carried one of these records, §5.5's single file included, because that fast path emitted a *set of one* — so `4 GiB − 1` was the writing wall for a lone file exactly as for a set's member, a producer had to apply it before laying any file out, and had to refuse a shard it could not record rather than truncating (a 5 GiB file written as ≈0.7 GiB is worse than a refusal, because every consumer that sized a download from the manifest then trusted a number the file contradicted). It was deliberately **not** widened: a field that dies with the manifest is not worth a format bump. **The wall lifted in FS7.5b2**, when §5's set machinery was removed — this row describes a record nothing writes any more |
...
**That did not move this path's threshold while a manifest existed, and the reason was that this
path is a set of one.** The file it wrote got a §5.2 record like any other shard, and `Bytes` there
was a `uint32` — so the threshold was `4 GiB − 1`, the same wall a set's member cleared. A producer
that gated this path on the reader's wall would have laid out a 5 GiB map, written every byte of it,
and only then discovered that its own manifest could not describe what it had just written.

**FS7.5b2 removed that machinery, which is the lift this paragraph named.** There is no manifest, no
record and no set of one; there is one file, its threshold is §1.1's interior, and the split
machinery went with the wall. The measured figures below are what a country costs — they described
the old boundary and describe the current one just as well, because a country is nowhere near
either.
...
- A consumer **MUST** project every file of the set — core, coarse shards, geometry shards — before
  ...
  ~~**A third wall is narrower than both.**~~ §5.2's `Bytes` was a `uint32` of bytes, so a manifest
  could not record a shard past `4 GiB − 1`, and every file an assembler wrote carried such a record
  — §5.5's single file included, because that fast path emitted a *set of one*. **That wall is gone
  as of FS7.5b2**, which is the event this block named as its own lift condition: the set machinery
  is deleted, nothing writes a manifest, and no `uint32` of bytes stands between a producer and the
  interior.

  So the reading wall and the writing wall are **one number again**, and it is §1.1's interior at
  the file's own `Offset Scale`. The live statement of the producer's obligation is §4.8 item 6.
...
- An assembler **MUST** fail rather than emit an over-size file, and MUST NOT "solve" one by
  splitting the nav graph, dropping POIs, or degrading coverage silently. (It MAY once have split a
...
id per feature type, in one order. A skin may change the other
seven bytes of each record plus the header's `Marker Color`. That is the byte-level meaning of the schema/skin split,
