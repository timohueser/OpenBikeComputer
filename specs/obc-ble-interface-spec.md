...
4. **`transferResult` correlation.** A shard's result echoes its **part**
   ...
   upload become".
5. **A staged set can be abandoned, and an `op=3` naming `mapSet` is how.** A set
   ...
   **The type is load-bearing, because an idle `op=3` has a second meaning.** A
   ...
   descriptor naming `mapShard` (or any type other than `mapSet`) is a quiesce,
   and MUST leave the session and every staged file untouched. A host MUST
   likewise name `mapSet` only when it means abandonment.
...
`mapSet`** (rule 5), deletes the whole set immediately; a power cut is reclaimed
...
rule 5's `mapSet`-only abandonment exists to protect — a re-send is preceded by
...
it MUST change no stored state — see §5 rule 5 for the one descriptor that is
