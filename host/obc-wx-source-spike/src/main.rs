...
use obc_wx_source_spike::{
    ...
    validate_gfs_cumulative_files, validate_grib_file, validate_grib_lead_minutes, validate_gzip_grib_file,
    validate_icon_eu_deaccumulation, validate_met_fixture, validate_met_response, ExpectedGrib,
    HRRR_CONUS_GRID_DEFINITION_HEX, ICON_EU_GRID_DEFINITION_HEX, MRMS_GRID_DEFINITION_HEX,
};

const COMPLEX_SPATIAL_ORDER_PACKING: &[u16] = &[3];
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.as_slice() {
            ...
            println!(
                ...
                validate_gzip_grib_file(
                    ...
                    ExpectedGrib {
                        ...
                        expected_grid_definition_hex: MRMS_GRID_DEFINITION_HEX,
        ...
        [command, path, expected_hour] if command == "icon-eu" => {
            let summary = validate_bzip2_grib_file(
                Path::new(path),
                ExpectedGrib {
                    discipline: 0,
                    category: 1,
            ...
            validate_grib_lead_minutes(
                &summary,
                expected_hour.parse::<u32>()?.checked_mul(60).ok_or("ICON lead overflows")?,
            )?;
            println!("{summary:#?}");
        }
        ...
        [command, path, expected_minutes] if command == "hrrr-prate" => {
            let summary = validate_grib_file(
                ...
                ExpectedGrib {
                    ...
                    product_template: Some(0),
                    representation_templates: COMPLEX_SPATIAL_ORDER_PACKING,
                    expected_messages: 1,
                    require_identical_messages: false,
                    missing_sentinels: NO_SENTINELS,
                },
            )?;
            validate_grib_lead_minutes(&summary, expected_minutes.parse()?)?;
            println!("{summary:#?}");
        }
