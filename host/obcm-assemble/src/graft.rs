...
//! a grid-aligned assembly bbox subdivides onto that square exactly, so the cell's quadtree subtree
//! is already the subtree the assembly would build at that position. Grafting it is:
//!
//! - chunk payload bytes — copied verbatim, never decoded;
//! - index nodes — copied with two constants: leaf values `+ chunk_id_base`, branch child bases
//!   `+ (block_base − 1)`;
...
//!
//! Nothing here calls a geometry library, and nothing here allocates per feature. Peak memory is
//! one cell's index block plus the fixed copy buffer, which is what makes a country assemble in a
//! tab.
...
/// One cell's contribution to one LOD region, with its two relocation constants.
...
pub struct GraftCell {
    ...
    /// Added to every copied offset-table entry. Units, not bytes: the entries a cell wrote count
    /// `U`-byte units and the ones the assembly writes count the same units, so the relocation is
    /// one addition in that currency.
    ...
    /// number of units because every chunk is padded to one, which is what lets the region be
    /// copied byte-for-byte and the next cell's block start on a boundary of its own.
    pub chunk_units: u64,
}

impl GraftCell {
    /// The cell's chunk-data region in bytes — what the verbatim copy moves.
    #[inline]
...
pub struct LodPlan {
    ...
    /// into its depth-`d` slot.
    pub upper: Vec<u32>,
    pub cells: Vec<GraftCell>,
    pub node_count: u32,
    pub chunk_count: u32,
    /// The region's chunk data, in bytes — every cell's contribution, each already a whole number
    /// of units.
    pub chunk_bytes: u64,
}

impl LodPlan {
    /// An empty region: no index, no chunk, and the single-`0`-entry offset table the format
    /// mandates. This is what a shard writes for a LOD its role does not carry, and what a cell
    /// writes for an out-of-band level.
    ...
    /// Total bytes of the region: index + offset table + filler + chunks.
    ///
    /// The region's index starts on a unit boundary, and the index and the offset table are both
    /// read by 4-byte indexing from there — so the one rounding step between `table_end` and
    /// `data_start` is a function of the two counts alone and needs no absolute offset.
    /// [`emit_lod`] finds that same step by asking its cursor for the boundary, and asserts the two
    /// agree.
    ///
    /// A region always ends on a unit boundary, so the next LOD's index, and the POI section behind
    /// the last of them, start on one without the caller aligning anything.
...
/// depth-`d` node index, which is the format's BFS order, so the output is a pure function of which
/// cells are present and never of fetch order.
pub fn plan_lod(
    lod: usize,
    max_mpp: Option<f64>,
    chunk_size: usize,
    ...
    // leaf where none is. Children are appended contiguously, so a branch's first-child index is
    // the node count at the moment it is expanded — the reader's `walk_leaves` invariant.
    ...
    for (slot, c) in slots {
        ...
        // chunk id with `EMPTY_LEAF`, so the usable ranges stop one bit and one value short.
    ...
    // Inline each present cell's relocated root into its depth-`d` slot.
    for g in &plan_cells {
...
/// `chunk_id_base`, an empty leaf not at all.
///
/// Every word is validated against the source cell first. A cell is an input, not the assembler's
/// own output, and an index word out of its cell's range relocates into a plausible index — a
/// branch pointing into another cell's subtree, or a leaf naming a chunk that belongs to a
/// neighbour. The verify pass cannot tell that apart from a correct map: it decodes, it just draws
/// someone else's geometry. So the bound is checked here, where the source cell's own counts are
/// still in hand.
#[inline]
...
/// Nothing is decoded and no chunk byte is touched.
///
/// The filler run is there because the chunks are addressed by scaled offsets, so `data_start` has
/// to be a unit boundary while the index and the table behind it, read by 4-byte indexing, do not.
/// Its bytes are `0xFF` like every other gap.
pub fn emit_lod(plan: &LodPlan, cells: &[Cell<'_>], w: &mut MapWriter<'_>) -> Result<()> {
    let start = w.at();
    if plan.node_count == 0 {
        // The mandatory single-`0` offset table, plus the filler that leaves the next region's
        // index on a boundary.
    ...
    // 2. The offset table: `chunk_count + 1` entries in units, each cell's shifted by its unit
    // base. Every copied pair is re-checked against the capacity bound — a cell that violated it
    // would poison the assembly.
    ...
    for g in &plan.cells {
        ...
        for (k, word) in raw.as_chunks::<4>().0.iter().enumerate() {
            ...
            // A chunk's content may not exceed `Chunk Size`, so its span is that content rounded up
            // to a unit. The looser `Chunk Size + U - 1` would admit spans no writer can produce.
    ...
    // 3. The filler that carries the region to `data_start`, then the chunk bytes, verbatim. One
    // streaming copy per cell, and the copy needs no per-chunk repadding: the cell's own chunks are
    // already unit-aligned, so its whole region moves as one run of bytes.
    w.begin_section()?;
    for g in &plan.cells {
        if g.chunk_units == 0 {
            continue;
...
mod tests {
    ...
    /// The spec's worked example, in index values: A is a five-node subtree with two chunks, B a
    ...
    /// these produces a plausible index into another cell's block — the one graft failure the
    /// verify pass cannot see, because the result still decodes.
    #[test]
