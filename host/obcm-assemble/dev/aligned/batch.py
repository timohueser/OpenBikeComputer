...
parser.add_argument('--candidate-maps', type=Path, required=True)
...
for case in manifest['cases']:
    ...
    for pair in range(3):
        ...
        for label in labels:
            ...
            cmd = [str(getattr(args, label).resolve()), str(args.candidate_maps / case['map'] if label == 'candidate' else map_path), *map(str, case['from'] + case['to']), str(case['profile']), str(output)]
            if 'original' in case:
            ...
            for run in runs:
                ...
                if label not in common:
                    common[label] = fixed
                assert fixed == common[label], f"behavior changed: {case['name']} {label}"
    ...
    for key in ['workspace_bytes', 'outcome', 'output_bytes', 'from_interior']:
        assert common['baseline'][key] == common['candidate'][key], f'{case["name"]}: {key} differs'
    results.append({'case': case['name'], 'input_sha256': digest(map_path), 'candidate_input_sha256': digest(args.candidate_maps / case['map']), 'output_sha256': expected[case['name']], 'invariant_per_plan': common, **groups})
report = {'platform': platform.platform(), 'warmups': 10, 'measured_plans_per_process': 250, 'pairs': 3, 'instrumentation': 'All nav-metrics disabled. Every iteration verifies bytes/outcome/workspace against first run; driver verifies the common OBCR SHA against pinned NG1 output; other counters are deterministic within each variant. Workspace, outcome, output size and snap type agree across variants.', 'binaries': {label: digest(getattr(args, label)) for label in ['baseline', 'candidate']}, 'results': results}
