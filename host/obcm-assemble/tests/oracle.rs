...
#[test]
fn initial_skin_stamping_uses_the_cells_canonical_rain_bands() {
    // Give the real cutter an explicit upper-band road; the other fixture styles stay below rain.
    let cfg = Config::parse(&CONFIG.replace("\"z_index\": 5", "\"z_index\": 30")).unwrap();
    let (ing, ways) = fixture(&cfg);
    let dir = scratch("initial-skin-bands");
    let full = cut(&dir, &cfg, &ing, &ways);
    let mut loaded = load(&dir, &full);
    let schema = schema(&cfg);
    assert!(schema.styles.is_empty(), "exercise the local CLI's id-only schema path");
    let skin = skin(&cfg);
    let road = skin.styles.iter().position(|s| s.z_index == 30).unwrap();
    let ground = skin.styles.iter().position(|s| s.z_index == 1).unwrap();
    let run = |loaded: &Loaded, skin: &Skin, store: &mut MemoryStore| {
        let inputs = loaded
            .cells
            .iter()
            .zip(&loaded.bytes)
            .map(|((id, band, partial), src)| CellInput { id: *id, band: band.clone(), partial: *partial, src })
            .collect();
        assemble(inputs, &schema, skin, &Options { accept_partial: true, ..Default::default() }, store, &NoClock)
    };
    let mut baseline = MemoryStore::default();
    run(&loaded, &skin, &mut baseline).unwrap();
    let mut within = skin.clone();
    within.styles[road].z_index = 24;
    within.styles[ground].z_index = 16; // reorder this fill above the other lower-band styles
    let mut expected = baseline.map.0.clone();
    obcm_assemble::emit::restamp_style_table(&mut expected, &within.resolve(&schema).unwrap(), within.marker_color)
        .unwrap();
    let mut result = MemoryStore::default();
    run(&loaded, &within, &mut result).unwrap();
    assert_eq!(result.map.0, expected, "within-band assembly is byte-identical to a valid restamp");

    // Both cross-band directions are invalid even though the skin avoids the reserved gap.
    for (index, z) in [(road, 16), (ground, 24)] {
        let mut swapped = skin.clone();
        swapped.styles[index].z_index = z;
        let mut untouched = MemoryStore { map: MemorySource(vec![0xA5]) };
        let error = run(&loaded, &swapped, &mut untouched).unwrap_err().to_string();
        assert!(error.contains("id/band assignment"), "{error}");
        assert_eq!(untouched.map.0, [0xA5], "refuse before beginning or writing output");
    }

    // Keep every ID fixed while changing just one producer cell's road baseline.
    let cell = &loaded.bytes[0].0;
    let offset = obcm_assemble::emit::header_style_offset(cell).unwrap() as usize + 1 + road * 8 + 1;
    assert_eq!(cell[offset], 30, "the canonical value came from the cutter, not the selected skin");
    for (z, message) in [(16, "disagree"), (20, "reserved rain band gap")] {
        loaded.bytes[0].0[offset] = z;
        let mut untouched = MemoryStore { map: MemorySource(vec![0xA5]) };
        let error = run(&loaded, &skin, &mut untouched).unwrap_err().to_string();
        assert!(error.contains(message), "{error}");
        assert_eq!(untouched.map.0, [0xA5]);
    }
    loaded.bytes[0].0[offset] = 31;
    let mut result = MemoryStore::default();
    run(&loaded, &skin, &mut result).unwrap();
    assert_eq!(result.map.0, baseline.map.0, "cell agreement concerns bands, not exact placeholder z values");
}
