...
impl<'a> RecordingUpstream<'a> {
    pub fn new(inner: &'a mut dyn Upstream) -> Self {
        Self { inner, role: Role::Service, captured: Vec::new(), lengths: BTreeMap::new() }
    }
...
pub fn capture(root: &Path, request: &CaptureRequest, network: &mut dyn Upstream) -> Result<CaptureReport, String> {
    ...
    let mut recorder = RecordingUpstream::new(network);
    ...
    // --- the observed ladder, from the same anchor.
    recorder.set_role(Role::Truth);
    let mut truth_frames = Vec::new();
    for requested in &request.truth_offsets_min {
        let (offset_min, valid_at) = snap_truth_offset(anchor, *requested);
        // Two requests closer together than the observation cadence would snap onto one instant
        // and one file. Say so rather than silently publishing a ladder with a rung missing.
        if let Some(clash) = truth_frames.iter().find(|frame: &&TruthFrame| frame.offset_min == offset_min) {
            return Err(format!(
                "truth offsets +{} and +{requested} min both resolve to +{offset_min} min — they are closer together than the {}-second observation cadence",
                clash.requested_offset_min,
                mrms::CADENCE_SECONDS
            ));
        }
        let bytes = bake_truth_frame(&mut recorder, anchor, offset_min, valid_at, request.bbox)?;
        let path = format!("{TRUTH_DIR}/f{offset_min}.obcg");
        write_file(&root.join(TRUTH_DIR).join(format!("f{offset_min}.obcg")), &bytes)?;
        truth_frames.push(TruthFrame {
            requested_offset_min: *requested,
            offset_min,
            valid_at: manifest::rfc3339(valid_at),
            path,
    ...
    event.write(root)?;
    Ok(CaptureReport { event, cycle, upstream_bytes, service_bytes, truth_bytes })
}
