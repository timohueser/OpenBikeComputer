//! Can pure Rust decode a real Copernicus GLO-30 tile? The one thing that was never on the table is
//! a native dependency in the packer tree, and the answer is yes: the `tiff` crate reads the
//! mirror's tiles as they are shipped.
//!
//! ```sh
//! cargo test -p obc-dem --test decode -- --ignored --nocapture
//! ```
//!
//! `#[ignore]`d: it downloads about 40 MB from the AWS Open Data mirror, cached under `target/`
//! (override with `OBC_DEM_TEST_SOURCES`). What it pins is the source's shape, the layer below any
//! OBCT byte; `tests/real_tile.rs` carries the same tile through a bake and back.
...
/// The tile this runs against: 46–47 °N, 8–9 °E — the Bernese and Urner Alps, and the square the
...
fn the_tiff_crate_decodes_a_real_glo30_tile() {
    ...
    // 1024 × 1024 internal tiles. No GDAL, no C, no shelling out.
    ...
    // `PixelIsPoint` that is the centre of the north-west post, so the south-west post — post (0, 0)
    // after the ingest flip — is exactly 3599 steps below it. Under `PixelIsArea` it would sit half
    // a step inside on both axes instead, and every sample in the bake would be about 15 m out.
    ...
    // nothing downstream may assume one global post lattice.
    ...
    // Orientation and water flattening, in one number. Lago Maggiore sits in the far south-east of
    // this square and is regulated at about 193 m; GLO-30 flattens it, so the tile reads exactly 193
    // over the whole lake. A raster ingested without the row flip would answer the height at
    // 46.87 °N instead — the Schächental, well over a kilometre up.
    ...
    // This square is entirely land and lake, so the tile has no voids. Water being flattened rather
    // than voided is why the bake has no inpainting to do: a lake arrives as a constant.
