//! A real Copernicus GLO-30 tile, decoded in pure Rust, baked, and read back through the shared
//! reader against surveyed elevations.
//!
//! `#[ignore]`d because it downloads about 40 MB from the AWS Open Data mirror. Run it
//! deliberately:
//!
//! ```sh
...
//! internal tiles, `PixelIsPoint`. That combination is what the `tiff` crate has to handle for a
//! bakery-side preconversion step to stay unnecessary.
//!
//! Road passes are pinned because a surveyed summit sign is a number anyone can check, and a pass
//! saddle is exactly the terrain a 30 m DSM represents well. Deliberately not pinned:
//!
//! - Sharp summits. Finsteraarhorn (4274 m surveyed) reads 4086 m in this tile, the highest sample
//!   in the whole 1° square. A pyramidal snow-and-ice peak is not something a 30 m surface model
//!   resolves, and pinning one would be pinning the dataset's limit, not our arithmetic.
//! - Tunnelled passes. The Susten road crosses its summit in a tunnel; the DSM reads the mountain
//!   above it, 2299 m against the road's 2224 m. Correctly.
//! - Reservoir surfaces as a surveyed number: a dam's crest elevation is not the water level on the
//!   day the radar flew. The lake check below asserts flatness, which is the property GLO-30's
//!   water flattening actually promises.
...
fn a_real_glo30_tile_decodes_bakes_and_reads_back_at_surveyed_elevations() {
    ...
    // Built from the tiles the box needs, not from everything in the cache directory: the same cache
    // also holds the Teningen tile once `build-map-package.sh terrain` has run.
    ...
    // A 2^16 cell keeps the fixture a few hundred KB while the posting — the thing that decides the
    // heights — stays the real v1 one.
    ...
    // Surveyed spot checks, through the shared reader. The tolerance is the source's, not ours:
    // GLO-30's stated vertical accuracy is 2 to 4 m RMSE and a pass sign is a point on a road, not
    // on the DSM's smoothed saddle.
    ...
    // why the assertion here is flatness rather than a surveyed height.
    ...
    // The plausibility envelope: the box spans the Grimsel and Furka roads and the Aare gorge below
    // them.
