//! Crest lifts: where a finer reference DEM says our lattice loses a summit.
//!
...
//! it loses the one feature that makes the mountain recognisable. The format lets the producer take
//! the sample at such a node from the reference instead, and this is that rule.
//!
//! The lift is added to the native height before the cell is baked, so the pyramid, the maxima, the
//! error codes and the cross-cell index all describe one surface. Peak View, the contours, the
//! ascent integral, the route profile and the altimeter therefore read the same heights.
//!
//! The rule is two tests and one dilation:
//!
//! * `LIFT_M` against the bilinear surface, not the node. The interesting quantity is how far the
//!   reference stands above the surface we actually draw, measured where it stands there. That same
//!   gap is also how far a node rises, because `node_max - native` is the difference between a
//!   maximum over a node's cell and a point sample at its centre, and on a slope that is half a
//!   posting of fall rather than a crest.
//! * `CONVEX_M` on the reference's own node maxima. Without it, a steep planar face reads as a
//!   crest, because a coarse lattice under-samples a 40° slope honestly, and the whole mountain
//!   inflates. It is also what leaves a saddle alone, so a pass does not move.
//! * One node of dilation, bounded by the gap. A lifted node beside unlifted neighbours makes the
//!   bilinear surface sawtooth, which the panorama shows as a jittering skyline, so the selection
//!   is extended by one node and a crest is lifted along its whole length. The gap keeps that
//!   honest: a dilated node where the reference stands at or below the surface rises by nothing.
//!
//! Every part of the rule reads only a node's 2-ring, and [`LiftMap::bake`] scans that ring beyond
//! the cell it is asked for ([`HALO`]). A node on a cell seam therefore gets the same lift whichever
//! of the two cells computes it, which is what keeps one published cell byte-identical to the same
//! square inside a wide shard.
//!
//! The reference is the archive of `reference.rs`: max-pooled whole metres on a `2^6` µdeg lattice,
//! in tiles a bake streams. So the scan is pixel-driven, not node-driven. It visits the tiles the
//! cell's node window touches, one decoded tile in memory at a time, and drops each pixel into the
//! node whose half-posting cell holds its centre. Every archive pixel inside the window is read
//! exactly once, which is both the cheapest possible pass and a finer probe grid than any node-side
//! sub-sampling.
//!
//! A pooled pixel is a maximum, not a sample, so the gap under one is measured against the highest
//! the native surface reaches over that pixel's own footprint ([`roof`](NativeWindow::roof)).
//! Measuring it at the pixel's centre instead reads the gap high wherever the ground is steep, and
//! lifts nodes the reference does not stand `LIFT_M` above.
...
///
/// Native level only. The coarser levels select native posts, so a pyramid baked through
...
pub struct LiftMap {
    ...
    lifts: Vec<i16>,
    tally: LiftTally,
    /// Every source that contributed a pixel to a tile this cell decoded, sorted. An index entry
    /// whose tile a mirror does not hold is not in here: a container cannot be derived from bytes
    /// the bake never read, and attribution follows what was read.
    sources: Vec<String>,
}
...
impl LiftMap {
    ...
    pub fn bake(
        ...
        // span several nodes — and the pass would silently produce no lift at all.
        ...
        // A node with a hole anywhere in its 3×3 native ring is out of the rule altogether: there is
        // no bilinear surface there to measure a gap against. Deciding it once per node rather than
        // once per pixel keeps the hole test off the inner loop, and it is what lets the loop below
        // interpolate without checking for a hole, because a pixel lands in the node nearest it and
        // the interval holding the pixel's whole footprint has all four corners in that node's
        // 3×3 ring.
        ...
        for y in 0..=side {
            for x in 0..=side {
                ...
                // The node rises by the gap — how far the reference stands above the surface — and
                // never past `node_max`, the highest reference the node owns. Never negative
                // either: the reference may sit below our surface in a hollow, and the rule raises
    ...
    /// Every reference source this cell's lifts are derived from, sorted — the attribution that must
    /// travel with a container holding this cell.
...
/// and the rounding then goes north and east.
...
impl Scan {
    ...
    /// Whether the scan holds this node and the ring around it, which is what the convexity test
    /// reads. `HALO` guarantees it for every node a map stores.
...
impl NativeWindow {
    ...
    /// Whether any of the 3×3 native heights around a node is a hole.
    ...
    /// The highest the native surface reaches over one archive pixel's footprint: the pixel's own
    /// square, [`HALF_PIXEL`] µdeg either side of the centre this is given.
    ///
    /// The gap has to be a lower bound on how far the reference stands above our surface, and an
    /// archive pixel is a maximum that may have come from anywhere inside its square. Measuring it
    /// against the surface at the pixel's centre therefore reads the gap high on steep ground — up
    /// to 3 m on a 40° face, a third of the 10 m gate — and lifts nodes the reference does not
    /// stand 10 m above. Against the roof of the footprint the gap can only read low.
    ///
    ...
    ///
    /// Requires a posting of at least [`STEP_LOG2`], which [`LiftMap::bake`] refuses otherwise. At
    /// or above that step the lattice interval boundaries are multiples of the pixel side, so a
...
/// answer there, and inventing one would make the lift depend on which cell asked for it.
...
/// Whether a node is selected or touches one, which is the one-node extension. 8-connected: a node
/// that touches a selected node at a corner is lifted too. [`HALO`] keeps the whole ring inside
/// `core` for every node a map stores, so there is nothing to bounds-test here.
