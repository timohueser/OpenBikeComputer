//! The **legacy** command/event dispatcher — the conformance baseline, and nothing else.
//!
//! This is the frame the three non-board hosts ran until #1397 S6a: one [`LegacyLoop`] drains
//! `App`'s [`HostMailbox`] once per pass and applies the eighteen commands in
//! [`HostCommand::DRAIN_ORDER`] against the caller's repositories. It has **no production caller**
//! any more — the simulator, its headless driver and the web demo drive [`crate::HostLoop`]'s typed
//! executor over `App::run_pass` — and it survives for exactly two reasons:
//!
//! - the DC1 corpus's `legacy-immediate` / `legacy-delayed` runners compare DeviceCore against it
//!   (`device_core_conformance.rs`), and
//! - the board-parity fixtures pin the board's `HostPass` against the same command semantics
//!   (`board_parity.rs`).
//!
//! It is deleted with the legacy protocol itself at #1397 S6c, once the board's own executor
//! (S6b) has removed the last production `drain_host_commands`.

use obc_app::{App, DrainStatus, HostCommand, HostMailbox, TrackAction};

use crate::dispatch::{feed_routes, reconcile_track, InflightPlan, PlanHold};
...
pub struct LegacyLoop {
    ...
    /// The resident active-route parse — the caller reads it (via [`session`](LegacyLoop::session))
    /// to open the tick+render [`RouteReader`](obc_route::RouteReader) after [`reconcile`](LegacyLoop::reconcile).
...
impl LegacyLoop {
    ...
    pub fn new() -> Self {
        LegacyLoop { mailbox: HostMailbox::new(), plan: None, detour_ready: None, session: ActiveRouteSession::new() }
    }
