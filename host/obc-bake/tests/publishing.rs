...
fn tree_with(dir: &Path, regions: &[&str]) -> PathBuf {
    ...
    std::fs::copy(repo("builder/presets/schema.json"), tree.join("presets/bikepacking.json")).unwrap();
    for region in regions {
        let region_dir = region.split('/').fold(tree.join("regions"), |p, s| p.join(s));
        std::fs::create_dir_all(&region_dir).unwrap();
        std::fs::copy(repo("apps/obc-sim/assets/monaco.obcm"), region_dir.join("bikepacking.obcm")).unwrap();
        std::fs::write(
            region_dir.join("bikepacking.obcm.json"),
        ...
        std::fs::write(region_dir.join(obc_bake::bake::state_file_name("bikepacking")), "{}").unwrap();
    }
    tree
}
...
fn the_manifest_is_published_last_and_the_bake_state_is_not_published_at_all() {
    ...
    assert!(puts.contains(&"presets/bikepacking.json".to_string()));
...
fn a_failed_artifact_upload_never_swaps_the_manifest_in() {
    ...
    let tree = bake_tree(&dir);
    let store = RecordingStore::failing_on("regions/europe/beta/gamma/bikepacking.obcm");

    let err = obc_bake::publish::publish(&tree, &store, &opts(), live()).unwrap_err();
    assert!(err.contains("regions/europe/beta/gamma/bikepacking.obcm"), "{err}");
...
fn an_object_that_is_not_fetchable_after_upload_blocks_the_swap() {
    ...
    let store = RecordingStore::losing("regions/europe/alpha/bikepacking.obcm");
...
fn a_directory_publish_produces_a_servable_copy_of_the_tree() {
    ...
    assert!(!dest.join("regions/europe/alpha").join(obc_bake::bake::state_file_name("bikepacking")).exists());
...
fn publishing_a_partial_bake_over_a_full_catalog_is_refused() {
    ...
    assert!(err.contains("europe/beta/gamma [bikepacking]"), "the error must name what disappears: {err}");
...
fn a_deliberate_shrink_proceeds_loudly_with_allow_shrink() {
    ...
    assert_eq!(report.coverage_lost, vec!["europe/beta/gamma [bikepacking]".to_string()]);
    assert!(
...
fn a_dir_publish_diffs_against_the_manifest_already_in_that_directory() {
    ...
    assert!(err.contains("europe/beta/gamma [bikepacking]"), "{err}");
    ...
    assert!(dest.join("regions/europe/beta/gamma/bikepacking.obcm").is_file());
