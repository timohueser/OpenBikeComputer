...
//! # The ownership rule
//!
//! A cell on the German/Swiss border baked from the German extract alone is missing every Swiss
//! side road, and only about half of each side's junctions exist in the other's file. So a
//! multi-region bake computes, for every cell, its source set:
//!
//! > A cell's source set is every maximal co-baked extract whose coverage polygon intersects the
//! > cell's square.
//!
//! Maximal, because Geofabrik's extracts are hierarchical cutouts: a region nested in another baked
//! region carries a strict subset of its parent's data, so reading its file adds bytes and a
//! snapshot skew, never coverage. Nested regions are selection-only — their coverage still picks
//! cells and their documents still publish — but plans read only the maximal extracts.
//!
//! Each cell is then cut exactly once, from an ingest of exactly that source set.
//! `obc-bake bake europe/germany europe/switzerland` runs three cut plans: Germany-only cells,
//! Switzerland-only cells, and the border cells both touch, cut from both extracts together so the
//! seam is complete in one file.
//!
//! Three properties make that rule safe rather than merely plausible:
//!
//! - It is a pure function of (cell, the run's extracts). No ordering, no first-writer-wins, no
//!   dependence on which region the operator listed first. The plans themselves are keyed by the
//!   sorted source set and executed in that order.
//! - It never writes a cell twice. One (band, i, j) is one path and exactly one plan owns it, so
//!   the "canonical and partial for the same id" state the catalog forbids cannot arise from a
//!   single run.
//! - It cannot silently downgrade. Across runs it could: baking Switzerland alone after a DE+CH
//!   bake would re-cut a border cell from one source. [`install_cell`] refuses that.
//!
//! `partial` is decided here rather than in the cutter, because the cutter's coverage input is a
//! box and a country is not a box: Germany's bounding box contains a slab of Czechia, and a box
//! test would publish those cells as canonical. [`crate::coverage`] decides it against the real
//! polygons, over the union of a border cell's whole source set.
//!
//! # What "unchanged" means
//!
//! The skip state uses two keys at cell granularity. The pack key hashes everything that can move a
//! byte — cell and crop recipe versions, OBCM version, schema config and revision, the band table,
//! the cutter's own description, the plan's crop, and the sorted `(extract id, extract SHA-256)`
//! pairs of the source set — by content, never by mtime. The sidecar facts, which are the sources'
//! snapshot dates, are compared separately, so a mirror re-publishing byte-identical extracts under
//! a new date rewrites four-line JSON files and re-cuts nothing.
//!
//! A plan whose every cell is current is skipped before its extracts are ingested, which is what
//! makes a re-run of a DACH bake minutes rather than hours.
//!
//! # The crop
//!
//! Every plan crops its extracts to its supercell square widened by [`CROP_MARGIN_UDEG`], the
//! relation-complete `--bbox` crop, which keeps every way touching the area, every renderable area
//! relation reached by those ways, and therefore every junction and polygon inside it. The crop is
//! part of the pack key, so it is part of the cell's recipe rather than an invisible variable. A
//! relation can still be incomplete at a Geofabrik extract's own edge; those genuinely missing
//! members are an input limitation, not something the crop guesses around.
...
/// How far past its supercell square a plan crops its extracts, µdeg (0.25° ≈ 28 km at DACH
/// latitudes).
///
/// Comfortably past Geofabrik's complete-ways overhang and past any single way, so the crop cannot
/// change what is a junction inside the plan's cells.
...
/// What actually cuts. A trait so the tests can drive plan building, the ownership rule,
/// installation, the downgrade guard and the skip logic without libGEOS and without a
/// multi-gigabyte extract — and so a test can inject a corrupt cell.
...
pub struct CellBakeOptions {
    ...
    pub fail_fast: bool,
    /// The schema's band table — schema data, published in the catalog and read back by every
    /// consumer.
    pub bands: BandTable,
    ...
    /// The terrain already in the tree, which the nav graph's `Ascent M` is integrated from, and
    /// the one direction the two revision tracks are coupled in. `None` bakes ascents of zero,
    /// which is decode-valid.
    ///
...
pub enum CellStatus {
    ...
    SidecarRefreshed,
    /// A canonical cell on disk was not replaced by a partial re-cut.
    KeptCanonical,
}
...
pub struct CellRunSummary {
    ...
    /// The ground this run's sources cover, km² — the denominator, and the one number that makes
    /// the table comparable with the spec's.
    pub covered_km2: f64,
...
/// One band's published footprint — the numerator of the store's density figure.
#[derive(Debug, Clone, Serialize)]
...
impl BandStats {
    /// MiB per 1000 km² of covered ground: the latitude-free unit the spec tabulates.
    ///
    /// Deliberately not per canonical cell square. A partial cell's bytes are the bytes of the part
    /// of its square the sources cover, so dividing the band's whole byte count by the sources' own
    /// area counts each byte once against the ground it describes. Dividing by canonical squares
    /// would throw away every edge cell, and a region the size of Freiburg-Regierungsbezirk has no
    /// canonical coarse cell at all.
...
/// One region's covered ground, so a per-region density can be computed from the catalog's own
/// per-region byte counts.
#[derive(Debug, Clone, Serialize)]
...
impl CellRunSummary {
    ...
    /// The run report, loud end first, density table included.
...
struct Resolved {
    region: Region,
    /// `None` for a selection-only region: one nested inside another baked region. Geofabrik's
    /// extracts are hierarchical cutouts, so the child's data is a strict subset of the parent's
    /// and reading the child's file per plan adds bytes, wall time and a latent snapshot skew,
    /// never data. The region keeps every other role: its coverage still selects cells, its
    /// document and boundary still publish, and it is still a catalog entry a rider can pick.
...
pub struct CellBakery<'a> {
    ...
    /// The packer config the cells are baked with — the schema. Its style-id assignment is baked
    /// into every chunk.
    pub schema: &'a StyleDoc,
    /// The skins published beside it. Each must style exactly the schema's feature types with
    /// exactly the schema's ids; [`CellBakery::run`] proves that before it cuts anything, and the
    /// catalog generator proves it again on the finished tree.
...
impl CellBakery<'_> {
    ...
    pub fn run(&self, progress: &Progress) -> Result<CellRunSummary, String> {
        ...
        // The density denominator: the union of the run's coverage polygons, so ground two regions
        // share is counted once, exactly as its cells are stored once.
    ...
    /// Every skin must fit the schema — checked before the first extract is fetched.
    ///
    /// The catalog generator makes the same check on the finished tree, so this one buys only the
    /// moment it fails at: a run that publishes a skin the generator will refuse otherwise spends
    /// hours cutting cells first and ends with a tree that has no catalog. A skin that does not fit
    /// is never a small mistake either — it means the document is a different schema, and the
    /// answer is a schema revision and a re-bake.
    fn check_skins(&self) -> Result<(), String> {
        for skin in self.skins {
            // Two different questions, and a document can fail either. Is it presentation only?
            // That is a question about the text, because a config cannot tell an absent `lods`
            // from one restating the defaults. And does it style exactly the schema's feature
            // types, with exactly its ids?
    ...
    fn resolve(&self, progress: &Progress, uncovered: &mut Vec<String>, warnings: &mut Vec<String>) -> Vec<Resolved> {
        ...
        for region in self.regions {
            ...
            // A region nested inside another baked region contributes selection only: its extract
            // is a Geofabrik cutout of the parent's, so its data is already in the run.
            let parent = self
                .regions
                .iter()
                .map(|p| &p.id)
    ...
    fn run_plan(
        ...
        // Every plan crops, single-source plans included: without a crop, an interior plan ingests
        // its whole extract per bucket. The box is the plan's supercell square and not its cells'
        // union, because the bucket is a pure function of each cell, so a cell's crop — and
        // therefore its pack key — cannot depend on which neighbours happened to be selected or
        ...
        let opts = CutOptions {
            ...
            select: stale.clone(),
            only_bands: Vec::new(),
            // Provenance without a coverage box: a country is not a box, so the canonical/`partial`
            // decision is taken here against the real polygons and the cutter is told only who the
            // sources are.
            sources: sources
                .iter()
                .map(|r| SourceExtent {
    ...
    ///
    /// Deliberately not in here: the extracts' snapshot dates, and the schema document's `_meta`
    /// block. Both are published facts that must not go stale, and neither can move a byte. The
    /// schema's revision and band table are in the key, but as the run's own `--schema-revision`
    /// and `--bands`, which is where they come from on this path.
    fn pack_key(&self, sources: &[&Resolved], crop: Option<&str>, landmark_key: &str) -> String {
        ...
        // digests — the revision is the terrain store's content identity, and hashing thousands of
        // 2 MiB rasters on every run to rediscover it would be absurd.
    ...
    /// The sidecar this run would write for a cell: who baked it, when they were published, and
    /// whether their combined coverage contains the whole square.
    fn sidecar_for(
        &self,
        cell: CellId,
        sources: &[&Resolved],
        ...
        let partial = match (coverage, boundaries.get(&cell.log2)) {
            // No coverage at all — a GEOS union failure — means nothing is demonstrably covered,
    ...
    ///
    /// Three things must agree, and the third is what catches rot: the pack key, the presence of
    /// both artifact and sidecar, and the artifact's current digest against the recorded one.
    ...
    /// A current cell whose published snapshot dates moved: rewrite four lines of JSON, carry
    /// `built_at` forward, re-cut nothing.
    fn refresh_sidecar(
        &self,
        cell: CellId,
        band: &str,
    ...
    ///
    /// The order is the contract: read the artifact back with the real reader first, refuse to
    /// downgrade a canonical cell second, write the sidecar third, and rename last — so nothing
    /// unverified and nothing less covered than what is already published ever exists under a name
    ...
    fn install_cell(
        ...
        // A covering bake already happened here. Publishing a thinner cell over it would take
        // coverage away silently — and unlike a partial that is merely stale, this one is a
        // regression the operator can fix by co-baking the neighbour again.
        if sidecar.partial {
        ...
        // A sidecar with no cell fails generation just as loudly as a cell with no sidecar, so a
        // failed rename must not leave one behind. Dropping the state file too makes the next run
        // re-cut rather than trust a record of bytes that never landed.
    ...
    ///
    /// The cell list is what the region selects and what exists: a cell the run failed to produce
    /// is dropped from the list and reported, rather than named in a document the generator would
    /// then refuse whole. Returns whether the region's selection came out complete.
    ...
    ///
    /// `schema.json` is the packer config the cells were cut with, with a `_meta` block carrying
    /// the schema's id, revision and band table — one document rather than two, so the style-id
    /// assignment the catalog publishes and the one baked into the chunks cannot disagree.
    ///
    /// The id is checked, not written. `--schema-id` and the document's own `_meta.id` are two
    /// statements of one fact, and overwriting the document with the flag makes a disagreement
    /// unobservable: a typo would publish one schema's cells under another's name, and a store's id
    /// is the identity a rider's already-downloaded cells are matched against.
...
/// wrong order here means baking terrain on Monday and refreshing the map on Tuesday, which is the
/// point of separate revision tracks.
...
/// Whether the published facts drifted — the only thing that can change without the bytes
/// changing. Extract ids are part of the pack key, so only the dates can move.
fn sidecar_drift(have: &CellSidecar, want: &CellSidecar) -> bool {
    have.sources != want.sources
}

/// Group every selected cell by its source set: the co-baked extracts whose coverage polygon
/// touches its square.
///
/// The grouping is the ownership rule and is a pure function of its input — the key is the sorted
/// index list, the output is ordered by it — so two runs over the same regions produce the same
/// plans in the same order.
///
/// A plan is additionally split by locality into [`RUN_SPAN_UDEG`] buckets. One source set can own
/// a country's whole interior, and the packer ingests and prepares a plan's entire crop box before
/// it writes a cell, so an unbounded plan is how a DACH bake runs a 16 GB machine out of memory.
/// The split changes nothing about a cell's identity: its source set is the pure function above,
/// untouched, and only which neighbours share its ingest shrinks. Bucketing is by the cell square's
/// min corner in supercell units, through `BTreeMap`s, so the plan list stays a deterministic
/// function of the inputs.
fn is_path_parent(parent: &str, child: &str) -> bool {
    child.len() > parent.len() && child.starts_with(parent) && child.as_bytes()[parent.len()] == b'/'
}

/// A plan source's extract. Plans are built from sourcing regions only, so this cannot miss; the
/// message is for the day that invariant breaks.
...
fn build_plans(resolved: &[Resolved], bands: &BandTable) -> Vec<Plan> {
    ...
    for log2 in &sizes {
        ...
        // A selection-only region's cells are almost always its parent's cells too; the exception
        // is a cell its own `.poly` touches by a sliver the parent's polygon just misses, through
        // boundary jitter between the two files. Geofabrik's cutout still contains that ground, so
        // attach the cell to the nearest sourcing ancestor rather than dropping it.
...
/// How much ground one cut run may span, µdeg: 2^21 ≈ 2.1°, roughly a Bundesland quarter, which is
/// the scale the packer has demonstrably cut in 16 GB. The trade is more runs, each re-reading its
/// extracts over a smaller `--bbox` crop; a bake is hours and re-reads are minutes.
const RUN_SPAN_UDEG: i64 = 1 << 21;

/// The `--bbox` a plan crops its extracts to: its [`RUN_SPAN_UDEG`] supercell square, widened by
/// [`CROP_MARGIN_UDEG`], as the `W,S,E,N` degrees spelling the packer parses.
///
/// The bucket square rather than the cells' union, deliberately: every cell lies wholly inside its
/// min-corner bucket, and the bucket is a pure function of the cell, so the crop a cell was cut
/// under — which is in its pack key — is identical in every bake that selects it. A union box would
/// vary with the plan's shape and re-cut cells whose own recipe never changed.
fn crop_box(cells: &BTreeSet<CellId>) -> Result<Option<String>, String> {
    ...
    for cell in cells {
        ...
        b = Some(match b {
            None => square,
            // Straddling buckets cannot happen for a single plan, since `build_plans` buckets by
            // the same function, but the box stays correct if a caller ever passes a mixed set.
    ...
    // Clamped to the geographic domain, which the grid's world box is wider than: the packer's
    // `--bbox` parser is a geographic one and rejects anything outside ±180/±90.
...
mod tests {
    ...
    /// The maximal-source rule. A nested region is selection-only: its cells are owned by the
    /// parent's extract alone, so no plan ever reads the child's file — and a sliver cell only the
    /// child's `.poly` touches still lands with the sourcing ancestor instead of being dropped.
    #[test]
    ...
    /// The run-span split. One source set can own a country's interior, and the packer ingests a
    /// plan's whole crop box before writing a cell, so an unbounded plan exhausts memory. Two cells
    /// of one region far apart must land in different plans; two in the same [`RUN_SPAN_UDEG`]
    /// supercell must share one, and every plan keeps the cell's true source set.
    #[test]
