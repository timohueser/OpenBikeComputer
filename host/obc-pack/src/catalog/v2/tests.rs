...
/// A scratch directory that removes itself (same approach as the v1 tests: the packer
...
impl TempTree {
    fn new(tag: &str) -> TempTree {
        ...
        p.push(format!("obc-catalog-v2-{tag}-{}-{:?}", std::process::id(), std::thread::current().id()));
...
fn skin_doc(id: &str, name: &str, description: &str, version: u32, marker: &str, preview: Option<&str>) -> String {
    let preview = preview.map_or(String::new(), |p| format!(",\n    \"preview\": \"{p}\""));
    format!(
        r#"{{
  "_meta": {{
    "id": "{id}",
...
fn write_skin(tree: &Path, id: &str, name: &str, description: &str, version: u32, marker: &str, preview: Option<&str>) {
    write(&tree.join(SKINS_DIR).join(format!("{id}.json")), &skin_doc(id, name, description, version, marker, preview));
}
...
fn example_tree(tree: &Path) {
    ...
    write_skin(tree, "default", "Bikepacking", DEFAULT_SKIN_BLURB, 4, "0xF800", Some("previews/default.png"));
    // No preview yet — the field is optional, and the example shows both shapes so a
    // consumer is built against reality rather than against the happy case.
    write_skin(tree, "contrast", "High contrast", CONTRAST_SKIN_BLURB, 1, "0x001F", None);
...
fn opts() -> CatalogV2Options {
    CatalogV2Options::new("https://maps.example.org/catalog/v2/", "2026-07-30T09:00:00Z")
}

fn generated(tree: &Path) -> GeneratedCatalogV2 {
    generate(tree, &opts()).expect("the example tree generates")
}

fn region<'a>(g: &'a GeneratedCatalogV2, id: &str) -> &'a RegionEntry {
    g.root.regions.iter().find(|r| r.id == id).expect("region")
}

fn satellite<'a>(g: &'a GeneratedCatalogV2, rel: &str) -> &'a Satellite {
    g.satellites.iter().find(|s| s.rel_path == rel).expect("satellite")
}

fn cell_index_doc(g: &GeneratedCatalogV2, band: &str) -> CellIndexDocument {
...
fn walks_a_tree_into_a_root_and_its_satellites() {
    ...
    assert!(g.root.artifacts.is_none(), "a v2 tree has no whole-region artifacts to describe");
    assert_eq!(
...
fn a_cell_entry_states_the_bake_and_carries_no_bbox() {
    ...
    assert_eq!(west.url, "https://maps.example.org/catalog/v2/cells/fine/1204/1052.obcm");
...
fn a_region_prices_its_cell_set_per_band() {
    ...
    assert_eq!(ch.cells_url, "https://maps.example.org/catalog/v2/regions/europe/switzerland/cells.json");
    ...
    // Overlap is free in v2: two regions that share ground share the same cells, and
...
fn a_region_boundary_is_a_drawable_outline() {
    ...
    // `[lat, lon]` microdegrees, matching the OBCM header and v1's bbox.
...
fn a_skin_is_the_schema_recolored() {
    ...
    assert_eq!(default.preview.as_deref(), Some("previews/default.png"));
    ...
    assert!(contrast.preview.is_none(), "`preview` stays optional");
    // v1's `preset_version` has no counterpart: a skin is stamped at assembly time, so
...
fn generation_is_deterministic_for_a_given_tree() {
    ...
    write_skin(b.path(), "contrast", "High contrast", CONTRAST_SKIN_BLURB, 1, "0x001F", None);
    write_cell(
        b.path(),
        "fine",
        fine_east(),
    ...
    write_skin(b.path(), "default", "Bikepacking", DEFAULT_SKIN_BLURB, 4, "0xF800", Some("previews/default.png"));
...
fn write_all_atomic_writes_the_satellites_then_the_root() {
    ...
    // guarantee as v1's single document (§11.1).
    ...
    let root_path = t.path().join(super::super::DEFAULT_MANIFEST_NAME);
...
/// The check that replaces v1's stored bbox: the *identifier* describes the cell, and
...
/// A bakery mid-migration serves whole-region artifacts alongside a filling cell
/// store. This generator does not produce that shape, but a consumer built on these
/// types has to be able to read it.
#[test]
fn a_v2_root_may_carry_v1_artifacts() {
    let t = TempTree::new("migration");
    example_tree(t.path());
    let mut root = generated(t.path()).root;
    let v1: super::super::CatalogManifest =
        serde_json::from_str(super::super::CATALOG_EXAMPLE_JSON).expect("the v1 example parses");
    root.artifacts = Some(v1.artifacts.clone());

    let text = root_json(&root);
    let parsed: CatalogV2 = serde_json::from_str(&text).expect("round-trips");
    assert_eq!(parsed, root);
    assert_eq!(parsed.artifacts.expect("artifacts").len(), v1.artifacts.len());
    // And it is genuinely optional: the field vanishes when there is nothing to say.
    root.artifacts = None;
    assert!(!root_json(&root).contains("artifacts"));
}
...
fn every_truncation_of_a_document_fails_to_parse() {
    ...
    assert!(serde_json::from_str::<CatalogV2>(&root).is_ok());
    for cut in (1..root.len()).step_by(23) {
        assert!(serde_json::from_str::<CatalogV2>(&root[..cut]).is_err(), "root truncated at {cut} must not parse");
    }
...
#[test]
fn checked_in_v2_schema_is_the_current_generated_schema() {
    let checked_in: Value = serde_json::from_str(CATALOG_SCHEMA_JSON).expect("checked-in v2 schema is valid JSON");
    assert_eq!(
        checked_in,
        catalog_schema(),
        "schema/catalog.v2.schema.json is stale; regenerate with `cargo run -p obc-pack --bin obc-pack -- schema \
         --catalog-v2 > host/obc-pack/schema/catalog.v2.schema.json`"
    );
}

#[test]
fn the_v2_schema_pins_the_envelope_version_and_the_field_patterns() {
    let s = catalog_schema();
    assert_eq!(s["properties"]["schema_version"]["const"].as_u64(), Some(u64::from(CATALOG_SCHEMA_VERSION)));
    assert_eq!(s["not"]["required"][0].as_str(), Some("presets"), "§11.9 forbids `presets` in a v2 root");
    ...
    // `preview` and `parent` are the optional fields; nothing else may be.
    let skin_required = s["$defs"]["SkinEntry"]["required"].as_array().expect("skin required");
    assert!(!skin_required.iter().any(|v| v == "preview"));
...
fn generated_documents_validate_against_the_checked_in_schema() {
    ...
    compiler.add_resource("catalog.v2.schema.json", schema).expect("add schema");
    let root_id = compiler.compile("catalog.v2.schema.json", &mut schemas).expect("compile root");
    let cell_id =
        compiler.compile("catalog.v2.schema.json#/$defs/CellIndexDocument", &mut schemas).expect("compile cell index");
    let region_id = compiler
        .compile("catalog.v2.schema.json#/$defs/RegionCellsDocument", &mut schemas)
        .expect("compile region cells");
    ...
    let mut v1ish = instance.clone();
    v1ish["schema_version"] = Value::from(1);
    assert!(schemas.validate(&v1ish, root_id).is_err(), "a v1 envelope must not validate as v2");
    let mut with_presets = instance.clone();
    with_presets["presets"] = serde_json::json!([]);
    assert!(schemas.validate(&with_presets, root_id).is_err(), "§11.9: `presets` must not appear in a v2 root");
...
#[test]
fn v2_examples_are_current() {
    let t = TempTree::new("example");
    example_tree(t.path());
    let g = generated(t.path());
    let files = [
        ("catalog.v2.example.json", root_json(&g.root), CATALOG_EXAMPLE_JSON),
        ("cell-index.v2.example.json", satellite(&g, "cells/fine/index.json").body.clone(), CELL_INDEX_EXAMPLE_JSON),
        (
            "region-cells.v2.example.json",
    ...
    // the v1 example uses:
    //   OBC_UPDATE_CATALOG_EXAMPLE=1 cargo test -p obc-pack v2_examples_are_current
        ...
        assert_eq!(
            *checked_in, *regenerated,
            "schema/{name} is stale — regenerate with `OBC_UPDATE_CATALOG_EXAMPLE=1 cargo test -p obc-pack \
             v2_examples_are_current`. (It is generated from the synthetic tree in this module's `example_tree`, so \
             only a manifest-shape change, an OBCM version bump, or a fixture change should move it.)"
        );
    }

    let parsed: CatalogV2 = serde_json::from_str(CATALOG_EXAMPLE_JSON).expect("the example parses");
