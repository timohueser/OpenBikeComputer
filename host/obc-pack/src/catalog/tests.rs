...
const RECOMMENDED_BAND_TABLE: &str = r#"[
    ...
    { "id": "fine", "cell_log2": 18, "lods": [9, 10, 11, 12, 13], "role": "geometry" },
    { "id": "network", "cell_log2": 18, "sections": ["nav", "poi"], "role": "core" }
]"#;
...
fn the_shipped_schema_and_skin_generate_the_current_catalog() {
    ...
    assert_eq!(g.root.schema.lods.len(), 14, "the shipped ladder is 14 rungs");
    assert_eq!(
        g.root.schema.lods.iter().map(|l| l.band.as_str()).collect::<Vec<_>>(),
        [
            "coarse", "coarse", "coarse", "coarse", "coarse", "mid", "mid", "mid", "mid", "fine", "fine", "fine",
            "fine", "fine"
        ],
