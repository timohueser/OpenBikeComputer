...
/// `2^18`, exactly as the v1 table's `fine` and `network` do.
...
/// The same seam property under the **shipped preset** — 7 LODs, real simplify tolerances from 200 m
/// down to 0.5 m, `merge_fills` and `merge_lines` both on, the v1 band table.
///
...
fn the_shipped_preset_still_meets_at_the_seam() {
    ...
    let opts = CutOptions { bands: BandTable::v1(), ..Default::default() };
...
fn a_broken_band_table_is_refused() {
    ...
    let cfg = config();
    let (ing, ways) = fixture(&cfg);
    // The v1 table is a 7-LOD table; this config's ladder has three levels.
    let opts = CutOptions { bands: BandTable::v1(), ..Default::default() };
...
/// constants, and a cutter that only worked at the v1 sizes would be hiding an assumption.
///
