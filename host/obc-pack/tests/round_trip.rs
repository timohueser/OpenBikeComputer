...
fn styles_round_trip() {
    ...
    assert!(s20.dashed && s20.fixed_width && s20.terrain_layer, "the E3 contour style round-trips whole");
    for s in [s1, s5, s12] {
        assert!(!s.fixed_width && !s.terrain_layer, "style {} sets neither new bit", s.id);
    }
