...
//! into the same [`Fix`](obc_ports::Fix)es a GPS driver emits, and [`BaroSensor`] feeds the
//! track's elevation as a barometer would. Both implement the `obc-ports` HAL traits, so a
//! shared app can't tell a replay from live hardware.
//!
