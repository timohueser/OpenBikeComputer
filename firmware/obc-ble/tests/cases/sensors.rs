//! The cycling-sensor GATT codecs, with the byte layouts spelled out by hand. Short and garbled
//! frames must give `None` and never panic.
...
fn hr_8bit_classic_strap_frame() {
    // flags 0x00: 8-bit value, no contact feature; 72 bpm.
    assert_eq!(parse_hr_measurement(&[0x00, 0x48]), Some(HrSample { bpm: 72, contact: None }));
}

#[test]
fn hr_16bit_value() {
    // Flag bit 0 set: the bpm is u16 LE. 0x0140 = 320.
...
fn hr_contact_bits() {
    ...
    // status set but not supported: report nothing
    assert_eq!(parse_hr_measurement(&[0b010, 60]), Some(HrSample { bpm: 60, contact: None }));
}

#[test]
fn hr_rr_bearing_frame_from_garmin_capture() {
    // A real Garmin HRM frame: flags 0x16 = status, supported and RR present, 8-bit bpm 65, then
    // two u16 RR intervals this crate ignores.
...
fn hr_energy_expended_flag_ignored() {
    // Bit 3 = energy expended: two extra bytes after the bpm.
...
fn cps_full_skip_chain() {
    // Bits 0, 2, 4 and 5 all set: the parser skips 1 + 2 + 6 bytes before it reads the crank.
...
fn cps_short_at_every_boundary() {
    // The full-skip-chain frame, truncated at each byte.
    let flags: u16 = (1 << 0) | (1 << 2) | (1 << 4) | (1 << 5);
    let mut frame = vec![];
    frame.extend_from_slice(&flags.to_le_bytes());
    ...
    // The mandatory head alone needs exactly 4 bytes.
...
fn cps_declared_fields_must_be_present_even_without_crank() {
    // Wheel flag only, no crank: a frame truncated inside the declared wheel field is garbled, so
    // not even the mandatory head is trusted.
    let flags: u16 = 1 << 4;
    let mut frame = vec![];
    frame.extend_from_slice(&flags.to_le_bytes());
...
fn cps_crank_after_only_wheel() {
    // Bits 4 and 5 only: the parser skips 6 wheel bytes, then reads the crank.
...
fn cadence_steady_90_rpm() {
    // 3 revs over 2048 ticks (2 s) = 90 rpm.
    let mut c = CrankCadence::new();
...
fn cadence_u16_rev_wrap() {
    // Revs wrap across zero; wrapping_sub gives the true small delta.
    let mut c = CrankCadence::new();
...
fn cadence_u16_time_wrap() {
    // The event time wraps; wrapping_sub gives the true small Δt.
    let mut c = CrankCadence::new();
    assert_eq!(c.update(crank(0, 65_534)), None);
    // 65534 + 2048 = 67582, which is 2046 mod 65536: +3 revs over 2048 ticks = 90 rpm.
    assert_eq!(c.update(crank(3, 2046)), Some(90));
}

#[test]
fn cadence_coasting_returns_zero() {
    // The sensor keeps notifying with unchanged revs and a frozen event time.
    let mut c = CrankCadence::new();
    assert_eq!(c.update(crank(50, 5000)), None);
    assert_eq!(c.update(crank(50, 5000)), Some(0));
    assert_eq!(c.update(crank(50, 5000)), Some(0));
    // Pedalling resumes and computes against the coasting baseline.
...
fn cadence_duplicate_event_holds() {
    ...
    assert_eq!(c.update(crank(13, 1000)), None); // Δt == 0 with Δrevs > 0: hold the baseline
                                                 // The next good frame computes against the held baseline, revs 10 at t 1000.
...
fn cadence_reset_on_reconnect() {
    ...
    assert_eq!(c.update(crank(10, 1000)), None);
    assert_eq!(c.update(crank(13, 3048)), Some(90));
    // Drop the baseline: the sensor counters can reset while it is disconnected.
    c.reset();
...
fn no_prefix_of_any_frame_panics() {
    // Every prefix of every flag-mutated frame goes through every parser.
    let frames: &[&[u8]] = &[
    ...
    for frame in frames {
        for byte in 0..=255u16 {
            // Mutate the flags bytes too, to exercise unusual skip chains.
...
fn classify_prefers_hr_over_power_over_cadence() {
    ...
    // A power meter that also exposes CSC crank data is Power, not Cadence.
    let mut pm = Vec::new();
...
fn classify_prefers_complete_name_over_shortened() {
    // The shortened name comes first; the complete one wins whatever the order.
    ...
    // Only a shortened name: it is used.
...
fn classify_no_sensor_service_is_none() {
    // Flags, a name, and only the battery service: not a sensor to pair with.
    let mut adv = ad(0x01, &[0x06]);
...
fn classify_tolerates_truncation() {
    // A structure that claims more bytes than remain ends the walk, but the HR service seen before
    // the runt is still reported.
    ...
    // An invalid-UTF8 name gives a match with no name, not an error.
...
fn cadence_arbitration_dedicated_wins() {
    // A saved dedicated cadence sensor takes the quantity.
    assert!(!power_crank_feeds_cadence(true));
    // With none saved, the power meter's crank data fills it.
