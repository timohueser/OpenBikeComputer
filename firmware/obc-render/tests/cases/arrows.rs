//! Tests for the route direction-chevron overlay. They feed a static route through the
//! [`RouteOverlaySource`] seam, stroke it into an in-memory `DrawTarget` and count the
//! distinctly-coloured arrow pixels. The arc-length cadence is unit-tested inside the crate.
...
/// A due-north route at a fixed longitude: constant longitude maps straight down the screen by
/// zoom alone, so the on-screen length and the chevron count are easy to reason about.
const NORTHWARD: &[(i32, i32)] = &[(7_800_000, 48_000_000), (7_800_000, 48_001_500), (7_800_000, 48_003_000)];

/// A longer due-north route, so a full screenful of chevrons fits at more than one zoom.
...
/// bbox and total distance derived from the points.
...
/// North-up viewport centred on the route's midpoint, zoomed so the route clears the edges.
fn vp() -> Viewport {
...
fn arrows_are_windowed_to_the_rider() {
    // Chevrons lead ahead of the rider and never trail behind, which is what stops an
    // out-and-back's two passes from colliding. Route start is at the screen bottom, so with the
    // rider near the top the chevrons lead the remaining distance and the lower screen is clear.
    let mut buf = Buf::new(400, 400);
...
fn route_stroke_has_the_requested_width() {
    // The fast quad stroke must render the line at about `weight` px wide; measure the run of
    // route pixels across a clean row.
    let mut buf = Buf::new(400, 400);
    RenderScratch::new().draw_route(&mut buf, &vp(), &StaticRoute(NORTHWARD), ROUTE, 6, ARROW, None);
...
/// Centre-to-centre screen gaps between consecutive chevrons down the route column. Each chevron
/// shows as a short run of white over the magenta stroke, so distinct white runs in the central
/// columns are the chevrons.
...
fn chevron_spacing_is_held_in_screen_space() {
    // Chevron spacing is a fixed screen cadence converted to ground metres at the current zoom, so
    // the on-screen gap stays the same as you zoom. Fixed-metre spacing would scale with zoom.
