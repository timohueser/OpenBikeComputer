...
model whose `apply` is the meaning of a journal record, and §6.3's recovery decision as a pure
function. Two host-only modules sit beside them behind the crate's `std` feature: a deterministic
faulting-media harness that models §1.1's program-page tearing and §13.1's adapter obligations, and
the storage half of the vector suite (`cargo test -p obc-storage regenerate_storage_vectors --
--ignored`, then the `obc-link` regenerate above, which indexes it). There is no engine here yet —
no transaction API, no leases, no garbage collector, no FAT adapter — and nothing in it knows a
filename.
