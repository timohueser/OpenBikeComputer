...
//! lenient: it powers the on-device GPX to OBCR conversion, where one bad point in an otherwise
//! good planner file must not abort the whole route.

use obc_formats::io::SliceSource;
use obc_route::{GpxScanner, RawPoint, RawWaypoint, WptScanner, WAYPOINT_SYMBOL_CAP};

/// Collect every point the scanner yields from `gpx`.
...
/// The scanner skips a `<trkpt>` missing a coordinate and reads on, so a lone bad point in a long
/// planner GPX does not abort the conversion. `obc-replay`'s `Track::parse` errors on the same GPX.
#[test]
fn scanner_skips_a_missing_coordinate() {
    ...
    // The skip also applies to a body-bearing bad point in the middle.
    let pts = scan(
...
/// the text verbatim; mapping it onto a category is `symbol.rs`'s job.
#[test]
...
/// char boundary rather than overflowing.
#[test]
