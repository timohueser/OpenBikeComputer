...
impl Semmc {
    ...
    /// What [`init_card`](Self::init_card) found, once it has run. The bring-up path already gets
    /// this back from [`start`](Self::start); kept as the after-the-fact accessor for diagnostics
    /// (a settings screen, a future bench) rather than making callers cache it.
    #[allow(dead_code)]
    ...
    ///
    /// No caller yet, deliberately: 0 is correct on every harness measured so far, and #1158's
    /// on-glass checklist still owns the `read_delay`/CMD21 tuning story for a marginal one. The
    /// knob exists so that answer is a one-line change at the bring-up site rather than a driver
    /// edit under a scope probe.
    #[allow(dead_code)]
    ...
    ///
    /// ## ⚠️ Synchronous — and it was `async` until the integration PR (#1158) measured the cost
    ///
    /// PR #1160 made this `async` and stated the contract that came with it: *the storage mode must
    /// be held across the entire `start().await`*, because it yielded three times (the card settle,
    /// the CMD8 deliver-and-abort's 3 ms wait, each ACMD41 poll) with the sEMMC image live, all six
    /// pads on `CTRLSEL = VPR`, and — at the CMD8 yield — a command actually on the wire that the
    /// function is about to abort by hand. A scheduler that treated one of those yields as a
    /// handover would repoint `INITPC`, reset the hart and flip B0/B1 out from under a transaction,
    /// leaving the card mid-command with no host to finish it: the one failure this driver cannot
    /// recover from.
    ///
    /// **The contract is now enforced by there being no yield at all.** Two things forced the
    /// change, and both are measurements:
    ///
    /// 1. **The premise for `async` does not hold on this board.** The rationale was that a blocking
    ///    bring-up would starve the panel's anti-DC-bias COM square wave. It cannot: `com::com_task`
    ///    runs on the SWI01 `InterruptExecutor` at priority **P3**, which *preempts* thread mode —
    ///    and on a `com-hw` build COM is a TIMER + DPPI + GPIOTE chain that uses no CPU whatsoever.
    ///    Thread-mode blocking is not a DC-bias hazard here.
    /// 2. **`async` cost 6,464 B of permanently-allocated stack.** Awaited from `main`, this chain's
    ///    coroutine flattens into `main`'s task-body poll frame, whose slot set is allocated on
    ...
    /// ride loop, the BLE stack or the USB plane exist. Nothing else is running to be starved.
    ...
    /// Synchronous for the reason given on [`start`](Self::start).
