...
//!
//! It never panics and never spins without a deadline: every wait is `Instant`-bounded and every
...
/// R1 error mask — every defined error bit. A set bit after a transfer is the card complaining.
///
/// ⚠️ **Deliberately stricter than the bench**, which only logged these: a driver that shrugs off
/// an address/CRC/ECC complaint is how corrupted map bytes reach the renderer. It is on #1158's
/// on-glass checklist to confirm no benign flag trips it during a long ride.
const R1_ERROR_MASK: u32 = 0xFDF9_E008;
...
/// Debug Module is the actual guarantee. Same lever `ls021_flpr::relaunch_flpr` uses on the
/// display blob; the sEMMC side needs it on every mode switch, not only for recovery.
pub fn park_hart() {
...
/// Deadline-bounded busy wait. Used only on the once-per-power-on identification path (the CMD8
/// settle and the ACMD41 poll interval), never inside a transfer — transfers sleep in `WFE`.
fn spin(d: Duration) {
...
impl Semmc {
    ...
    /// **The one-call bring-up**: pads → storage, cold-boot the image, power it on, identify the
    /// card. Run this once per power-on, with the FLPR already parked and the display side idle;
    /// every later handover is [`enter_storage_mode`](Self::enter_storage_mode) /
    ...
    /// Call once per power-on; every later mode switch uses the warm path.
    pub fn cold_boot(&mut self) -> Result<(), SemmcError> {
    ...
    /// measured 29 µs.
    pub fn warm_boot(&mut self) -> Result<(), SemmcError> {
    ...
    pub fn enable(&mut self) -> Result<(), SemmcError> {
        vri_write(VRI_ENABLE, 1);
    ...
    ///
    /// Interrupt-driven: the loop sleeps in `WFE` and is woken by [`on_vpr00_irq`]. The VRI event
    /// words stay the authority — they are ordinary SRAM, so re-reading them per wake is two loads
    /// — which is what makes an interrupt that never arrives (an unbound vector, an `INTENSET`
    /// dropped because the core was stopped) a *slow* path bounded by the deadline rather than a
    /// hang. That case is diagnosed once, loudly, because it is a wiring-up bug and not a card
    /// fault.
    fn wait_completion(&mut self, timeout: Duration) -> Result<(), SemmcError> {
        ...
        loop {
            ...
            // Sleep until *something* happens. The Cortex-M event register is sticky, so an
            // interrupt taken between the checks above and here returns from `WFE` immediately —
            // there is no lost-wake-up window.
            cortex_m::asm::wfe();
        }
    }
    ...
    fn take_completion(&mut self) -> Option<Result<(), SemmcError>> {
        if vri_read(VRI_EV_XFERCOMPLETE) != 0 {
            vri_write(VRI_EV_XFERCOMPLETE, 0);
            vri_write(VRI_EV_READYTOTRANSFER, 0);
            return Some(self.close_transaction());
        }
        if vri_read(VRI_EV_ABORTED) != 0 {
            let status = vri_read(VRI_STATUS);
            vri_write(VRI_EV_ABORTED, 0);
            vri_write(VRI_EV_XFERCOMPLETE, 0);
            // Nordic's abort path closes the transaction the same way a completion does; without
            // the barrier the firmware never re-arms.
    ...
    fn cmd8_deliver_abort(&mut self) -> Result<(), SemmcError> {
        ...
        vri_write(VRI_EV_ABORTED, 0);
        vri_write(VRI_EV_XFERCOMPLETE, 0);
        if !(stopped && acked && self.close_transaction().is_ok()) {
    ...
    fn cmd6_high_speed(&mut self) -> Result<bool, SemmcError> {
        ...
        if !in_tran {
            // Belt and braces: a stray CMD12 unsticks a card still streaming.
    ...
    pub fn init_card(&mut self) -> Result<CardInfo, SemmcError> {
        self.card = None;
        self.rca = 0;
        self.bus_width = 1;
        ...
        let ocr = loop {
            ...
            spin(Duration::from_millis(10)); // the spec's poll interval, not a tight hammer
        };
    ...
    fn read_one(&mut self, lba: u32, block: &mut AlignedBlock) -> Result<(), SemmcError> {
        ...
        self.cmd(17, self.block_arg(lba), RESP_R1, PROC_IGNORE, Some((addr, BLOCK_BYTES as u32, 1)), READ_DEADLINE)?;
    ...
    ///
    /// Blocking (a `WFE` sleep on the completion interrupt) — the same profile the SPI transport
    /// had, and what `embedded_sdmmc`'s synchronous `BlockDevice` needs.
    pub fn read_blocks(&mut self, lba: u32, buf: &mut [u8]) -> Result<(), SemmcError> {
        let n = self.check_buffer(buf.as_ptr() as usize, buf.len())?;
    ...
    pub fn write_blocks(&mut self, lba: u32, buf: &[u8]) -> Result<(), SemmcError> {
        ...
        if n == 1 {
            self.cmd(24, self.block_arg(lba), RESP_R1, PROC_PROCESS, data, WRITE_DEADLINE)?;
        } else {
    ...
    /// Validate a caller buffer against the firmware's DMA rules and return the block count.
    fn check_buffer(&self, addr: usize, len: usize) -> Result<u32, SemmcError> {
        if self.card.is_none() {
            return Err(SemmcError::NotInitialised);
        }
        if len == 0 || !len.is_multiple_of(BLOCK_BYTES) || !addr.is_multiple_of(4) {
            return Err(SemmcError::BadBuffer);
        }
        Ok((len / BLOCK_BYTES) as u32)
    }
}
...
fn csd_v2_blocks(csd: &[u32; 4]) -> Result<u32, SemmcError> {
    ...
    // 1024 blocks per 512 KiB unit. `c_size` is 22 bits, so this cannot overflow a u32 below the
    // 2 TiB SDXC ceiling; saturating keeps a garbage CSD from wrapping into a tiny capacity.
