 ...
 * stays **below the ping-pong buffer base** (control_base + 0x100, see WRITE_BUF_ADDR in ls021_flpr.rs),
 ...
 * *fast-forwarded*. The gate token walks a shift register top-down — `GSP` loads it, every `GCK`
 * period (a `dummy_advance` OR a `write_gate_row`'s MSB-phase rising edge) advances it one row — so
 * to reach visible row N you must clock past 0..N-1, but you can:
 ...
 * ascending + disjoint). `gate` is the next visible row the scan will land on (0 after the lead
 * dummies); `dirty` is the ping-pong index, toggled per **dirty** row consumed (the M33 publishes
 * dirty rows ascending into the alternating buffers, so this index — not `row & 1` — selects which
 * buffer carries this row). The envelope:
 ...
 * A full frame is just `n_spans=1, spans[0]=(0<<16)|320` → the while-loop never runs and all 320
 * rows drain in order, byte-identical to the F5 scan. Returns the number of **dirty** rows scanned
 * (= sum of span counts), which the M33 cross-checks. */
static uint32_t run_frame(void)
{
    ...
    for (uint32_t s = 0; s < n_spans; s++) {
        ...
        for (uint32_t k = 0; k < count; k++) {
            drain_row(dirty & 1u); /* write this dirty row; its MSB-phase GCK edge advances onto `gate` */
            dirty++;
            gate++;
        }
    }
