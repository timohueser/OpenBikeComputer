            ...
            crate::security_manager::SecurityEventData::SendLongTermKey(handle, ediv, rand) => {
                ...
                if let Some(identity) = identity {
                    ...
                    if let Some(ltk) = ltk {
                        ...
                        // OBC LOCAL PATCH (spec 5.4 Vol 4 Part E §7.8.26): no LTK for this peer —
                        // answer with the *negative* reply so the controller rejects the
                        // encryption start with "PIN or Key Missing". The stock 0.7.0 code
                        // disconnected with AuthenticationFailure here, which tells a stale-bonded
                        // central (iOS) nothing — it keeps its bond and reconnect-loops forever.
                        // The negative reply is the standard signal that makes the central
                        // re-pair (replacing its stale bond). Upstream: embassy-rs/trouble.
                        warn!("[host] no long term key for peer — sending LTK negative reply (peer bond is stale)");
