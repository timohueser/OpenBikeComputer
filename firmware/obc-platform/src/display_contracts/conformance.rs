...
/// frame itself is never (observably) written.
///
/// `overlay_rect` is the frame-space overlay window; `mark_at` a pixel inside it the closure
/// paints; `backdrop_at` a pixel inside it the closure leaves alone.
pub async fn check_overlay_backdrop<F, P>(
    frame: &mut F,
    p: &mut P,
    base: F::Color,
    mark: F::Color,
...
/// without ever flashing the overlay off, and the overlay's next own re-present composites over
/// the **current** clean frame (fresh backdrop, no map re-render).
///
/// `outside_at` must lie outside the presenter's *widened* region for `overlay_rect` (the concrete
/// test knows the widening rule).
#[allow(clippy::too_many_arguments)] // a test harness taking explicit probe points, not an API to hold small
pub async fn check_overlay_exclusion<F, P>(
    frame: &mut F,
    p: &mut P,
    base: F::Color,
    mark: F::Color,
...
/// shrinking bulge needs no undo pass — withdrawn pixels revert to backdrop), and clearing the
/// overlay restores the clean frame with **no map re-render**. With `expect_refinement`, the
/// presenter's damage state must already agree with the clean frame afterwards: the next
/// unknown-damage present pushes nothing.
#[allow(clippy::too_many_arguments)] // a test harness taking explicit probe points, not an API to hold small
pub async fn check_overlay_pop_retract_clear<F, P>(
    frame: &mut F,
    p: &mut P,
    base: F::Color,
    mark: F::Color,
