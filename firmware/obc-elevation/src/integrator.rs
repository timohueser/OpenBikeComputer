//! [`ProfileIntegrator`]: the dead-banded ascent and descent accumulator over a
//! `(distance, elevation)` stream, the one place a polyline turns into the two numbers everything
//! downstream displays.
//!
//! [`DeadBand`] alone answers how much a signal climbed. A profile also needs over what length,
//! and every consumer that wants one wants the other: the packer stores per-edge ascent next to
//! that edge's length, route emit re-derives the same totals the imported-GPX path derives, and
//! the profile and ride ledger carry both.
//!
//! Sampling ascent is not the same as differencing endpoints: a pass road between two 500 m
//! junctions has enormous climb and zero net. That is why this is a stream fold and why the
//! dead-band lives inside it, applied to the samples and not to the result.
//!
//! The distance channel is also the ordering guard: distances must not go backwards, and a repeat
//! at the same distance contributes nothing.

use crate::deadband::{DeadBand, Elev};

/// Running ascent, descent and length over a `(distance_m, elevation_m)` stream.
///
/// Feed samples in travel order and read the totals at any point. It holds four numbers, so a
/// caller integrating thousands of edges keeps one on the stack per edge.
...
impl<T: Elev> ProfileIntegrator<T> {
    ...
    /// A fresh integrator over a caller-configured [`DeadBand`], for a consumer that has measured
    /// its own noise floor.
    ...
    ///
    /// A sample at or before the previous distance is not rejected: its elevation still
    /// integrates, because a doubled vertex is common in real geometry and its height is real, but
    /// it cannot shorten the line. Non-monotone input adds no length rather than a negative one.
    ...
    /// Length of the integrated line in metres: the distance spanned by the accepted samples.
    #[inline]
    pub fn length_m(&self) -> f32 {
        self.length_m
    }

    /// The dead-band underneath, for a caller that also wants the smoothed height or has to pause
    /// it across a gap.
    #[inline]
...
impl ProfileIntegrator<f32> {
    /// Ascent as the `uint16` metres an OBCM neighbour entry carries, saturating. Saturation is
    /// the safe direction for a router: an absurd edge becomes maximally expensive instead of
    /// free, and 65,535 m of climb on one edge only happens on corrupt input.
    #[inline]
...
mod tests {
    ...
    /// The headline property: ascent is an integral, not an endpoint difference. A pass with equal
    /// start and end heights has zero net change and hundreds of metres of climb.
    #[test]
    ...
    /// The dead-band is inside the integrator, so a jittery DEM resample books nothing.
    #[test]
