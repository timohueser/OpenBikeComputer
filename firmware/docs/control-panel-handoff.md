...
a faithful stand-in for the real nRF54L + LS021B7DD02 hardware. The same app
...
| Step | What | Status |
|---|---|---|
| 1 | `obc-app` crate: `Fix`, `LocationSource`/`InputSource` traits, `AppState` follow/free camera | ✅ done |
...
**Step 4 as built** (`obc-sim/src/gui.rs`): a second "Controls" immediate viewport
...
`mpp_to_zoom`, `format_distance`) are unit-tested (obc-sim now 7 tests).
...
`obc-app/tests/app.rs` (now 10). Next natural step: the user-position marker (§9.1).

---

## 2. Architecture & crate map

Dependency direction: **`obc-sim → obc-app → obcm`**. The firmware will be a
second host beside `obc-sim`, reusing `obc-app` + `obcm` unchanged.
...
`AppState.mode` — **no changes to `obc-app` or `obcm` should be needed.**
...
**Run cargo from `firmware/`.** Sample maps (`monaco.obcm`, `luxemburg.obcm`,
`freiburg.obcm`, `malta.obcm`, `kz.obcm`) live at the **repo root**, so use
`../monaco.obcm` from `firmware/`. No SDL env vars needed anymore.

```sh
cd firmware
cargo build --release -p obc-sim
cargo clippy --workspace --all-targets      # must stay clean
cargo test --workspace                       # obcm 12, obc-app 10, obc-sim 7

# Render a rotated (heading-up) frame headlessly to verify rotation:
./target/release/obc-sim ../monaco.obcm --true-color --heading 45 --png /tmp/r45.png
...
./target/release/obc-sim ../monaco.obcm --png /tmp/a.png
# Live composited frame via egui's own capture (proves texture upload + display):
./target/release/obc-sim ../monaco.obcm --screenshot /tmp/b.png
```
...
- **Bash cwd drifts** between tool calls (a heredoc that `cd`s into repo root
  leaves you there). Always `cd firmware` (or use absolute paths) before cargo.
...
- **`--screenshot` can't capture the Controls viewport.** Confirmed: eframe/egui
  ...
  live look** — `cargo run --release -p obc-sim -- ../monaco.obcm` and inspect the
...
- **Shared logic lives in `obc-app`** (a real `no_std` crate, not inlined in the sim).
...
- `cargo clippy --workspace --all-targets` clean; `cargo test --workspace` green;
  `obc-app` still builds for `thumbv8m.main-none-eabihf`; `monaco.obcm --png`
...
1. **User-position marker** — draw a triangle at `AppState.user_fix` oriented along
   ...
   drawing goes in the shared renderer, *not* the host: `obcm` draws, `obc-app`
...
3. **Virtual buttons** — implement `InputSource` (host side: keyboard + on-screen
   ...
   UI-state handling in `obc-app`. Revisit the provisional `Button` enum against
   the real hardware.
...
(`heapless`) for the MCU, then the nRF54L front-end (embassy + LS021B7DD02 driver)
