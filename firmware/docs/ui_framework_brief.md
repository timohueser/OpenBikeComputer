...
validated **entirely on the desktop host**: `obc-app` already runs the same
...
`obc-app`, fed by `App::tick`, read by the screens.
...
4. **Tests** mirroring `obc-render/tests/priority.rs`: feed `Gesture`s, assert
