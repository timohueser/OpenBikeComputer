...
- **Converter:** [`fonts/convert_bdf.py`](../obc-render/fonts/convert_bdf.py) (BDF →
  ...
  OFL license under `obc-render/fonts/terminus/`; the seam is
  [`obc-render/src/font_data.rs`](../obc-render/src/font_data.rs) (no call sites changed).
- **Layout refit (all screens):** `title_frame` slimmed to a 34 px bar with a left-aligned,
  ...
  100 km.
- Tests (`obc-render/tests/text.rs`) + `--text-demo` updated; full workspace builds clean.
...
Current mapping in [`obc-render/src/text.rs`](../obc-render/src/text.rs):
...
- Convert each BDF → an `embedded-graphics` `MonoFont` (a 1bpp `ImageRaw` glyph strip + a
  ...
  `obc-render/src/font_data.rs` (static `&[u8]` strips + `MonoFont` consts). Bitmaps are a
...
### 4. Verify + document
- **Update `--text-demo`** (`obc-sim/src/main.rs` text-demo path) to show the new fonts and
...
- Update `obc-render/tests/text.rs` (metrics + the device-64 quantization check still
...
## Pointers
- Font seam: [`obc-render/src/text.rs`](../obc-render/src/text.rs) (swap here),
  `Canvas::text` in [`canvas.rs`](../obc-render/src/canvas.rs).
- Tightest layout: [`obc-app/src/screen/statistics.rs`](../obc-app/src/screen/statistics.rs) `tile()`.
- Verify at true size: the 1:1 calibration mode (`--calibrate` then `--physical`),
  [`obc-sim/src/calib.rs`](../obc-sim/src/calib.rs).
- Snapshot workflow: `cargo run -p obc-sim -- ../freiburg.obcm --script TOKENS --png OUT --scale N`
