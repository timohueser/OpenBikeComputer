...
`obc-render/src/lib.rs` (the renderer), `obc-reader/src/reader.rs` (decode +
quadtree), `obc-app/src/screen/map.rs` + `app.rs` (orchestration),
`obc-sim/src/{framebuffer,gui}.rs` (host), `OBCM_Spec.md` (format), and the
...
The renderer is one `no_std`, zero-allocation crate (`obc-render`) that runs
**byte-for-byte identically** on the desktop simulator and on the nRF54L
...
* **`DrawTarget`** (`embedded-graphics` 0.8) is the pixel sink. The sim's is
  `Framebuffer` (`obc-sim/src/framebuffer.rs`), a plain row-major RGB888 buffer
...
* **`color_fn: Fn(u16) -> D::Color`** is the colour seam. Styles store
  ...
  (`obc-reader/src/color.rs`). The renderer never sees panel specifics.
...
| :-- | :-- |
| `obc-reader` | Parse header/styles/LOD table; quadtree walk; streaming feature decode. |
| `obc-route`  | Parse `.obcr` routes; per-chunk decode; ground-distance math. |
| `obc-render` | Projection + the whole draw pipeline (this doc). `canvas`/`text` are UI chrome, not map. |
| `obc-app`    | Orchestration: screen stack, camera state, what to draw on top of the map. |
| `obc-sim`    | Host shell: window/event loop, `Framebuffer`, colour policy, frame timing. |
...
* **Route** (`.obcr`, `obc-route`): chunked polyline with per-chunk bbox +
...
`main.rs:584`) — `obc-render` is clockless `no_std`, so the **host** times it (the
...
* **`obc-render/tests/priority.rs`** — the payoff test: a synthetic two-chunk map
...
* **Reader** (`obc-reader/tests/format.rs`) — `filtered_decode_skips_without_drifting`,
...
* **Chunk-seam cracks → kept ≤1px outward overdraw.** Independent per-chunk
  clipping (`packer/obcm/quadtree.py`) emits different boundary vertices on each side of a
...
in `back_to_you` too. A small shared "directional glyph" helper in `obc-render`
