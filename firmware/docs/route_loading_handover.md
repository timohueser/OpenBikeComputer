...
1. **`obc-route` crate** (`no_std`, the single source of truth, shared by sim + future
   firmware — there is **no** packer/web_builder/Python converter; GPX is converted where it lands):
...
5. **Sim "SD card"** (`obc-sim/src/routes.rs`): `RouteStore` = a folder of `.obcr`
...
- **Route load enters the riding view** — `AppState::enter_riding_view` (Follow,
  ...
  (done 2026-06-18). `m/px → zoom` via `obc_render::zoom_for_mpp`.
...
cargo run -p obc-sim -- ../freiburg.obcm --import ../test.gpx --routes-dir /tmp/routes
cargo run -p obc-sim -- ../freiburg.obcm --boot --routes-dir /tmp/routes --script pp --png /tmp/route.png --scale 2
# interactive (drag-drop a .gpx onto the window):
cargo run -p obc-sim -- ../freiburg.obcm --boot --routes-dir /tmp/routes
cargo test --workspace
```

File map: format/converter `obc-route/src/{byte_io,reader,gpx,convert}.rs`; route draw
`obc-render/src/lib.rs` (`draw_route`); app catalog/menu/map
`obc-app/src/{route.rs, app.rs, screen/{mod,route_menu,map}.rs}`; sim store
`obc-sim/src/{routes.rs, main.rs, gui.rs}`.
