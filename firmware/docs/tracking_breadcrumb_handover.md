...
swap*; the route can change within it. The shared `obc-app` core owns the session state
...
- **`obc-route/src/track.rs`** (new, no_std): `TrackPoint` + a headerless 16-byte record
  ...
  `tests/track.rs`.
- **`obc-route/src/matcher.rs`**: first-lock earliest-progress tie-break (`TIE_EPS_M = 8.0`)
...
- **`obc-app`**: `Activity` gains `session`/`session_seq`/`track_action`/`last_alt` +
  ...
  trail behind = red).
- **`obc-render`**: `MapRenderer::stroke_path(iter of (i32,i32))` — `draw_route`'s
...
- **`obc-sim/src/track.rs`** (new): `TrackRecorder` mirroring `RouteStore` —
...
- **Full loop (headless):** from the repo root —
  ...
  (`--routes-dir firmware/routes` matters — `routes/` is cwd-relative.)
- **Swap prompt:** `--boot --script ppBprp --routes-dir firmware/routes --png /tmp/swap.png`.
