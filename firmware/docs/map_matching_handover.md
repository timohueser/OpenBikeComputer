...
> **STATUS: DONE (2026-06-18).** Shipped `obc-route::matcher` (`RouteMatch`/`Match`),
...
- `obc-route`: `Profile` + `RouteReader::elevation_profile()` (`src/profile.rs`);
...
- `obc-app`: `Screen::Elevation` (`src/screen/elevation.rs`); `App` owns a resident
  `profile` (rebuilt in `render_frame` on `active_route` change) exposed via
  `Render.profile`; `Activity { mode, active_route }` (grow it here).
- `obc-app`: `AppState::enter_riding_view` (the follow + heading-up camera preset);
  `Render.route: Option<&RouteReader>` already threaded to draw.
- `obc-render`: `Canvas::vline` / `Canvas::disc`.

## Step 1 — `RouteMatch` (obc-route, new `src/match.rs`)
...
- **Host edits** (both hosts open the reader already): reorder the frame so the route is
  ...
  - `obc-sim/src/gui.rs` ~L257–288: move the `sync_active` + `RouteReader::open` block
    (currently L275–279) above the `tick` calls (L260/L272).
  - `obc-sim/src/main.rs` headless: it renders a single frame without `tick`; for
...
- **`obc-route` unit tests** (`tests/match.rs`): synthetic route + crafted fixes —
