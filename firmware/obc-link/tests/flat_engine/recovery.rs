...
/// `specs/vectors/ride-v5.bin`: three 20-byte `TrackPoint` samples, then the fixed footer.
const RIDE_V5: &[u8] = include_bytes!("../../../../specs/vectors/ride-v5.bin");
...
fn journaled_recording(seed: u64) -> (SparseDisk, (u64, u64)) {
    ...
    let key = {
        ...
        let (samples, footer) = RIDE_V5.split_at(RIDE_V5.len() - FOOTER_LEN);
        checkpoint(&device, key, samples, samples);
        checkpoint(&device, key, footer, RIDE_V5);
        key
    };
    (disk, key)
}
...
fn catalog_write_offset() -> u32 {
    ...
    finalise(&device, key, RIDE_V5.len() as u64, crc32(RIDE_V5)).expect("the probe finalises");
    let (op, _, _) = disk
        .write_log()
        .into_iter()
...
fn an_interrupted_recording_recovers_and_exports_the_pinned_gpx() {
    ...
    assert_eq!(
        finalise(&device, key, RIDE_V5.len() as u64, crc32(RIDE_V5)),
    ...
    assert_eq!(recovered.payload_len(), RIDE_V5.len() as u64, "the recovery lost journaled bytes");
    assert_eq!(recovered.payload_crc, crc32(RIDE_V5), "the recovery reconstructed a different ride");
    ...
    assert_eq!((entry.payload_len, entry.payload_crc), (RIDE_V5.len() as u64, crc32(RIDE_V5)));
    ...
    assert_eq!(payload, RIDE_V5, "GET served something other than the recovered ride");
