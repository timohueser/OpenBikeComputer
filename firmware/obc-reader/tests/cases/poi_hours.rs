//! Contract tests for the device-side POI hours lookup (`Reader::poi_hours`).
//!
//! Each test builds a synthetic `.obcm` whose POI section carries a real hours pool, then asserts
//! the reader resolves a POI's `hours_ref` back to the pooled [`WeeklySchedule`], and that every
//! out-of-range or corrupt case yields `None` rather than a panic. The schedule eval semantics are
//! unit-tested in `obc_reader::hours`; this pins the read and decode path through the file layout.
...
/// A 29-byte pool blob from `flags` and per-day `(open_q, close_q)` slot pairs, Monday first.
...
/// Assemble a full `.obcm` with one accommodation category holding two POI records and a hours
/// pool of `blobs`. Records A and B reference `ref_a` and `ref_b`.
fn build_map_with_pool(blobs: &[[u8; POI_HOURS_BLOB_LEN]], ref_a: u16, ref_b: u16) -> Vec<u8> {
    // Everything up to the POI section from build_file, then a populated directory and pool.
    let base = build_file(
        GLOBAL,
        STYLES,
    ...
    // Two hotels near the map centre, so nearest_pois returns both.
    ...
    // Every offset the directory names is a unit boundary, so the index sits at the first one past
    // the directory and its chunks one rounding step later. The 512-byte chunk then leaves the
    // cursor aligned for the pool.
    ...
    // The populated POI section displaced base's tail nav section, so re-append an empty one at
    // the next unit boundary and patch the header's nav offset.
...
/// Run a query and `poi_hours` over a built map, returning the two POIs' hours_refs and the
/// decoded schedules.
...
fn poi_hours_resolves_pooled_schedules_end_to_end() {
    ...
    // Find each by its hours_ref, since the query orders by distance.
...
/// A `ByteSource` that reports a larger `len()` than the real bytes and fails any read reaching
/// past the real end. This lets a corrupt-but-parseable map exist: the parse bounds the pool
/// against the inflated length and succeeds, but the per-blob read must still fail cleanly.
...
fn poi_hours_corrupt_count_past_eof_is_none() {
    // A directory whose declared hours_pool_count runs past the real bytes but fits the inflated
    // reported len, so parse accepts it and the read for the missing blob must fail cleanly. The
    // parse's own pool-region check cannot catch a source that lies about its length.
    let blob0 = blob(0, [[(32, 72), (0, 0)]; 7]);
    ...
    // The last blob's read reaches past the real bytes, so it is None rather than a panic.
    assert_eq!(r.poi_hours(forged_count - 1), None, "missing blob ⇒ read fails ⇒ None");
}

#[test]
fn weekly_schedule_decode_short_slice_is_none() {
    // A hand-made short slice decodes to None: the corrupt-pool guard at the decode boundary.
