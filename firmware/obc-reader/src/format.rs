...
/// Style-record `flags` byte — bit 2 set marks a **dashed** line (clear ⇒ a solid stroke).
/// Ignored for polygons (a fill is never dashed).
pub const STYLE_DASHED_BIT: u8 = 0x04;
/// Style-record `flags` byte — bit 3 set means the record carries a **secondary color**
/// (`color2`, the u16 at record offset 6). Clear ⇒ those two bytes are written `0x0000` and MUST
/// be ignored: `0x0000` is a legit RGB565 color (black), not a "no color2" sentinel.
