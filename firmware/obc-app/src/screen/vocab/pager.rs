//! The two-page auto-flip the detail compositions share. This module owns the timing; the screens
//! own what each page draws.

use crate::screen::ScreenTick;

/// How long each page shows before the auto-flip.
pub(crate) const PAGE_FLIP_MS: u32 = 5_000;

#[derive(Debug, Default)]
pub(crate) struct ContentPager {
    second: bool,
    /// `None` until the first tick anchors it, so the first page gets a full dwell on entry.
...
impl ContentPager {
    ...
    /// is `wrapping_sub`, so it stays correct across the `u32` millis wrap.
