//! The Power screen: the GPS fix interval and a power-saver toggle. The interval step adapts, 1 s
//! up to 10 s and then 5 s, so a long interval takes few steps.
...
/// Row height. It fits a two-line label and a stepper field with room for the arrows.
const ROW_H: i32 = 58;
...
/// Step the fix interval by `n` steps: 1 s below 10 s, 5 s above. The boundary is checked at each
/// step, so a sweep up reads 1 to 10, then 15, 20.
...
/// `editing` is set only while the GPS-fix stepper is open. The toggle has no edit mode.
