...
use super::vocab::list::step_selection;
...
impl SetupEffortScreen {
    pub fn handle(&mut self, g: Gesture, cx: &mut Ctx) -> Transition {
        match g {
            Gesture::Step(n) => {
                self.selected = step_selection(self.selected, n, LIMITS.len() + 1);
                Transition::None
            }
...
mod tests {
    ...
    fn the_title_bar_counts_the_titled_steps() {
        let places = [SetupStep::Language, SetupStep::Units, SetupStep::Effort].map(place);
        assert_eq!(places, [(1, 5), (3, 5), (5, 5)]);
    }
}
