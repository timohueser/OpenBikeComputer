...
pub static AARE: Photo = Photo {
    pixels: include_bytes!("../../assets/landmarks/aare-large.rgb222"),
...
pub static FALLS: Photo = Photo {
    pixels: include_bytes!("../../assets/landmarks/falls-large.rgb222"),
...
pub static DUNLOUGH: Photo = Photo {
    pixels: include_bytes!("../../assets/landmarks/dunlough-large.rgb222"),
...
pub fn draw<D, F>(cv: &mut Canvas<D, F>, photo: &Photo, at: Point)
where
    D: DrawTarget,
    F: Fn(u16) -> D::Color,
{
    ...
    let _ = target.fill_contiguous(&rect(at.x, at.y, 216, 240), photo.pixels.iter().map(|&i| palette[i as usize]));
}
