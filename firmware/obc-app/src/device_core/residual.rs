...
//! effects. What it still drains from the old [`HostMailbox`](crate::HostMailbox) is exactly two
...
/// The two residual classes in the **drain's** own vocabulary — what
...
/// [`RESIDUAL`] is the same two classes as prose and [`residual`] as a predicate;
/// `the_residual_table_names_exactly_what_the_predicate_admits` pins both together.
pub(crate) const RESIDUAL_CLASSES: [HostCommandClass; RESIDUAL_CLASS_COUNT] =
    [HostCommandClass::FinishTrack, HostCommandClass::ForgetBond];
...
pub const RESIDUAL_CLASS_COUNT: usize = 2;
...
pub const RESIDUAL: [&str; RESIDUAL_CLASS_COUNT] = ["FinishTrack", "ForgetBond"];

/// Whether `command` is one of the two a typed executor deliberately leaves on the old protocol.
///
...
pub fn residual(command: &HostCommand) -> bool {
    matches!(command, HostCommand::FinishTrack(_) | HostCommand::ForgetBond)
}
...
mod tests {
    ...
    /// actually decides — so they have to name the same two classes. A class added to one and not
    ...
    fn the_residual_table_names_exactly_what_the_predicate_admits() {
        ...
        assert_eq!(RESIDUAL.len(), 2, "two classes, and the table says which");
        assert_eq!(
