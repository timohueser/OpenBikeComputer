...
//! set-point: the time the user or a GPS fix last established. The live time is that set-point
//! advanced by however long has elapsed since it was stamped. Every clock-bearing screen reads
//! through here, so they all agree and tick together.
//!
...
/// halves. Screens get the already-computed `now` and never see the epoch.
...
pub struct WallClock {
    ...
    /// once [`set`](WallClock::set) has run. The Home date line hides while this is false, because a
    /// date with no trusted origin would mislead.
    established: bool,
}
...
impl WallClock {
    ...
    /// [`Settings::clock`](crate::Settings::clock) at boot, so without an RTC the clock resumes from
    /// the last-set value until a GPS fix or the user re-stamps it.
    pub fn new(base: DateTime) -> Self {
        WallClock { base, epoch_ms: 0, established: false }
    }

    /// Declare that `base` is the time now (`now_ms`), so the clock resumes ticking from the
    /// freshly established value, and mark it [`established`](WallClock::is_established).
    ...
    /// Whether a set-point has ever been established — the Home date line's gate.
    ...
    /// Unix seconds at `now_ms`, reading the set-point as UTC. Unlike [`now`](WallClock::now) this
    /// keeps the sub-minute remainder: the GPS re-stamp back-dates `epoch_ms` by the fix's
    /// seconds-into-the-minute, so second-level truth survives the minute-resolution set-point. The
    /// set-point is local time, and [`App::wall_unix_now`](crate::App::wall_unix_now) folds the UTC
    /// offset back out.
    pub fn unix_now(&self, now_ms: u32) -> u32 {
        self.base.to_unix().wrapping_add(now_ms.wrapping_sub(self.epoch_ms) / 1000)
    }

    /// Milliseconds from `now_ms` until the displayed `HH:MM` next rolls over — the deadline the
    /// host arms one wake timer to, so the M33 can sleep until then rather than free-run to discover
    /// the change. Always in `1..=60_000`: at an exact boundary the full minute remains.
...
/// [`changed`](MinuteTicker::changed) from its `tick_timers`, dirtying itself once each time the
/// displayed minute rolls over, so a static screen repaints without polling on a blind heartbeat.
///
/// The first observation only initialises the baseline and reports no change: a screen's first
/// paint is already driven by whatever made it appear.
...
impl MinuteTicker {
    ...
    /// observation. Always `false` on the first call.
