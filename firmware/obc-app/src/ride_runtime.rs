...
impl RideRuntime {
    /// Idle boot state: no route/session needs reconciling and no route index is usable.
    ...
    /// Record that the board's placement-sensitive slot now contains `route`'s index.
    #[inline(always)]
    pub fn route_index_built(&mut self, route: usize) {
        self.indexed_route = Some(route);
    }
    ...
    /// Whether the board may pair its resident index slot with `active_route`'s source.
    #[inline(always)]
    pub fn route_index_ready(&self, active_route: Option<usize>) -> bool {
        self.indexed_route.is_some() && self.indexed_route == active_route
    }
}
...
mod tests {
    ...
    fn index_result_is_one_key_without_a_duplicate_validity_flag() {
        ...
        assert!(!runtime.route_index_ready(Some(2)));

        runtime.route_index_unavailable();
        assert!(runtime.route_index_needs_rebuild(Some(2)));

        runtime.route_index_built(2);
        assert!(runtime.route_index_ready(Some(2)));
        assert!(!runtime.route_index_ready(Some(3)));
        assert!(!runtime.route_index_needs_rebuild(Some(2)));
        ...
        assert!(!runtime.route_index_ready(Some(4)));
    }
}
