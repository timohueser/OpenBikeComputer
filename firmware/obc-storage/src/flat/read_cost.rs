...
//!
//! The failure this guards against was scattered reads going through a FAT seek: FAT is a
//! singly-linked list, every backward seek restarts at the file's first cluster, and the one-block
//! FAT cache thrashed against the data reads. The flat store keeps the arithmetic one layer down:
//! `Ranges::locate` over at most eight extent ranges, then one card command per contiguous run.
//!
//! Both patterns take their offsets from the fact that a chunk section starts at a header-length
//! boundary, not a block one, so the reads straddle 512-byte blocks exactly as they do on a real map.
//! An aligned pattern would flatter every path equally and prove less.
...
/// The object both paths serve: 1 MiB, which is one default-geometry extent. Contiguous on purpose —
/// a fragmented comparison would measure the allocator rather than the read path.
const OBJECT_BLOCKS: usize = 2 * 1024;
const OBJECT_LEN: usize = OBJECT_BLOCKS * 512;

/// `obc-reader`'s nav chunk and geometry chunk ceiling. Named here rather than imported, because
/// `obc-storage` does not depend on `obc-reader`.
...
/// rather than a random sequence, because a pinned count needs a pinned pattern.
...
/// step, because panning is not monotonic.
fn render_pass() -> Pass {
...
/// per-read fixed cost that scales with offset. A pinned total alone would not catch a path that read
/// a constant 40 extra blocks. So this reads one chunk at the front of the object and one at the back
/// and requires them to cost the same, at offsets congruent mod 512 so that alignment is held fixed.
#[test]
