//! The superblock (`FLAT_Store_Format.md`): written once by initialization and never again.
//!
...
/// The recorded extent size, as a base-2 logarithm of bytes. It sits right after the block count
...
///
/// The fields are public and [`encode`](Self::encode) checks nothing, so a test can write the card
/// initialization never writes. Production has one constructor, [`for_card`](Self::for_card), which
/// cannot produce a record [`decode`](Self::decode) refuses.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Superblock {
    pub store: StoreId,
    pub total_blocks: u64,
    /// The extent size chosen for this card, which every address on it is derived from.
    pub geometry: Geometry,
}

impl Superblock {
    /// The superblock written for a card of `total_blocks`, geometry included. `None` for a card this
    /// format cannot express — see [`Geometry::for_card`].
    ...
    /// The extent count for this card at its own extent size. Within [`MAX_EXTENTS`] by construction:
    /// a superblock that would exceed it does not decode.
    ...
    /// Decodes block 0 of a copy. Both geometry rules are checked here, before any address is derived
    /// from the record: a superblock that fails either is not a superblock, so mount falls back to the
    /// other copy and then classifies the card as unformatted rather than mounting it read-only.
...
mod tests {
    ...
    /// Block 0 is deliberately not an MBR. Its bytes `510..511` are zero, so a host sees an
    ...
    /// The scaling rule, recorded and read back: the size scales with the card and the count stays
    /// inside the index either way.
    #[test]
    ...
    /// The two geometry refusals. A card whose geometry does not decode has no addresses at all, so
    /// neither mounts read-only.
    #[test]
    fn an_inadmissible_geometry_is_not_a_superblock() {
        ...
        // index cannot name. Refused rather than capped: the tail would be space the superblock claims
