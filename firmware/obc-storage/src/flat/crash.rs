...
//!
//! Each scenario runs on a fresh deterministic card per cut point, then reboots and mounts. The
//! result must be the model before the scenario or the model after it — nothing in between, and never
//! a read-only mount. The comparison is the whole state, byte image included.
//!
//! A cut point is each of [`EVERY_WHEN`] per counted media operation, plus three [`When::Inside`]
//! shapes per block of every write that moved more than one. Each scenario pins its own total, so a
//! change that makes cut points disappear fails rather than quietly passing.
...
fn ride_checkpoint<'a>(append: &'a [u8], resume: &'a [u8; RIDE_RESUME_LEN]) -> RideCheckpoint<'a> {
    RideCheckpoint { id: ObjectId(1), revision: Revision(1), append, payload_crc: crc32(append), resume }
}

/// A card with 64 extents: enough for a 32 MiB ride reserve, small enough to cost nothing.
...
/// Installs a card of `blocks` holding `model` in `copy`, without counting a media operation.
fn card_of(seed: u64, blocks: u64, model: &Model, copy: usize) -> SparseDisk {
    ...
    // A model whose geometry is not this card's would install every payload at an address the store
    // never reads. Asserted rather than derived, because `extents` is the scenario's own number too.
...
/// The card builder every matrix takes: it decides whether the commit under test targets B or A.
...
/// The catalog, its gate, and every entry's payload bytes. Installing the payloads is what makes the
/// card consistent: the entries claim a length and a CRC, and `Snapshot` reads those bytes back.
...
/// operations, plus [`When::Inside`]'s three shapes per block of every wide write. Totals fall when
/// writes are batched, but no write's coverage gets smaller: batching removes read operations, and a
/// cut inside a read changes no byte on the card.
...
/// The card seed one cut point runs on. An interior cut takes a stream of its own, because two cut
/// points sharing a seed would share a torn page's contents too.
...
/// The crash matrix for one scenario: the old state or the new one, and nothing else. `cuts` is
/// [`cut_points`]'s census, pinned.
fn matrix(
    name: &str,
    cuts: usize,
    before: &Model,
...
/// The same, for the one scenario whose cut points admit a third state.
fn matrix_admitting(
    name: &str,
    cuts: usize,
    admissible: &[Snapshot],
    ...
    // state is checked before the remount, so a commit that lied about its bytes in RAM is caught too.
    let disk = build(99);
    let mut store = FlatStore::mount(&disk);
    scenario(&mut store);
    assert_eq!(
...
/// The same scenario on a card whose catalog starts in copy A and again on one starting in copy B.
...
/// Mount fell back to the older copy because the newer one's gate is well-formed but its body fails,
/// so the commit must target the copy it is not serving, at a sequence past the ill-formed copy's.
#[test]
fn a_commit_after_a_fallback_targets_the_copy_it_is_not_serving() {
    ...
    // And the whole path, cut everywhere. Three states are admissible here rather than two: the
    // high-water mark lives only in the gates and step 1 erases the one that carried it, so a cut once
    // step 1 is durable leaves the same catalog with the mark back at 7.
...
/// A catalog whose body spans program pages, cut everywhere inside it. An old gate still names an old
/// entry count and body CRC, which a partially written body can accidentally satisfy in the prefix the
/// count selects. This body is 151 blocks over five program pages.
#[test]
fn a_multi_page_catalog_body_recovers_the_old_or_the_new_state() {
    ...
    // One object with bytes to read back, and 599 rollback reserves behind it. What matters is the
    // length of the body, and a reserve is the cheapest entry that occupies its 128 bytes.
...
/// The cut matrices above start from a single valid copy, which is the initial state, not the steady
/// one where both copies are valid.
#[test]
...
/// The superblocks are destroyed first and written last, so a card that mounts at all mounts as
/// exactly one of two whole stores. Anything in between classifies as not a flat store.
#[test]
fn initialization_produces_no_store_or_a_complete_empty_one() {
    ...
    // question with an answer. Re-initialization is the ordinary case, not migration.
    let old = {
...
/// A real "128 GB" card: 250,000,000 blocks, 119.2 GiB, which a fixed 1 MiB extent could not address
/// at all — 122,068 extents against an index that names 65,536.
///
/// A decimal size on purpose: `128e9 / 65,536` is 1,953,125 bytes, so this card gets 2 MiB only
/// because the rule rounds up. A card of exactly 128 GiB divides to 2 MiB on the nose and would let a
/// truncating rule through.
const BIG_CARD_BLOCKS: u64 = 250_000_000;
/// The extents recomputed at the 2 MiB this card gets.
const BIG_CARD_EXTENTS: u32 = 61_034;

/// Initialization on a card past the index's reach, then the whole seam on the geometry it chose.
/// The payload spans two extents at the recorded size, so every LBA `locate` computes is 2,048 blocks
/// further along than on a 1 MiB card.
#[test]
fn a_card_the_index_cannot_reach_at_one_mib_gets_bigger_extents() {
    ...
    // have picked, its own extent count is past the index and the superblock is refused.
    assert_eq!(
    ...
    // The second extent's first payload byte sits 2,048 blocks past the first's: the same extent
    // index means twice as many blocks on this card.
...
/// One doubling further out, and the ride path with it: a 512 GiB card gets 8 MiB extents, and a
/// checkpoint's 16 KiB payload page still lands page-aligned inside the first one.
#[test]
fn a_ride_records_and_recovers_on_a_card_scaled_geometry() {
    let disk = SparseDisk::blank((512 << 30) / BLOCK as u64, 11);
    let store = FlatStore::initialize(&disk, STORE).expect("a 512 GiB card formats");
    ...
    // Two checkpoints, the second past a whole payload page, so the rollover flushes one into the
    // ride's own extents at an address derived from the 8 MiB stride.
    let mut rider = Rider::new();
    rider.grow(200);
    assert!(rider.checkpoint(&store));
    rider.grow(PROGRAM_PAGE - 100);
    assert!(rider.checkpoint(&store));
    assert_eq!(rider.durable / PROGRAM_PAGE * PROGRAM_PAGE, PROGRAM_PAGE, "the checkpoint flushed no page");
    ...
    assert_eq!(store.read_recovered(expected.flushed, &mut recovered).unwrap(), expected_tail.len());
    assert_eq!(recovered, expected_tail);
    // And the flushed page is where it belongs: payload offset 0 of extent 0.
    assert_eq!(disk.block(EXTENT_AREA)[..8], rider.payload[..8], "the flushed page missed the extent area");
}

/// A model of a card-scaled card, which [`card_of`] then holds the card to.
...
/// A commit, cut everywhere, on a card whose extents are 2 MiB. Initialization writes the same
/// blocks at every extent size, so a matrix there would pin nothing the geometry can break; a commit
/// does, because the staged block goes to an address `locate` derived at the recorded stride and the
/// mount after every cut rebuilds the free map at this card's count.
#[test]
fn a_commit_on_a_card_scaled_geometry_recovers_the_old_or_the_new_catalog() {
    // The pre-state payload sits at extent 3, not extent 0: extent 0 is the one address every stride
    // agrees on, so a scenario whose only installed bytes live there would pass at a geometry the
    // store had dropped. The commit's own object then takes extent 0, which first-fit hands it.
...
/// 2 TiB range, where the contiguous run is exactly `2^32` blocks. It cannot fail on a 64-bit host;
/// on any target it catches a pass that writes fewer blocks than it should, or none.
#[test]
fn a_write_through_a_two_tebibyte_run_consumes_its_input() {
    ...
    // The bytes are still in the card's volatile cache — a `write` promises nothing durable — so the
    // sync is the harness's.
    (&disk).sync().unwrap();
...
/// Both refusals are reachable only by forgery: initialization writes neither.
#[test]
...
/// A card carrying a ride that has recorded `flushed + tail` bytes: the entry, and the slot the
/// trailing bytes live in.
fn recording_card<'m>(
    model: &'m Model,
    ride: &'m Entry,
    flushed: u64,
    ...
    move |seed| {
        ...
        // The pages already flushed are in the ride's extents; the rest is in the newest slot.
...
/// The last partial page moves out of the journal slot and into the ride's extents, because a
/// checkpoint only ever flushed whole pages.
#[test]
...
/// The other end of the page boundary: a ride shorter than one program page has flushed nothing, so
/// every byte it publishes comes out of the slot.
#[test]
...
/// No payload write and no catalog write is issued when either check fails.
#[test]
...
/// write-once: compare it and proceed to the catalog instead of programming it twice.
#[test]
...
/// The rider's side of a ride: the whole payload recorded so far, and how much a successful logical
/// checkpoint has consumed. A checkpoint hands over only the bytes added since then.
struct Rider {
    payload: Vec<u8>,
    durable: usize,
}
...
impl Rider {
    fn new() -> Self {
        Rider { payload: Vec::new(), durable: 0 }
    }
    ...
    fn append(&self) -> &[u8] {
        &self.payload[self.durable..]
    }

    /// One checkpoint. Success consumes the complete delta; an error leaves it for an identical retry.
    fn checkpoint(&mut self, store: &Card) -> bool {
    ...
    fn expect(&self, sequence: u64) -> (RideRecovery, Vec<u8>) {
        let recovery = RideRecovery {
            ...
            flushed: (self.durable / PROGRAM_PAGE * PROGRAM_PAGE) as u64,
            tail_len: (self.durable % PROGRAM_PAGE) as u32,
            payload_crc: crc32(&self.payload),
            resume: RESUME,
            slot: (sequence % SLOTS as u64) as u16,
        };
...
/// never a mixture, and never a ride that rolled back further than one interval.
fn checkpoint_matrix(name: &str, cuts: usize, growths: &[usize]) {
    ...
    let run = |disk: &SparseDisk, cut: Option<FaultPlan>| {
        ...
        for growth in &growths[..last] {
            rider.grow(*growth);
            assert!(rider.checkpoint(&store), "{name}: the setup checkpoints must succeed");
        }
        let baseline = disk.ops();
    ...
    for (op, when) in points {
        ...
        if let Some(recovered) = recovered {
            ...
            store.read_recovered(recovered.flushed, &mut read).unwrap();
            assert_eq!(
                &read,
...
fn a_checkpoint_recovers_the_previous_one_or_itself() {
    checkpoint_matrix("checkpoint", 132, &[200, 200]);
}

/// The page flush and the slot, in the order the format fixes: a payload page is written only when
/// every byte in it is already in a slot on the card. A cut between the two leaves the previous slot
/// authoritative, and recovery rewrites the page.
#[test]
fn a_checkpoint_that_flushes_a_page_recovers_either_side_of_the_flush() {
    // Both logical sides are exact 20-byte sample boundaries: 100 before, 16,400 after. The
    // intermediate 16,384-byte proof is deliberately not an admissible recovery.
    checkpoint_matrix("checkpoint with flush", 636, &[100, PROGRAM_PAGE - 84]);
}

/// "No slot at all" is admissible: a ride start commits its entry before any checkpoint exists.
#[test]
fn the_first_checkpoint_recovers_nothing_or_itself() {
    checkpoint_matrix("first checkpoint", 117, &[300]);
}

/// A proof is physical write-once evidence, not a recorder state.
#[test]
...
/// The split-slot durability order, visible in the media ledger: a full 16 KiB tail page is durable
/// before the one-block header gate is issued. This pins which operations are the tail write, its
/// sync, the header write and its final sync, so a batching change cannot reverse them.
#[test]
...
/// A damaged source may leave uncommitted bytes in the target tail page, so the checkpoint must be
/// refused before the target header gate makes them recoverable.
#[test]
fn a_corrupted_source_slot_refuses_reconstruction_without_gating_the_target() {
    let disk = card(17, &holding(&[recording()], 6), 0);
    let store = FlatStore::mount(&disk);
    let first = payload(300);
    store.journal(ride_checkpoint(&first, &RESUME)).unwrap();

    let mut damaged = disk.block(slot_block(1));
    damaged[7] ^= 0x80;
    disk.install(slot_block(1), &damaged);
    let target_header_before = disk.block(slot_header_block(2));

    let mut whole = first;
    let from = whole.len();
    whole.extend_from_slice(&payload(400)[from..]);
    assert_eq!(
        ...
        Err(super::error::StoreError::Invalid),
    );
    assert_eq!(
        disk.block(slot_header_block(2)),
        target_header_before,
        "a bad source snapshot was nevertheless gated as the next logical checkpoint",
    );
}
...
fn the_recovered_payload_crc_covers_the_prefix_on_the_card() {
    ...
    let first = 1_000;
    store.journal(ride_checkpoint(&tail[..first], &RESUME)).unwrap();
    store
        .journal(RideCheckpoint {
            id: ObjectId(1),
            revision: Revision(1),
    ...
    let mut suffix = vec![0u8; recovered.tail_len as usize];
    store.read_recovered(recovered.flushed, &mut suffix).unwrap();
...
/// Restarting the count at `1` would leave this ride's stale slots carrying greater sequences, and
/// the next recovery would roll the ride back.
#[test]
fn recording_resumes_at_the_recovered_sequence_plus_one() {
    ...
    let mut whole = rider.payload;
    let from = whole.len();
    whole.extend((from..from + 500).map(|index| (index * 7 + 11) as u8));
    store
        .journal(RideCheckpoint {
            id: ObjectId(1),
            revision: Revision(1),
...
/// Production preflight never writes a slot at `u64::MAX`. Recovery must not wrap it to zero or let
/// it outrank the newest continuable checkpoint.
#[test]
fn recovery_skips_a_logical_slot_whose_sequence_cannot_advance() {
    let ride = recording();
    let disk = card(14, &holding(&[ride], 6), 0);
    install_slot(&disk, STORE, &ride, u64::MAX - 1, 0, &payload(200));
    install_slot(&disk, STORE, &ride, u64::MAX, 0, &payload(400));

    let store = FlatStore::mount(&disk);
    let recovered = store.recovered_ride().unwrap();
    assert_eq!(recovered.checkpoint_sequence, u64::MAX - 1);
    assert_eq!(recovered.payload_len(), 200);
    let mut bytes = [0u8; 200];
    assert_eq!(store.read_recovered(0, &mut bytes).unwrap(), bytes.len());
    assert_eq!(bytes, payload(200).as_slice());
}

/// Covers the ordinary one-slot path and the two-slot rollover path, whose logical gate would
/// otherwise land at MAX and discover only afterwards that it had no resident `next_sequence`.
#[test]
fn journal_sequence_exhaustion_is_preflighted_before_media_writes() {
    let ride = recording();

    let ordinary_disk = card(16, &holding(&[ride], 6), 0);
    install_slot(&ordinary_disk, STORE, &ride, u64::MAX - 1, 0, &payload(200));
    let ordinary = FlatStore::mount(&ordinary_disk);
    let whole = payload(220);
    let before = ordinary_disk.write_log().len();
    assert_eq!(
        ordinary.journal(RideCheckpoint {
            id: ObjectId(1),
            revision: Revision(1),
            append: &whole[200..],
            payload_crc: crc32(&whole),
        ...
        Err(super::error::StoreError::ReadOnly),
    );
    assert_eq!(ordinary_disk.write_log().len(), before, "ordinary sequence exhaustion wrote media before refusal");

    let rollover_disk = card(18, &holding(&[ride], 6), 0);
    install_slot(&rollover_disk, STORE, &ride, u64::MAX - 2, 0, &payload(PROGRAM_PAGE - 4));
    let rollover = FlatStore::mount(&rollover_disk);
    let whole = payload(PROGRAM_PAGE + 16);
    let before = rollover_disk.write_log().len();
    assert_eq!(
        rollover.journal(RideCheckpoint {
            id: ObjectId(1),
            revision: Revision(1),
            append: &whole[PROGRAM_PAGE - 4..],
            payload_crc: crc32(&whole),
            resume: &RESUME,
        }),
        Err(super::error::StoreError::ReadOnly),
    );
    assert_eq!(rollover_disk.write_log().len(), before, "rollover sequence exhaustion wrote a proof or gate");
}

/// Sequence 16 gates the exact page in slot 0 and sequence 17 gates the remainder in slot 1. The
/// byte-for-byte comparison proves the payload page was written only from the durable image.
#[test]
fn a_rollover_wraps_the_ring_and_copies_the_durable_page_exactly() {
    ...
    let append = PROGRAM_PAGE - rider.payload.len() + 77;
    rider.grow(append);
    assert!(rider.checkpoint(&store));
...
fn corrupted_rollover(seed: u64) -> (SparseDisk, Vec<u8>) {
    ...
    let store = FlatStore::mount(&disk);
    store.journal(ride_checkpoint(&tail[..300], &RESUME)).unwrap();
    store
        .journal(RideCheckpoint {
            id: ObjectId(1),
            revision: Revision(1),
...
/// A later mount returns read-write only after the exact logical payload is durable.
#[test]
...
/// Mount never reads the slot headers afterwards, so a stale slot cannot resurrect a finished ride.
#[test]
...
/// A recovery offer belongs to the mount that discovered it, not to every later `RECORDING` entry. A
/// real reset before the new ride's first checkpoint still recovers it at zero length.
#[test]
...
/// The handle promise is about the revision a handle resolved, and an amend does not make a new one.
#[test]
...
/// Finalising a ride frees 31 of its 32 reserve extents, and a reader holding the entry keeps them
/// out of the allocator until it closes.
#[test]
...
/// `release` defers a trimmed tail because a hold names it, so the hold has to keep naming it. A trim
/// keeps a prefix, so the wider ranges still serve every byte of the amended length.
#[test]
...
/// A slot whose ranges differ from the recording entry's belongs to an earlier ride.
#[test]
...
/// No well-formed gate, two well-formed gates at equal sequences, and no candidate body that
/// validates are all media damage. Each mounts read-only with the evidence preserved.
#[test]
...
/// The other counter: a card whose newest gate carries commit sequence `u64::MAX` has no sequence to
/// continue to. It mounts read-only rather than wrapping — and rather than panicking, which is what an
/// unchecked `high_water + 1` would do in a debug build.
#[test]
...
/// The card starts one short of `u64::MAX`, so exactly one commit is left in it.
#[test]
...
/// The id cursor has to end up strictly greater than every id in the array, and `u64::MAX` leaves
...
/// `free_extents` is public, and a number left over from the copy a mount refused would be a promise
...
/// An allocation is RAM state until the commit that names its extents, so the next mount rebuilds the
/// bitmap from the catalog and cannot see it.
#[test]
...
/// The extents stay out of the allocator even after a commit has removed the entry.
#[test]
...
/// Fragmentation's worst case is a refused allocation, never a partial object.
#[test]
...
/// `read` is arithmetic on the entry's ranges and nothing else.
#[test]
fn a_payload_round_trips_across_a_range_boundary() {
    ...
    // This card's extents are the 1 MiB minimum, and the payload spans two of them.
...
/// The identity rule at the seam: an `ObjectId` the cursor has passed named an object once and may
/// never name another. A `Fresh` put re-using one would find the array empty, take `Revision(1)` from
/// the compare-and-swap as if it were a create, and re-create a key a reader's hold still names over
/// different extents.
#[test]
...
/// A later head may already have retired revision 1 before the queued compensation runs. `NotFound`
/// proves that exact publication is absent, so the caller neither retries forever nor removes
/// revision 2 instead.
#[test]
...
/// `Revision` is the compare-and-swap token every mutation carries, so a fresh publication has to be
/// exactly one past the head. A `Revision` that already exists is never overwritten.
#[test]
...
/// The card here is big enough to give each of the 1,916 entries an extent of its own, because the
/// mount that builds the free bitmap rejects an overlap and a fake catalog would prove nothing.
#[test]
...
/// The batched body write at the array's widest, which is the one place it could reach something that
/// is not body: the entry array is 480 blocks and the copy's gate is the block after them.
///
/// This pins the boundary — a window whose base drifted would program that gate as part of the body it
/// is about to certify. It does not pin the no-padding rule: 480 blocks is exactly 60 windows, so a
/// writer that padded every window would pass unchanged. Padding is caught by [`cost`](super::cost).
#[test]
...
/// Once both rollover gates are durable, a payload-copy failure leaves the store pending. Repair must
/// return success without applying the caller's bytes a second time or rewriting either gate.
#[test]
fn a_checkpoint_that_fails_after_its_page_flush_does_not_advance_the_flushed_length() {
    ...
    let tail = payload(PROGRAM_PAGE + 296);
    store.journal(ride_checkpoint(&tail[..300], &RESUME)).unwrap();
    let append = &tail[300..];
    let checkpoint =
        RideCheckpoint { id: ObjectId(1), revision: Revision(1), append, payload_crc: crc32(&tail), resume: &RESUME };
    // Ten writes reconstruct and gate the proof and logical slots. The following payload write
    // fails, leaving a pending resident rollover whose retry must not rewrite either gate.
    faulty.fault_after(MediaOp::Write, 10);
    ...
    let shorter = RideCheckpoint { append: &append[..append.len() - 20], ..checkpoint };
    assert_eq!(store.journal(shorter), Err(super::error::StoreError::Invalid));
    let mut changed = append.to_vec();
    changed[0] ^= 0x80;
    let changed = RideCheckpoint { append: &changed, ..checkpoint };
    assert_eq!(store.journal(changed), Err(super::error::StoreError::Invalid));
    ...
    assert_eq!(recovered.resume, RESUME);
...
#[test]
fn pending_rollover_survives_a_second_repair_failure() {
    ...
    // Refuse the first repair write. The authoritative proof and rollover gate remain unchanged.
    faulty.fault_after(MediaOp::Write, 0);
    assert_eq!(store.journal(checkpoint), Err(super::error::StoreError::Media));
    assert!(faulty.fired(), "the second fault did not reach proof repair");
    assert_eq!(disk.block(slot_header_block(1)), proof_header);
    assert_eq!(disk.block(slot_header_block(2)), logical_header);
...
/// Once the catalog atomically removes the recording, neither its possibly torn payload page nor its
/// pending proof is reachable, so discard need not satisfy the checkpoint-retry contract.
#[test]
fn a_pending_rollover_may_be_discarded_without_repairing_it() {
    let disk = card(45, &holding(&[recording()], 6), 0);
    let faulty = FaultOnce::new(&disk);
    let store = FlatStore::mount(&faulty);
    let tail = payload(PROGRAM_PAGE + 296);
    store.journal(ride_checkpoint(&tail[..300], &RESUME)).unwrap();
...
/// fails has not answered that question, and "the entry is gone" is not the answer to assume: the next
/// object would be written over a live entry's extent, and only a mount can see the overlap.
#[test]
...
/// A fragmented allocation is several block writes, and one can fail with the others already on the
/// card. What must survive that is the reservation: `row` names an `Allocation` by its cursor as well
/// as its nonce, so a cursor left ahead of the caller's makes the row unnameable — not publishable,
/// not retryable, not cancellable — until the next mount. The cursor goes back where it was instead.
...
/// The cursor did not move, so the retry writes the same bytes to the same payload offsets.
/// `Snapshot` reads every payload through the seam and hashes it against the entry's CRC.
#[test]
