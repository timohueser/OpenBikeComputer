// The v2 catalog root: one schema, the skins, the named regions, and the pins on
// the per-band cell indices. Normative in `OBCC_Spec.md` §11, produced by
// `obc-pack catalog --v2` / `obc-bake`.
//
// v1's §7 contract survives intact and is why this takes a *string*: a document
// is read whole and parsed as one JSON value, and a failure rejects all of it
// rather than leaving a half-populated catalog on screen. What v2 adds is that
// the guarantee now has to span four kinds of document instead of one, so the
...
// the builder promises to get right before a download starts (§11.5).
//
// The checks below are the spec's MUSTs, not a taste for strictness. Where §11
// says a consumer MUST reject, there is a `fail()` here with the reason spelled
// in the message.
 ...
 *  anything else is rejected outright (§7, §11.9). */
 ...
 *  (§11.8). **Presentation only** — it MUST NOT be used to compute a cell set
 *  (that is stored, §11.7), to price a selection, or as a packer input bbox. */
...
/** What a satellite has to match before it is believed (§11.1). */
...
function parseGrid(v: unknown, where: string): GridConstants {
    ...
    // §11.3 restates the constants so no consumer hard-codes them — but this one
...
function parseSkins(v: unknown, where: string, schema: SchemaEntry): SkinEntry[] {
    ...
    return raw.map((entry, k) => {
        ...
        const styles = arr(o.styles, `${at}.styles`).map((s, n) => {
            ...
            // §11.4: a skin naming a feature type the schema lacks is a stale
        ...
        const preview = optionalStr(o, "preview", at);
        if (preview) skin.preview = preview;
        return skin;
    });
}
...
function parseRegions(v: unknown, where: string, bandIds: Set<string>): RegionEntry[] {
    ...
    const regions = raw.map((entry, k) => {
        ...
        // §11.5: the split is what makes pricing per *file* rather than merely
...
/**
 * Parse a v2 root, or throw.
 *
 * `artifacts` (§11.9's migration field) is deliberately dropped rather than
 * validated: a v2 consumer MAY ignore it entirely, this client has no path that
 * offers a whole-region artifact, and carrying unvalidated legacy entries on the
 * parsed object is exactly how one of them ends up being used one day. A bakery
 * still serving both keeps working — the v1 client reads the same field from the
 * v1 document it is still publishing.
 */
export function parseCatalogV2(body: string): CatalogV2 {
    ...
    if (root.schema_version !== CATALOG_V2_SCHEMA_VERSION) {
        fail(
            `schema_version ${JSON.stringify(root.schema_version)} is not supported ` +
                `(this client reads ${CATALOG_V2_SCHEMA_VERSION})`,
        );
    }
    // §11.9: a preset is no longer a thing that exists; it is a schema and a
    // skin. A root carrying one is a generator that half-migrated.
    if (root.presets !== undefined) fail("catalog: `presets` must not appear in a v2 root");
...
export function band(catalog: CatalogV2, id: string): BandEntry | undefined {
 ...
 *  without exactly one. */
export function coreBand(catalog: CatalogV2): BandEntry {
    return catalog.schema.bands.find((b) => b.role === "core")!;
}

/** The region with this id, or `undefined`. */
export function region(catalog: CatalogV2, id: string): RegionEntry | undefined {
    return catalog.regions.find((r) => r.id === id);
}

/** The root's pin on a band's cell index. */
export function cellIndexRef(catalog: CatalogV2, bandId: string): CellIndexRef | undefined {
