...
// the extract and carries no bytes. `OBCC_Spec.md` §11.5 publishes `bytes` per
...
import type { BandRole, CatalogV2, RegionEntry } from "./manifest";
...
function coverageReport(
    ...
    for (const band of bands) {
        if (!band.partialCells.length) continue;
        if (band.contextOnly) {
            partialContextCount += band.partialCells.length;
            continue;
        }
        partialDetailByBand.set(band.band, band.partialCells);
        partialDetailCount += band.partialCells.length;
    }
    return {
        holesByBand,
        holeCount,
 ...
 *
 * This is `OBCC_Spec.md` §11.5's whole reason for putting `bytes`,
