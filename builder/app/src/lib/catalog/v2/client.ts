// The v2 catalog client: one root, its digest-pinned satellites, and nothing
...
//
// The shape of the guarantee (`OBCC_Spec.md` §11.1): the **root** is the only
...
// exactly the guarantee v1 had from one file: the whole consistent thing, or
...
export interface CatalogV2ClientOptions {
...
/**
 * A loaded v2 catalog and the lazy path to its satellites.
 *
 * Construction is `CatalogV2Client.load(url)` rather than `new`: a client with
...
export class CatalogV2Client {
    ...
    private readonly opts: CatalogV2ClientOptions;
    ...
    private constructor(catalog: CatalogV2, baseUrl: string, opts: CatalogV2ClientOptions) {
     ...
     * which is why §11.1 short-caches it and content-addresses everything else.
     */
    static async load(url: string, opts: CatalogV2ClientOptions = {}): Promise<CatalogV2Client> {
        let base: string;
        try {
     ...
     * (§11.7's cross-document MUST). It is not checked against ones that are
    ...
    private downloadOptions(): DownloadOptions {
        return { fetchImpl: this.opts.fetchImpl, digest: this.opts.digest, signal: this.opts.signal };
    }
}
