// The message vocabulary between the UI and the assembly worker.
//
// The assembly is one synchronous wasm call, so it runs in a dedicated Worker; the
// UI's cancel is `worker.terminate()`; progress crosses by `postMessage`. This
// module is the *words* of that conversation — types, guards and transfer-list
// builders — kept apart from the worker's entry point so the protocol is testable
// in Node, where `Worker` does not exist.
//
// A run produces exactly one map and says so exactly once, in one of two ways:
//
//   * `stored-map` — the assembly wrote the file straight into OPFS through a sync
//     access handle, so it was never in wasm memory and never crossed this port.
//     What arrives is a digest and a length. The worker posts it only after closing
//     its handle, because a sync access handle is an exclusive lock and the page
//     cannot open the file while the worker holds one. This is the path that
//     matters: a DACH map is larger than the wasm32 address space.
//   * `file` — the admitted mode permits buffering and no sink opened, so the bytes
//     ride the port in the transfer list. They *move* rather than copy.
//
// Cells can travel as names instead of bytes: `sourceCells` carries an identity, a
// length and an OPFS filename per cell, and nothing is transferred. The worker is
// the only thread that can read them synchronously, which is what the wasm engine
// needs. A request with `cells` is the same conversation with the bytes in it, for a
// browser with no usable storage.
//
// Every failure is one `error` message carrying the bridge's own code. An uncaught
// error in a worker is a console line the UI cannot read, and `AssembleError`'s code
...
/** One cell that stayed on disk. Same identity as {@link WorkerCell}, with the
 *  catalog's byte count and the OPFS filename — its digest — instead of the bytes.
 *  Nothing is transferred: that is the point. */
...
/** One terrain cell crossing into the worker. Same fields as `AssembleTerrainCell`;
 *  a canonically void square is simply not sent, because it has no object at all. */
...
/** The catalog's terrain lattice. Absent means the catalog publishes no raster and
 *  the set carries no `terrain` role. */
...
export type AssembleWorkerRequest =
    | {
           ...
           *  worker can assert. */
          onDisk: boolean;
          /** The engine's sort budget, which after the external merge **is** the
           *  engine term on an OPFS host. */
          mergeBudgetBytes: number;
          /** Lowered by a caller that knows it is on a phone. */
          budgetBytes?: number;
      }
    | {
          type: "assemble";
           ...
           *  `cellStore` naming the revision directory they are in. A request may
           *  carry both lists; the browser sends one or the other. */
...
/** The assembled map, written **into OPFS**: its identity, and nothing else. The
 *  page reads it back with `readMapOutput()` and saves it under a name it chose
 *  itself, so nothing map-sized — and no path-shaped string — crosses this port. */
...
/**
 * How the run that is starting gets at its cells. Posted **before** the assembly,
 * not with its result, because the case where it matters most is the one that never
 * produces a result: a bug report about a failed country-scale run has to say which
 * path was taken.
 *
 * - `streamed` — from OPFS through sync access handles, the engine reading a block
 *   at a time. Input residency is the read cache.
 * - `buffered` — from OPFS, but read back into memory first, because this browser
 *   has no sync access handles.
 * - `memory` — the cells arrived over the wire in this request. No store.
 */
export type CellReadMode = "streamed" | "buffered" | "memory";

/**
 * …and where this run's map went, posted for the same reason.
 *
 ...
 * - `memory` — buffered in wasm memory and handed over at the end. What binds the
 *   size of map a browser without usable storage can build.
 */
...
/**
 * Whether a value that arrived over `onmessage` is a response this protocol speaks.
 * A worker's inbox is an untyped seam — a stray message must be droppable rather
 * than crash the download screen — so the guard checks shape, not just the tag.
 */
