 ...
 *
 * A map is one object. There is no multi-file map upload here — no manifest, no shards, no separate
 * terrain file to order against them — so every write in this file is a single `PUT` against one
 * `.obcm`, `.obcr` or `UPDATE.BIN`, and there is no state that outlives a transfer.
 *
 * All three are the same shape underneath: declare the length, the whole-payload CRC, the kind and a
 * display name, stream the payload, read back what the commit published. What differs is what has to
 * be true *before* the first byte moves — the map checks the selected map's current id and revision,
 * the route has its OBCR header read back and shown, and the firmware has its whole OBCU container
 * checked (header CRC, image CRC, slot ceiling).
 *
 * "Uploaded" means the device has a valid object. The device verifies the declared length and the
 * whole-payload CRC, runs the kind's validator, and only then commits; anything else is an error
 * response, which `FlatStoreClient.put` turns into a throw.
 *
 * Cancelling and unplugging are not special-cased. Every await takes the job's signal, and any break
 * before the commit leaves the card as if nothing had happened: the allocation is released, the
 * written bytes are anonymous, the catalog is untouched. The next attempt is an ordinary first
 * attempt rather than a recovery path.
 *
 * There is no free-space check before a map send. A `PUT` that does not fit is `noSpace`, whose
 * context is the bytes required, so asking in advance would be a second answer to the same question
 * and a stale one by the time the bytes arrive.
 */
...
/** The display-name field: at most 48 UTF-8 bytes. */
 ...
 *
 * No second staging copy: a picked `File` and the assembler's Blob are read twice in bounded chunks,
 * once for the CRC and once to send. That is what `blobSource` is for.
 *
 * Replaces the map the device will select: the active (non-retained) `MapShard` with the lowest
 * `ObjectId`. That is the firmware's deterministic selection rule, so a second send moves the map the
 * rider is actually using instead of accumulating an unreachable sibling. A card with no map creates
 * one. `LIST` supplies both compare-and-swap fields immediately before `PUT`.
 */
...
async function sendMapSource(
    ...
    return client.put(
        ...
        source,
        {
            signal: ctx.signal,
            onProgress: (done, total) => ctx.progress(done, total),
            // The commit is bounded work the ordinary timeout covers, and it still takes long
            // enough to be worth naming: the device has to land the last staging half first.
 ...
 * the same GPX twice therefore makes two objects. The catalog carries every object's payload length
 * and CRC, so a caller that wants convergence looks before it uploads —
 * `FlatStoreClient.findCreated` is that lookup, used to reconcile an upload whose answer was lost
 * rather than to pre-empt an ordinary second drop.
 */
...
/**
 * Verify an `UPDATE.BIN` and write it to the card as an update-package object.
 *
 * **Staging only.** Uploading never installs; `ARM` is the separate, explicit step
 * ({@link armUpdate}), and the two are different decisions precisely so delivery cannot arm anything.
 *
 * It **replaces** an update package the card already holds rather than adding a second one, which is
 * client-side policy rather than a wire rule: without it every staging attempt would leave another
 * multi-megabyte object on the card. The compare-and-swap on the existing revision is what makes it
 * safe — a package something else replaced in between fails rather than clobbering.
 */
...
/**
 * Ask the device to make a staged package the next boot.
 *
 * **The device's current policy refuses this**, answering `rejected` — a stated gap, not a failure of
 * this call. It is wired because the shape is settled and a refusal a rider can read is better than
 * an affordance that quietly does nothing.
 *
 * On success the device commits a rollback reserve, writes the boot handoff, answers, and reboots, so
 * the answer is the last thing this link will hear from it.
 */
