 ...
 * hold one.
 */
export const REFERENCE_OBCM_VERSION = 12;
    ...
    private async transfer(d: TransferControl): Promise<void> {
        if (d.op === Op.Abort) {
            ...
            if (!active) {
                ...
                // **Only `mapSet` abandons the staged set.** A `mapShard` abort here is the quiesce
                // the host sends after a shard the device refused, and the caller is about to
                // re-send that one shard — deleting the set under it is how a single CRC refusal
                // used to take the whole map with it.
                ...
                if (d.type === ObjectType.MapShard) {
                    // Quiesce only: the pipe is empty, the set is untouched.
                    await this.status({
                        msg: "transferResult",
                        objectId: d.objectId,
                        status: TransferStatus.Aborted,
