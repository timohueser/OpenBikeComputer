...
//! folder. This module is the **host** side of the [`obcm_route`] abstraction: it
//! backs [`ByteSource`](obcm_route::ByteSource)/[`ByteSink`](obcm_route::ByteSink) with
...
//! over FatFs; nothing above this line (`obcm-app`, `obcm-render`) knows it's a folder.

use std::path::{Path, PathBuf};

use obcm_route::{gpx_to_obcr, ByteSink, Error, RouteStats, RouteSummary, SliceSource};
...
impl RouteStore {
    ...
    /// The route catalog (summaries), for [`App::set_routes`](obcm_app::App::set_routes).
    ...
    /// A [`ByteSource`](obcm_route::ByteSource) over the active route's bytes, for
    /// opening a [`RouteReader`](obcm_route::RouteReader) to stream geometry from.
