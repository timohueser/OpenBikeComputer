...
/// restarts: `$XDG_CONFIG_HOME/obcm-sim/calibration` (else `$HOME/.config/...`).
fn config_path() -> Option<PathBuf> {
    let base = std::env::var_os("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|h| PathBuf::from(h).join(".config")))?;
    Some(base.join("obcm-sim").join("calibration"))
}
...
mod tests {
    ...
    fn save_load_roundtrips_and_rejects_junk() {
        let dir = std::env::temp_dir().join(format!("obcm-sim-calibtest-{}", std::process::id()));
        ...
        std::fs::write(dir.join("obcm-sim").join("calibration"), "not a number").unwrap();
        assert_eq!(load(), None);
        std::fs::write(dir.join("obcm-sim").join("calibration"), "-3").unwrap();
