...
The marker has to render **byte-identically on the simulator and the real nRF5340
...
1. **The marker is drawn in the shared path, never in a host.** Do **not** add
   marker drawing to `obcm-sim/src/gui.rs` or `framebuffer.rs`. The host passes a
...
2. **Dependency direction is `obcm-sim → obcm-app → obcm`.** `Fix`/`AppState` live
   in `obcm-app`; `obcm` cannot see them. So the drawing primitive in `obcm` takes
   **plain numbers** (`lon: i32, lat: i32, course: Option<f32>, color`), and
   `obcm-app` unpacks `self.state.user_fix` into those. Never make `obcm` depend on
   `obcm-app`.
...
- **`obcm-app`** — `App::render_frame`, after `MapRenderer::render`, reads
...
**Header today** (`obcm/serialize.py:215`, `reader.rs:166`): `struct "<4sBiiiiIBI"`,
...
- **(clean, needs network) Re-pack** `monaco` from a fresh extract:
  ...
  `.venv/bin/python obcm_pack.py monaco.osm.pbf config.json monaco.obcm` once the
  packer writes v4.
...
- `obcm/serialize.py`: `V3_HEADER_LEN = 30` (line 180) → add `V4_HEADER_LEN = 32`;
...
Anchors in `webapp/static/app.js`:
...
### `obcm-app/src/app.rs` — the wiring (the only glue)
...
already imported here. **No host changes** — `obcm-sim` and the firmware call
...
Run cargo from `viewer-rs/`. Sample maps are at the repo root (`../monaco.obcm`).
...
cargo test --workspace                         # obcm 12, obcm-app 10, obcm-sim 7 (+ your new tests)
cargo build -p obcm-app --target thumbv8m.main-none-eabihf   # firmware-readiness MUST hold

# Headless visual check (marker rides on the seeded fix):
./target/release/obcm-sim ../monaco.obcm --true-color --heading 45 --png /tmp/m.png
...
- **obcm-app**: `render_frame` draws the marker when `user_fix` is set and skips it
...
**Locked**
- Marker drawing lives in the **shared renderer** (`obcm` draws, `obcm-app` wires);
  hosts unchanged. (§2)
