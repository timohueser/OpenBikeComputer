...
- Every line is a solid single-colour stroke: the `Kind::Line` arm of
  [`MapRenderer::render`](../obcm-render/src/lib.rs#L478) → `Polyline::with_stroke(color, weight)`.
...
- **Style record = 6 bytes**, `struct "<BbHBB"` = `id, z_index(i8), color(u16 RGB565), weight(u8),
  ...
  [reader.rs `parse_styles`](../obcm-reader/src/reader.rs#L508), [`OBCM_Spec.md` §2](../../OBCM_Spec.md)).
  **flags bits 0–1 = priority; bits 2–7 are free.** There is **no second colour** field today.
- `Style` struct: [reader.rs:28](../obcm-reader/src/reader.rs#L28). The draw-time `Span`
  ([lib.rs:349](../obcm-render/src/lib.rs#L349)) caches the resolved `color`/`weight` but **not**
...
- Authoring: [`config.json`](../../config.json) is the source of truth; the webapp editor builds a
  ...
  [`app.js` ~L530–674](../../webapp/static/app.js#L530).

## Format change → **v6**

We must bump the format (the repo hard-cuts old versions; [reader.rs:176](../obcm-reader/src/reader.rs#L176)
...
- **`Style`** ([reader.rs:28](../obcm-reader/src/reader.rs#L28)) gains `line_style: u8` + `color2: u16`;
  `parse_styles` reads them ([reader.rs:524](../obcm-reader/src/reader.rs#L524)).
...
- **`stroke_dashed()`** (new, in `lib.rs`): walk the **already-projected** screen points, accumulate
  ...
  - `railway` → solid base pass in `color`, then `stroke_dashed(color2)` on top (the cheap, well-reading
    ...
    [`arm`](../obcm-app/src/screen/map.rs#L353)) — more "railway", a bit more code.
...
- **`webapp/static/app.js`** style row (~[L672](../../webapp/static/app.js#L672)): add a line-style
  ...
  [app.js:129](../../webapp/static/app.js#L129)).
...
Requires a **repack** of `freiburg.obcm` with the new config (≈ 213 MB, minutes — see `obcm_pack.py`).
...
- **`obcm-reader/tests/format.rs`**: extend the byte builders for the **8-byte** record; round-trip
