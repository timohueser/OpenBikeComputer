...
* **`MapRenderer::draw_map`** (`obcm-render/src/lib.rs:482`) — the draw phase, its
...
1. **Format (v6) first** — follow `line_styles_plan.md` "Format change → v6":
   ...
   u16); reader `Style` (`obcm-reader/src/reader.rs:28`) gains `line_style: u8` +
...
* `no_std` crates cross-build: `cargo build -p obcm-render -p obcm-reader
  -p obcm-route --target thumbv8m.main-none-eabihf`.
