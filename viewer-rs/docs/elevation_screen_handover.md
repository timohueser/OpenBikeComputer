...
2. **Elevation profile from the route.** Add a helper that fills a fixed
   ...
   `min_ele_m..max_ele_m`. Put it on `RouteReader` (`obcm-route`, testable, MCU-shared).
...
4. **`RouteMatch`** (new, in `obcm-route`): a forward-biased cursor (chunk + segment +
...
read the PNG — profile shape, peak label, stat grid. Add `obcm-route` unit tests for the
