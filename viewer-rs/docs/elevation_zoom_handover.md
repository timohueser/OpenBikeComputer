...
- `obcm-route`: grow `Profile` (`src/profile.rs`) into a small pyramid — a finest-level
...
- `obcm-app` `screen/statistics.rs`: add zoom state (zoom level + pan offset + an idle
...
`obcm-route` unit tests for the pyramid (downsample correctness; `window()` ranges at a
