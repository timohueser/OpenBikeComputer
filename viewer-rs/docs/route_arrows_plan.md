...
- The route is one solid polyline: [`MapRenderer::draw_route`](../obcm-render/src/lib.rs#L602) projects
  each chunk and strokes it with `Polyline::with_stroke(color, weight)`. Called from
  [`screen/map.rs:94`](../obcm-app/src/screen/map.rs#L94) with `palette::ROUTE` (deep blue) at
  `ROUTE_WEIGHT = 4` ([map.rs:37](../obcm-app/src/screen/map.rs#L37)). The breadcrumb (`stroke_path`)
  strokes over it; the marker over that.
- **The chevron machinery already exists twice:**
  - [`draw_marker`](../obcm-render/src/lib.rs#L529) builds an oriented chevron from a *forward unit
  ...
  - The pan HUD's [`chevron` / `arm`](../obcm-app/src/screen/map.rs#L325) draws an **open, round-capped
...
- The route is decoded **in travel order**, chunk by chunk, consecutive chunks sharing a seam vertex
  ([`draw_route` loop](../obcm-render/src/lib.rs#L616)). So "direction of travel" is just increasing
  index; no extra data needed.
- `draw_route` runs **after** `render()` returns, and `render()` returns `stats.lod`
  ([map.rs:90](../obcm-app/src/screen/map.rs#L90)). So the finest-LOD gate is
...
- **Direction** at an arrow = the normalised local segment delta `(dx, dy)` you already have from the
  ...
  alpha-max-plus-beta-min `|v|` approximation from [`arm`](../obcm-app/src/screen/map.rs#L353) to avoid
...
- **`obcm-render/src/lib.rs`** — extend `draw_route` to take `draw_arrows: bool` and emit chevrons on
  ...
  idea.
- **`obcm-app/src/screen/map.rs`** — compute `draw_arrows` from the returned `stats.lod`, pass it to
...
`RenderStats` ([lib.rs:325](../obcm-render/src/lib.rs#L325)) is the natural carrier, but `obcm-render`
...
- The **host** times the work and fills it: the sim already has a monotonic clock
  ([`device_input.rs` `now_ms`](../obcm-sim/src/device_input.rs#L57) / `Instant`); time `draw()`
  (render + overlays) and write it into the stats it stores
  ([`gui.rs` `last_stats`](../obcm-sim/src/gui.rs#L117)). On device, the same field is later filled from
  the Cortex-M **DWT->CYCCNT** cycle counter — no API change.
- **Display:** add a "render: X.X ms" row to the stats panel
  ([`gui/panel.rs` ~L397–433](../obcm-sim/src/gui/panel.rs#L397)) and the field to the headless line
  ([`main.rs` ~L589–596](../obcm-sim/src/main.rs#L589)).
...
- Tests (`obcm-render`): arrow count scales with route length / spacing; **no** arrows emitted when
  `draw_arrows = false`.
