...
| --- | --- | --- | --- |
| `komoot-schwarzwald.gpx` | komoot export, `creator="https://www.komoot.de"`, "Schwarzwald Tour · Tag 2": 150 track points and five `<wpt>`. The symbols are Garmin names komoot copies through — `Flag, Blue` three times, `Restaurant`, and `Fishing Hot Spot Facility`, which no category fits. | GPL-3.0-only (authored route) | `companion-ios/Packages/OBCKit/Sources/OBCMock/Fixtures/sample-import.gpx` |
