...
the Peak View regression test draws. This is the only ground truth Peak View has.
...
| View | Position | swissALTI3D ground | Camera | LiDAR horizon |
| --- | --- | ---: | --- | ---: |
| `hahnen` | 46.80966, 8.42112 (Brunni) | 1022.9 m | 26 mm, heading 58.61°, pitch +12.71° | 100 % |
| `rigidalstock-from-brunni` | 46.80966, 8.42112 | 1022.9 m | 26 mm, heading 10.63°, pitch +7.91° | 100 % |
| `urnerstaffel` | 46.86881, 8.44342 | 1694.5 m | 26 mm, heading 141.83°, pitch +18.57° | 100 % |
| `below-titlis` | 46.85212, 8.41904 | 2564.5 m | 48 mm, heading 184.22°, pitch −0.07° | 54 % |
| `below-west` | 46.85212, 8.41904 | 2564.5 m | 48 mm, heading 228.83°, pitch +0.45° | 32 % |
| `top-south` | 46.85227, 8.41959 | 2591.3 m | 48 mm, heading 150.56°, pitch −0.67° | 81 % |
...
above the horizontal plane through the camera, on a quarter-degree bearing grid.
`lidar_horizon_fraction` is the share of the cast bearings whose horizon the 2 m reference found,
rather than Copernicus beyond it. `rms_limit_deg` is the limit the test holds the drawn skyline to.

**A seventh photograph, `rigidalstock-top-north`, is not here.** It looks north-west over the
Mittelland: its horizon is 50 km away, outside this shard's own box, only 2 % of its cast bearings
find that horizon in the 2 m reference, and its sky floor of 0.72 was never swept. The line that
came off it was a ridge in the foreground, not the skyline, so it measured the wrong thing. The
commit that retired `scratch/peak-view/` still holds the photograph.

## Where the photographs come from

The owner took all of them near Engelberg, Switzerland, and gave the positions he stood at. The
...
comparison treats as truth. It is probed from the reference rasters here rather than copied from
`scratch/peak-view/photos.toml`, and two values move a little for it: `urnerstaffel` 1697.0 →
1694.5 m and `top-south` 2591.4 → 2591.3 m.
...
The test asserts that second digest, because a repacked shard would move every limit at once and
without a word.
...
### What the camera fit does and does not establish

- **The heading is bounded to ±12° of a hint**, and every fit landed inside that bound with room
  to spare (the widest is +11.63° on `rigidalstock-from-brunni`, whose hint is a gazetteer bearing
  for a wide ridge). Widening the bound to ±25° moves no answer at all, so the bound checks the
  fit rather than deciding it. An unbounded search does wander, which is why there is a bound.
- **`photos.toml`'s heading and pitch cannot validate these.** They came from a fit with the lens
  **fixed at 26 mm**, and the fit here finds four of these frames to be 48 mm. Agreement between
  the two is therefore not evidence: the headings differ by up to 7.0° and the pitches by up to
  2.8°, and the 26 mm numbers are the ones the wrong focal length distorts.
- **The focal length is the long-side convention**, `f = (long side / 2) × mm / 18`. Apple states a
  35 mm equivalence over the frame *diagonal*, which for a 4:3 frame is about 3 % shorter than
  this. The effect is 3 % of every off-axis angle; it is inside the 0.5° filter over these frames
  and it is not corrected here.
- One frame is portrait and five landscape. `fit.py` derived the focal length from the frame's
  **width**, which is wrong by 4/3 for a portrait frame, so that affected `hahnen.jpg` alone. The
  48 mm finding is separate and came from the lens search, not from the portrait fix. Two of the
  landscape frames are 16:9 and so are cropped from 4:3, but the crop keeps the full width, which
  is still the original long side.
...
- **The reference stops, and so does the box.** swissALTI3D was fetched for about 15 km around
  ...
  its west edge is 42.5 km from the Rigidalstock, which is nearer than the Bernese Alps on the
  south-west skyline.
- **In `below-west` the line is a near ridge, not the far horizon.** That frame is hazy enough
  ...
  no `sky_floor` separates the two (0.95, 1.0 and 1.03 were tried; above 1.0 the mask starts
  calling real sky terrain). The columns are still a real terrain edge at a real angle, and the
  0.5° rule keeps only the ones the reference agrees with, but its limit is looser for it.
