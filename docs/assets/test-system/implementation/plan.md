...
| `obc test heavy NAME` | One explicitly named hermetic heavy recipe. Unknown names fail. |
...
- Keep `obcm-assemble`'s allocator measurement in its own target. Carve the `fixtures.*` suites that select with `--test <name>` as named targets so their routes survive. Use nextest locally and in CI for merged targets with process-global state.
...
Maintain a small explicit list of hermetic heavy recipes: the existing render exhaustive ignored-library suite; the display timing probe as a measurement recipe; and the retained iOS UI suite on a supported simulator. Preserve their native exit status and results. Do not invent a shared scenario format or a result converter.

The release-heavy workflow runs correctness recipes on the exact release revision on their required platforms before publishing. Make the release publisher depend on successful completion, not merely an artifact from an older branch head. Timing probes produce evidence without unstable shared-runner timing thresholds. Generators, external-service downloads, and physical procedures are excluded. Failures stop publication. Keep existing release version/key/image verification unchanged. [firmware/obc-render/src/rain.rs:1468](firmware/obc-render/src/rain.rs:1468) [firmware/obc-display/src/ls021/rowdiff.rs:342](firmware/obc-display/src/ls021/rowdiff.rs:342) [release.yml:3](.github/workflows/release.yml:3)
