...
| Document | Purpose |
| --- | --- |
| [Requirements](requirements-draft.md) | 371 requirements in 27 groups, written for review and manual entry. REQ-336–358 retain their 16 September review status. REQ-359–372 and revised REQ-239/262 are accepted additions from 18 September. |
...
Keep requirement IDs stable when editing or moving entries. REQ-321 was retired when tunnel warnings were removed; do not reuse it. The last allocated ID is REQ-372. Update referenced acceptance criteria when a requirement changes. A TODO is unfinished acceptance work, not a waived requirement or a passing result.
