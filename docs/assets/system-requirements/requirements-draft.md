...
The 18 September owner review accepts REQ-359–384 and the revised REQ-239 and REQ-262. These define device coverage notices, routing suitability, operation after failures, lifetime independence, and riding usability. They do not approve the separate 16 September proposals or establish implementation evidence.
...
- **REQ-375 — Configurable glanceable information.** The device shall let the rider make battery state, remaining distance, next waypoint, current speed, moving average speed, recorded ascent, remaining ascent, current route grade, and the whole-route elevation profile available at a glance on configured riding views. Values and their meaning shall be recognizable without opening a menu or detail view. They need not all appear on one page. Missing inputs shall follow REQ-172.
...
- **REQ-384 — Route upload during a ride.** The paired companion shall be able to upload a route while recording is active and not paused, including with the supported heart-rate, power, and cadence sensors connected. Transfer shall not require pausing, disconnecting those sensors, or ending navigation. Recording and sensor freshness shall continue to meet their requirements under the declared supported transfer conditions.
...
- **REQ-373 — Essential riding controls.** Switching between Map, Statistics, and the enabled Climb view, changing map zoom, and pausing or continuing a ride shall use consistent physical controls without text entry or a menu search. These actions shall meet the riding-interaction criteria in acceptance-targets.md. Ride Assistant operation, detailed browsing, and settings changes are outside the required while-moving interaction scope.
- **REQ-374 — Essential status at a glance.** Recording failure, off-route state, and unavailable live position shall each be recognizable from Map, Statistics, and Climb without opening a menu or detail view or reading a full sentence. Off-route state and unavailable position shall remain distinguishable. An unresolved recording failure shall remain indicated after its warning is dismissed.
- **REQ-376 — One-press pause and continue.** From Map, Statistics, or Climb during an active recording, the rider shall be able to pause with one short button press without a confirmation. From the paused ride view, the rider shall be able to continue with one short button press. These actions shall preserve the same recording.
- **REQ-377 — Direct navigation gestures.** From each ordinary riding view, the rider shall be able to open Ride Assistant, the main menu, the top quick drawer, and the applicable bottom context drawer with one gesture for each destination. A gesture may be a short press, a hold, or a simultaneous button combination; it shall not require first opening another view.
- **REQ-378 — Feature and setting access.** Each device feature and setting shall have a documented access path of at most four actions from the main menu or an applicable ordinary riding view. Actions, starting states, and the boundary between access and task completion shall follow the counting rules in acceptance-targets.md.
- **REQ-379 — Full-screen interruptions during a ride.** While a ride recording is active and not paused, unsolicited full-screen messages shall be limited to safety-critical or function-critical conditions, such as storage read/write failure, except for the rider-initiated route-receipt prompt in REQ-381. Zero speed shall not by itself lift this restriction. The restriction does not apply while paused or without an active recording; input isolation and explicit acceptance rules still apply.
- **REQ-380 — Riding information banners.** During an active, unpaused recording, informational notices, including approaching waypoints, off-route indications, and sharp-turn warnings, shall use partial-screen banners or indicators instead of taking over the screen. They shall preserve enough of the underlying riding view to retain its essential context, disappear without requiring acknowledgement when no longer relevant, and preserve any required sound or persistent state indication.
- **REQ-381 — Rider-initiated route-receipt exception.** Receipt of a route deliberately sent from a connected application shall be permitted to show a full-screen route-received or route-updated prompt during an active, unpaused recording. The prompt shall include any coverage notice required by REQ-359, allow dismissal without changing navigation or recording, and dismiss automatically after a defined interval without accepting a navigation change. It shall not interrupt an input gesture, pending confirmation, emergency-location screen, or higher-priority critical warning.
- **REQ-382 — Return to the riding map.** From any ordinary browsing view during a ride, including Ride Assistant and article views, the rider shall be able to return to the live position-following map in at most two actions without changing navigation or recording. When a live position is unavailable, the map shall show its defined unavailable-position state. Protected confirmation, fault-recovery, and installation states shall retain their explicit completion or exit rules under REQ-206.
- **REQ-383 — Main-menu default.** Each time the main menu opens, Map shall be the selected default entry. Selecting it shall open the riding map during a ride. Together with the one-gesture main-menu action, this shall provide the two-action return path without scrolling.

The while-moving interaction scope is limited to essential riding controls. Convenient access to Ride Assistant does not require its search, comparison, or acceptance flows to be usable while moving. Display brightness remains available through the quick drawer; no additional while-moving brightness promise is added. These requirements do not claim that operating the device is safe in all riding conditions. Existing rider-selected automatic Climb presentation remains governed by REQ-178–179.
