...
## Riding usability and interruptions

REQ-373–384 define the interaction scope. An ordinary riding view is Map, Statistics, Climb when enabled and available, or the paused ride view. The while-moving scope covers view switching, map zoom, and pause/continue. It excludes Ride Assistant flows, detailed reading, and settings changes. No physical speed threshold is needed to enforce the notification policy: an active, unpaused recording is the protected state, including when the rider stops without pausing.

| Criterion | Required behavior or bound | Conditions and open details |
| --- | --- | --- |
| UI-01 | Essential controls without text entry or menu search | Switch riding views, zoom the map, and pause/continue using consistent physical controls. Evaluate brief visual attention and control errors in representative riding use. Viewing conditions, attention limit, and success/error thresholds: TODO. No new glove or wet-hand commitment is inferred. |
| UI-02 | Recognize essential status without a full sentence | Distinguish recording failure, off-route state, and unavailable live position on Map, Statistics, and Climb. Status identification conditions and recognition-time bound: TODO. Apply color accessibility under REQ-304. |
| UI-03 | Configurable information at a glance | Battery, remaining distance, next waypoint, current and moving average speed, recorded and remaining ascent, current route grade, and whole-route elevation profile. Configuration may distribute these across riding pages. A profile must show the whole route, with progress when available, without entering inspection. Text/graphic recognition conditions and limits: TODO. |
| UI-04 | Pause and continue: one short press each | Pause from each ordinary active riding view; continue from the paused ride view. Count the press that changes state, not merely access to a control. No confirmation or hold. |
| UI-05 | Main menu, Assistant, and drawers: one gesture each | Count one short press, one completed hold, or one simultaneous button combination as one gesture. Apply from each ordinary riding view, with a context drawer where that view has context actions. Exact button bindings remain interaction-design choices. |
| UI-06 | Feature or setting access: at most four actions | For each fixed feature or setting, document a qualifying path from the newly opened main menu with Map selected or a named ordinary riding view. Count each selection movement, activation, hold, or button combination; include page changes. Do not count an arbitrarily long held scroll as one action: count each selection step it produces. Do not rely on remembered menu selection or a specially prepared shortcut. |
| UI-07 | Boundary of the four-action limit | End when the feature view or setting control is available. Value editing, confirmation of a consequential change, and traversal of variable-length content lists such as saved rides, routes, places, or article pages are task completion, not fixed-feature access. Access to each such list still counts. The bound is four actual actions, not four menu levels. |
| UI-08 | Full-screen warning policy while active and unpaused | Allow critical conditions that require attention to protect the rider or preserve core device operation, including storage faults. Waypoint, off-route, and sharp-turn notices use partial banners/indicators, with required sound. Rider-opened flows and configured automatic Climb transitions are not unsolicited warning popups. Exact critical-event list and priorities: TODO; an ordinary informational event must not be classified critical only to bypass the rule. |
| UI-09 | Explicit route upload is a full-screen exception | Permit the received/updated-route prompt even when the rider has stopped without pausing. Include coverage information; dismissal and timeout preserve navigation and recording. Defer presentation during an input gesture, pending confirmation, emergency-location screen, or higher-priority critical warning. Timeout and deferred-prompt handling: TODO. Route-start coverage information under REQ-360 remains brief and non-blocking. |
| UI-10 | Return to live map: at most two actions | From ordinary browsing, including Assistant and articles: one gesture opens the main menu with Map selected; one press opens the live map. Preserve navigation and recording. Protected confirmation, recovery, and installation states follow their own exit rules. No gesture may accept a pending change as a side effect. |
| UI-11 | Route upload with active recording and sensors | Include paired-phone route transfer with all supported sensor quantities connected. Preserve navigation and recording and satisfy sensor freshness and data-loss bounds. Supported transfer workload, radio conditions, and timing use PERF-17 and remain TODO; this is not evidence of measured radio performance. Map upload remains prohibited during any ride session under REQ-282. |

Four actions is an access target to verify against the complete fixed feature/settings inventory, not a claim that the current interface meets it. It must not be reinterpreted as four submenu levels if menu scrolling exceeds the bound. No completed UI action-count audit or riding-usability trial is claimed.
