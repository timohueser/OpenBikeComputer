...
Weather follows that split before any screen or cache is attached. [`obc-formats/src/obcw.rs`](src:firmware/obc-formats/src/obcw.rs) owns only OBCW's fixed byte facts and checked slice codecs. The sibling [`obc-weather`](src:firmware/obc-weather) core walks an arbitrary `ByteSource`, validates the whole object, and decodes one 16 × 16 rain tile into caller-owned memory. It has no SD-slot, refresh, provider, alert, or render knowledge, and the dependency guard classifies it as core so it cannot pull platform storage upward. The weather app/store work composes around the reader without turning the format authority into a subsystem: `obc-app`'s `WeatherDomain` owns the installed identity, visible freshness, and the alert decision, and the flat store validates a weather object before it becomes visible.

Validation keeps that random-access seam practical: the reader hashes 512-byte blocks and walks
hourly/tile directories in bounded windows. The DWD-shaped regression pins open at 269 reads and
864 bytes of explicit scratch, so the abstraction remains allocation-free without degenerating
into one SD seek per record or payload.
...
<figure class="fig">
<svg viewBox="0 0 720 372" role="img" aria-label="The shared core sits in the middle and connects through four seams to each host. DrawTarget carries pixels out: both hosts render into a resident RGB222 framebuffer and present it through the shared display contracts — a native-frame format paired with a presenter (the device's FLPR scans the frame to the panel; the simulator self-diffs it and uploads the changed rows to a texture). The colour function maps a 16-bit colour to native RGB222 (64-colour) on both hosts. ByteSource brings bytes in (an in-memory slice in the sim; the map's bytes on the SD card on the device — a FAT file today, an object in the flat store after the cutover). The HAL traits bring the world in (the control panel, a GPX replay and the keyboard in the sim; GPS, a barometer and GPIO buttons on the device).">
  ...
  <text class="d-sub" x="610" y="251" text-anchor="middle">FAT file &#8594; flat store</text>
...
**`ByteSource` — reading bytes you don't have room for.** A full map is far larger than the device's RAM, so the reader never holds the whole file. It reads through a tiny random-access trait, a chunk at a time, backed by an in-memory slice on the host and by the map's own bytes on the device's SD card — a FAT file today, an object in the [flat store](src:specs/FLAT_Store_Format.md) once the cutover below lands:
...
Beyond rendering, each loop pass also carries the app's **requests** to the platform and the platform's **answers** back. One call carries all of it: `App::run_pass` takes what finished, what changed underneath, and what the rider did, runs every domain in a fixed order, and returns a plan — what to repaint, when to come back, and one bounded **effect** per domain. Nothing is a "command the host drains": an effect is work a domain decided on, and it carries an **operation token** the answer must hand back, so a domain refuses an answer to something it has since cancelled or replaced rather than acting on it. Effects and outcomes carry only bounded ids and small results — bulk data such as catalogs and elevation profiles stays in app-owned buffers the platform fills directly — and the input/overlay plane deliberately stays *off* this channel, so a button edge never queues behind store work.

The *sequencing* is therefore not the platform's to get right: it is inside the pass, written once and identical everywhere. What is left outside is **execution** — do this read, this delete, this planner step — written once for every frame-stepped host in [`obc-host-core`](src:host/obc-host-core/src/dispatch.rs), against narrow repository interfaces that the simulator's folder-backed stores and the web demo's in-memory stores both satisfy. The board runs the same pass behind its own asynchronous execution. A shared conformance suite replays one scenario table through both answer cadences — same call and one pass later — and requires the same rider-visible end state, which is what proves behaviour does not depend on how fast a platform answers.

Three things are still asked for the old way, and they are named as a list rather than left as a habit: closing the ride log, forgetting the paired phone, and deleting a trip. Each is a request whose completion no domain can yet validate — the close is answered by a catalog re-feed rather than by a ride identity, the bond removal by a link-status fact — so they stay on a three-slot mailbox until the domains that own them land. The list, and the issue that retires each entry, is [`device_core/residual.rs`](src:firmware/obc-app/src/device_core/residual.rs).
...
**Where a computed route's elevation comes from.** An OBCR carries a height per point, and an imported GPX brings its own; a route the *device* planned has to get them from somewhere. That somewhere is a **terrain raster the map carries** — ground heights on the same grid, in the map file's own terrain region, mounted at boot through the same handle the map streams from. A map packed straight from an extract carries no such region and therefore no terrain on the device: the **assembler is the terrain producer**, and whether the packer should splice one too is an open question on [#1420](https://github.com/timohueser/OpenBikeComputer/issues/1420). The emit step samples that raster at each vertex it writes, inserting a few interpolated points where a road segment runs long enough to hide a crest between its ends. The route's climb and descent totals then come out of the *same* dead-band integrator an imported track's do, so a planned route and its exported GPX agree. Terrain is strictly an enhancement: the sampler sits behind a one-method seam whose null implementation answers "no height here" for everything, so a map with no terrain plans, rides and renders exactly as before, with flat profiles — and no screen, stat or export needed a line of code to know the difference.
...
Snapping is to the **road geometry**, not to the nearest junction. The map carries sparse lookup-only anchors on long edges, with endpoints and anchors never more than 300 m apart along the road. A 251 m internal node/anchor window therefore discovers every road within the accepted 100 m envelope, even around a long curve; the anchor only names an edge. The router then projects the request segment-by-segment onto that edge's full stored polyline and starts or ends the visible route at that exact projection. Most endpoints prove completeness after one 200 m lookup; the ambiguous remainder takes a second 251 m lookup. Neither lookup spacing nor window radius quantizes the displayed endpoint.

The miss budget is sized from measurement. The router reads §8 through **32 whole 512-byte graph/edge slots plus sixteen aligned index windows** — exactly 24,852 B, caller-owned in the route-only scratch-arena arm rather than added to linked resident RAM. The frontier pops the *globally* best-`f` node and hops between several simultaneously-active leaves, so smaller working sets thrashed on the multi-map command study; useful locality continued through 32 slots. A search step is bounded by **cache misses** (roughly one SD read each) rather than a fixed settle count, so a warmer cache performs more useful search per cooperative pass without making a cold pass unbounded.

Three bounds are worth stating, because the router trades optimality and range for fitting in fixed RAM — and not even RAM of its own: the search table and the render scratch are two arms of one reserved block, so the router's ceiling is what the *largest* consumer already costs, not an allocation added beside it.

- **No distance cap — running out of memory *is* the range limit.** There is no crow-flies pre-check; the limiter is the fixed A\* table. A full table doesn't abort on sight: the first failed insert latches a `table_full` flag and the search keeps going without adding new nodes, so a goal discovered before the table filled is still returned. Only once the frontier drains does the plan fail — surfaced as `Exhausted` → "Too far to route here.", while every other failure (no routable road within the 100 m acceptance envelope, an empty frontier, a failed read) is the generic "Couldn't find a route." tier. Range is therefore **terrain-dependent**, not a radius: a sparse alpine network plans into the double-digit kilometres, a dense urban grid exhausts far sooner — the table holds a node *count*, and terrain decides how much ground that count covers.
...
The router itself — snap, the weighted-A\* settle loop, and the OBCR emit — is [`obc-route/src/nav.rs`](src:firmware/obc-route/src/nav.rs); the §8 graph it reads is described on the [data formats](../formats/#the-navigation-graph-a-routable-network) page and built by the [packer](../packer-routing/#building-the-navigation-graph); the app-side request/answer seam is the pass — Navigator hands the executor one bounded planning operation and the executor answers it with that operation's own token ([`navigator.rs`](src:firmware/obc-app/src/navigator.rs)), while a route arriving over BLE reaches the same screens as an unrequested fact.
...
- The allocation-free OBCW traversal and single-tile decode seam (no storage/cache policy): [`obc-weather`](src:firmware/obc-weather)
