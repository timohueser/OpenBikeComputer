...
<figcaption>Hosts depend on <b>obc-app</b>; app on <b>obc-render</b> — and directly on <b>obc-reader</b> + <b>obc-route</b> as well. Render and reader meet only through <b>obc-map-scene</b>: the reader streams visible candidates and complete selected geometry, while the renderer never learns OBCM offsets, quadtrees, or cache policy. The separate active-route overlay remains a narrow chunked-polyline seam. Beneath them, <b>obc-formats</b> owns persistent byte facts, while <b>obc-ports</b> owns semantic samples and narrow sensor/input/track/settings traits. Host and <b>obc-platform</b> implementations depend directly on those ports, never upward on app policy; every persistent-format consumer imports its byte facts from <b>obc-formats</b> directly (the migration's transitional re-exports were removed once the graph settled). Because every arrow points down, the shared core compiles and runs without <i>any</i> host. (Two crates sit outside this runtime stack: <b>obc-pack</b>, the offline map producer, and <b>obc-web-convert</b>, a wasm shim that lends <b>obc-route</b>'s converters to a browser tab — neither constructs an <b>App</b>.)</figcaption>
</figure>
...
Inside the crate, the `App` a host constructs is a **composition root**, not the implementation site: one plain struct per responsibility, composed by value and driven through the same façade methods hosts have always called. The [ride engine](src:firmware/obc-app/src/ride_engine.rs) owns everything derived from the sensors and the active route (the matcher, the once-per-load elevation/climb/waypoint caches, the breadcrumb, the repaint edges); the [UI runtime](src:firmware/obc-app/src/ui_runtime.rs) owns the screen stack, timers, repaint accumulation, and the delivery discipline every host-pushed card shares; the [catalog state](src:firmware/obc-app/src/catalog_state.rs) owns the route/ride/trip catalogs with their durable ids — the id ↔ summary pairing and the rescan remap live in one place, so an inserted or deleted file can never silently retarget what's navigated; the [host module](src:firmware/obc-app/src/host.rs) owns the typed command/event vocabulary *and* its pending state; and the render-only scratch (the dominant slice of the app's resident footprint) sits behind its own [placement-initialized wrapper](src:firmware/obc-app/src/render_res.rs), since on the device the whole `App` is initialized field-by-field straight into its reserved RAM rather than built on the stack. Components talk through parameters, not back-references — a delivery rule that needs to know "is a ride being tracked?" is handed that fact, so the dependency direction inside the crate is as one-way as the crate graph around it.

At the bottom, [`obc-formats`](src:firmware/obc-formats) is deliberately smaller than a reader: it has no allocator, storage adapter, cache, converter, executor, or rendering policy. The root format specifications remain the normative byte contracts; this crate is their code authority for versions, fixed lengths, flags, sentinels, endian primitives, and the neutral byte-source/sink traits. Every persistent-format producer and consumer imports those facts directly — including `obc-pack` and the deliberately independent `obcm-testkit` byte builder. `obc-reader` and `obc-route` keep the parsing and streaming algorithms and import their constants, codecs, and byte seam from `obc-formats`; the migration's transitional compatibility re-exports were removed in the final audit, so there is now exactly one import path for each byte fact.
...
Beside it, [`obc-ports`](src:firmware/obc-ports) owns dependency-free values and traits that cross the app/host boundary. The app consumes those definitions directly, and platform, board, simulator, replay, browser-host, and USB implementations implement them directly; `obc-app::hal` remains only a compatibility re-export. Calendar arithmetic is semantic here, while the app keeps its own editor/storage year policy; `SettingsStore` uses an associated value so the foundation never depends upward on the app's `Settings` model.

## Two hosts, one core — and the seams between them

A "host" is whatever constructs an [`App`](src:firmware/obc-app/src/app.rs) and drives it. The simulator ([`obc-sim`](src:apps/obc-sim)) is an `eframe`/`egui` desktop shell; the device firmware ([`obc-fw-nrf54l`](src:firmware/obc-fw-nrf54l), via [`obc-platform`](src:firmware/obc-platform)) is bare-metal on the nRF54LM20. (A third host, [`obc-web-demo`](src:apps/obc-web-demo), puts the same core behind a small wasm API for the landing page's live demo — no GUI framework at all, the page's JS owns the canvas and the frame loop. Seam-wise it's a minimal sibling of the simulator, with in-memory stores and a GPX replay for sensors; the host logic the two share — replay stepping, the frame-interleaved route planner, the in-memory stores, and the whole command/event orchestration (below) — lives in [`obc-host-core`](src:host/obc-host-core). A second wasm crate, [`obc-web-convert`](src:apps/obc-web-convert), is deliberately *not* a host: it constructs no `App` and drives no loop, it just exposes `obc-route`'s converters to the web builder's JavaScript — GPX ↔ OBCR both ways, plus reading a stored route's polyline back out for the desktop app's previews.) (The seams were first proven on an STM32F429 prototype, since removed; the nRF is what the project ships on, and the *same* core ran unchanged on both.) Each owns its window/panel, its storage, and its sensors — and hands the core four small abstractions. Those four **seams** are the entire device-specific surface area; find them and you've found every boundary that matters.
...
**The HAL — the world, abstracted.** GPS fixes, the GPS receiver's UTC time, barometric altitude, ambient temperature, the electronic-compass heading, the heart rate / power / cadence from BLE sensors, the recorded-track log, settings persistence, and raw button events each arrive through their own trait in [`obc-ports`](src:firmware/obc-ports/src/lib.rs), with live sensors bundled into a `Sensors` set the app polls once per tick. The app and every implementation bind to this crate directly: board-agnostic adapters live in [`obc-platform`](src:firmware/obc-platform/src) (the source/handoff adapters — button debouncer, debug-sensor codec, sensor hand-offs, synth/stub sources), with the pure sensor chip decoders split into [`obc-sensors`](src:firmware/obc-sensors/src) and the SD storage adapters into [`obc-storage`](src:firmware/obc-storage/src); recorded-ride sources in [`obc-replay`](src:host/obc-replay/src), and shared desktop/browser stepping in [`obc-host-core`](src:host/obc-host-core/src/replay.rs). [`obc-app::hal`](src:firmware/obc-app/src/hal.rs) remains an import-compatible re-export, not a second interface layer. The app integrates each stream on its own asynchronous cadence and is completely oblivious to whether a fix came from a satellite or a GPX replay — or whether a heart rate came from a real strap or an injected test line. The three BLE-sensor sources are the one set not read off a wire the device owns: they're fed by the radio's [central-role manager](../companion-link/#sensors-the-device-as-ble-central) (or, on the host, the simulator's sliders), and a value older than 5 s drains as `None` — so a dropped strap reads `--` and records as absent rather than freezing its last number into the log.
...
The receiver's UTC time rides the same mailbox model but is published **independent of the position fix** — the GPS resolves time before a 3D lock — so the clock is set during acquisition, while there's still no usable fix. There is no battery-backed RTC, so the wall clock is a stored set-point advanced by the monotonic timer; a GPS stamp simply re-establishes that set-point, and between stamps it free-runs from the last value. A fresh boot's set-point is therefore stale (off by however long the device was powered down), so it is **display-only until a real source re-establishes it this boot** — a GPS fix, or the phone over BLE ([`setClock`](../companion-link/#the-trusted-clock-and-route-retention) on connect); manual date/time editing was removed so nothing else can. That trusted-this-boot distinction is the safety gate the [storage auto-expiry](../ui/#self-cleaning-storage-routes-and-rides-expire) sweep won't delete without. The harmless UTC-offset stepper stays — it shifts the displayed hour only, since the underlying time is pure UTC — so the Date & Time screen is now just the read-only GPS-fix / local-time rows and that offset.
...
Beyond rendering, each loop pass also carries the app's **requests** to the host and the host's **answers** back. That traffic speaks one typed vocabulary ([`host.rs`](src:firmware/obc-app/src/host.rs)): everything the app can ask — delete this route, run this plan, persist the settings, rescan the store — is a `HostCommand` the host drains once per pass into a small bounded mailbox it owns, and every answer or fact — the plan finished, a route upload committed, the card scan's result — is a `HostEvent` it applies back. The protocol encodes each request's semantics rather than leaving them to convention: one-shots drain exactly once, store-change bursts ride as a count that can't be lost, the two fill cues re-emit until answered, cancellation always drains before new work, and a full mailbox back-pressures (the command stays latched) instead of dropping anything. Commands and events carry only bounded ids and small results — bulk data such as catalogs and elevation profiles stays in app-owned buffers the host fills directly — and the input/overlay plane deliberately stays *off* this channel so a button edge never queues behind store work. The per-request methods the hosts historically called remain as thin adapters over the same single pending state during the migration.

The *sequencing* of that traffic — drain the mailbox, delete a route and re-feed the catalog, step the resumable planner one bounded pass, reconcile the ride log — is written once, for every frame-stepped host, in [`obc-host-core`](src:host/obc-host-core/src/dispatch.rs). A single dispatcher drives the drain against narrow repository interfaces (`RouteRepository`, `RideRepository`, `TrackRepository`, and a trip catalog) that the simulator's folder-backed stores and the web demo's in-memory stores both satisfy — one shared conformance suite proves they behave alike — while a resident *active-route session* keeps the parsed route index across frames so a settled map never reparses it. The board keeps its own asynchronous execution (its FAT and object-store work stays on the executor, not a desktop store abstraction), but it consumes the identical `HostCommand`/`HostEvent` vocabulary, and shared protocol tests pin that its ordering matches the dispatcher's.
