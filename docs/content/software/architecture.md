...
- The render-only scratch — the dominant slice of the app's resident footprint — sits behind a [placement-initialized wrapper](src:firmware/obc-app/src/render_res.rs), because on the device the whole `App` is initialized field-by-field straight into reserved RAM rather than built on the stack.
...
<svg viewBox="0 0 720 320" role="img" aria-label="The router seam as a request-and-answer loop. From a POI detail screen, a press opens a Create a route confirm showing the destination's category glyph and its straight-line distance. Accepting records a one-shot NavRequest — the rider's fix as the start, the POI coordinate as the goal. On accept the confirm swaps to a planning screen — a spinning compass needle, cancellable with Back. The host drains the request and steps obc-route's NavPlanner — one bounded step per loop pass — against the resident map on the SD card, streaming the emitted OBCR into the reserved nav file. It then rescans the route catalog and answers back with the NavPlanned event carrying the new route's durable id — the same rescan-then-resolve contract BLE uploads use. On success the planning screen is replaced by the NEW ROUTE overview — distance, bike type, and a decimated route-shape preview — and the route activates, re-entering the normal load and navigation pipeline. On failure a two-tier card shows either too far to route here — the router's fixed search table filled before the goal was reached, which is the device's honest range limit — or couldn't find a route for everything else.">
  ...
  <text class="d-sub" x="420" y="110" style="font-size:9.5px">obc-route::NavPlanner — edge snap ≤250 m,</text>
...
A plan takes seconds on the SD-bound device, so the flow visibly *breathes* instead of blocking: accepting the confirm opens a **planning screen** — a spinning compass needle over plain copy — while the host steps the planner **one bounded slice per loop pass** (snap scans one band of its search box a step, then a miss-budgeted burst of A\* settles a step — the step ends after a small fixed number of SD chunk reads, so a warm cache does far more useful search per pass — then a few emit hops a step). Render, input and the watchdog all run normally *between* steps, and **Back cancels**: the screen pops back to the POI detail and the host simply stops stepping — nothing reaches the reserved file until the emit phase, and a cancelled plan's partial file is discarded.

Snapping means the nearest point on the nearest routable **edge polyline** within 250 m — not the nearest junction. The packer splits graph edges to at most 500 m, which makes that edge discoverable with a bounded ~501 m endpoint query (including rounding guard) through the existing node quadtree instead of a second global index. Only edges that can still win are actually fetched and projected: the junction record already carries every neighbour's coordinate *inline*, and an edge of length `L` whose endpoints are `C` apart cannot stray further than `sqrt(L² − C²)/2` from that endpoint chord — so a chord distance minus that bulge is an honest lower bound, and one that already loses is skipped before it costs a chunk read. A projection inside an edge becomes a temporary A\* node connected to both endpoints with proportional partial cost; two projections on the same edge also connect directly. Emission then clips the first and last edge at those exact projected coordinates. A rider beside the middle of a long road therefore starts on the road, and the OBCR neither jumps back to a distant junction nor charges the untravelled part of that edge.
