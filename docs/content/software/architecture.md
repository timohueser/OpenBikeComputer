...
A "host" is whatever constructs an [`App`](src:firmware/obc-app/src/app.rs) and drives it. The simulator ([`obc-sim`](src:apps/obc-sim)) is an `eframe`/`egui` desktop shell; the device firmware ([`obc-fw-nrf54l`](src:firmware/obc-fw-nrf54l), via [`obc-platform`](src:firmware/obc-platform)) is bare-metal on the nRF54LM20. A third host, [`obc-web-demo`](src:apps/obc-web-demo), puts the same core behind a small wasm API for the landing page's demo — no GUI framework; the page's JS owns the canvas and the frame loop. Seam-wise it's a minimal sibling of the simulator, with in-memory stores and a GPX replay for sensors; the host logic the two share — replay stepping, the frame-interleaved route planner, the stores, and the command/event orchestration below — lives in [`obc-host-core`](src:host/obc-host-core). (Two further wasm crates are deliberately *not* hosts — neither constructs an `App`. [`obc-web-convert`](src:apps/obc-web-convert) exposes `obc-route`'s GPX ↔ OBCR converters to the web builder's JavaScript, and [`obc-web-preview`](src:apps/obc-web-preview) exposes `obc-reader` + `obc-render` to it, so a style preset's card can draw a small demo map at the panel's own resolution. They take exactly the layer they need out of the same stack, and no screen, replay or planner comes with it.)
