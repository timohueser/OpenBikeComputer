...
<figcaption>The seam that hides the panel's wire format is <code>Panel</code>: the caller renders a small band, and the backend reformats + transports it. The band scratch is only a few rows, <b>reused for every band</b>, so the frame lives once in the RGB222 plane and never as a second full RGB565 copy.</figcaption>
</figure>

The wire format lives behind [`Panel::flush_band`](src:firmware/obc-platform/src/panel.rs). The opt-in bring-up panel (`--features tft`) is an **ST7789** over SPI: each band is packed to the panel's **12-bit RGB444** format — two pixels into three bytes, ~25% fewer bytes than RGB565, and the RGB222 gamut survives 4-bit channels losslessly — then a `CASET`/`RASET` window is addressed and the bytes streamed by DMA. Because the scratch is just a few rows (~7 KB), the 320-row frame tiles through it in ~23 pushes; the frame itself never gets a second full-frame buffer.

The panel the device is *designed* for is a reflective **memory-LCD (LS021B7DD02-class MIP)**, driven by the nRF's **FLPR** coprocessor — and the real map/ride app **already runs on it** — it's now the **default** firmware build, with the ST7789 kept as the opt-in `--features tft` backend. The FLPR scans the frame top-to-bottom in one pass, so for this backend the M33 renders into the RGB222 plane and then **presents** it whole (`flush_band` only fills the plane; there's no per-band feed to glass) — the M33 packs each line into two ping-pong buffers the FLPR drains. Worth saying plainly: the FLPR is **not** a free scan-out engine, and a *full* MIP frame (~97 ms today) is actually slower than the ST7789's. So it doesn't rewrite the whole frame when it needn't: a **span-masked scan** ([issue #163](https://github.com/timohueser/OpenBikeComputer/issues/163)) fast-forwards the gate over unchanged rows and writes only the changed row-spans, early-stopping after the last — frame cost scales with *changed rows*, not a flat 320. That's render-on-demand carried onto the glass; the overlay below is the first thing to ride it.
