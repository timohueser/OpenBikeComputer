...
Weather (epic #1185) adds one optional stage *inside* this same paint order: a **precipitation raster** drawn after every span below the road band and before the road band itself, so rain shades the ground while roads, the route, the rider and the map chrome all stay legible above it. The hook mirrors the casing insertion — a z-boundary split into the already-sorted spans, at a fixed firmware-owned boundary below the first road style — and with no rain lent the split is skipped entirely, byte-identical to the frame before the feature existed.

The rain itself obeys three locked rules. Sampling is **nearest-neighbour provider cells**: the screen→grid transform is affine, so cell coordinates are walked with per-pixel fixed-point increments and each pixel takes exactly the intensity of the one provider cell under it — no bilinear, no contouring, no fabricated resolution, and a rotated heading-up view just rotates the same hard cell edges. Intensity maps to colour through a **firmware-owned RGB222-exact table** that lives with the renderer, not in map skins — rain is semantics, not cartography, and a skin change can never restyle it. And the ordered **Bayer dither is transparency, not smoothing**: a band's coverage says how many cells of the fixed screen-anchored 4×4 matrix show the rain colour; every other pixel keeps the already-rendered basemap, so light drizzle reads as a sparse tint the map shows through and a storm core reads nearly solid. Tiles stream through the weather store's bounded cache — each visible 16×16 tile is decoded at most once per frame — and dry, expired or missing data draws nothing at all: staleness gating happens before the renderer ever sees a frame, and no-data is never allowed to look dry.
