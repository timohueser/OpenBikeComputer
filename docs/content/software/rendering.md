...
- **Pass B** returns the winning tokens to the scene source, which preserves its natural chunk-major walk and re-decodes only those **winners** directly into caller-owned scratch. In the OBCM adapter, only chunks that own a winner are re-read, and each such chunk is loaded and borrowed **once** — every winner it owns decodes from the resident bytes under that single borrow, so a chunk with a thousand winners still costs one cache request, not one per winner.
