...
There are two deliberately separate kinds of degradation here. A **frame-budget drop** happens only after a complete feature produced a stub; it follows the deterministic priority policy above and increments `features_dropped`. A **decode failure** never produces partial geometry: an over-capacity feature is consumed and dropped whole, malformed bytes are rejected, and medium/cache failures remain typed. The renderer counts these separately as capacity drops, malformed features, read failures, and cache contentions. Pass A simply omits a failed feature's stub and continues when its byte extent is known; if a winner refetch fails in pass B, the collector rolls its point/ring offsets back before continuing, so later work cannot inherit stale geometry.
