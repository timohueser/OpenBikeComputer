...
The wire format lives behind [`Panel::flush_band`](src:firmware/obc-platform/src/panel.rs). Today's bring-up panel is an **ST7789** over SPI: each band is packed to the panel's **12-bit RGB444** format — two pixels into three bytes, ~25% fewer bytes than RGB565, and the RGB222 gamut survives 4-bit channels losslessly — then a `CASET`/`RASET` window is addressed and the bytes streamed by DMA. Because the scratch is just a few rows (~7 KB), the 320-row frame tiles through it in ~23 pushes; the frame itself never gets a second full-frame buffer.

The panel the device is *designed* for is a reflective **memory-LCD (LS021B7DD02-class MIP)**, driven by the nRF's **FLPR** coprocessor — and the real map/ride app **already runs on it** through the same `Panel` seam (the firmware builds with `--features panel-ls021`). The FLPR scans the *whole* frame top-to-bottom in one pass, so for this backend the M33 renders the frame into the RGB222 plane and the **`end_frame`** drives all 320 rows at once (`flush_band` only fills the plane; there's no per-band feed to glass) — the M33 packs each line into two ping-pong buffers the FLPR drains. Worth saying plainly: the FLPR is **not** a free scan-out engine, and a *full* MIP frame (~97 ms today) is actually slower than the ST7789's. The next step leans on the MIP's native **partial-line updates** (a few changed rows cost ~1–3 ms): push *only what changed*, which is exactly what render-on-demand already delivers.

### The overlay composites on the push

The hold-progress bulge — the little arc that swells as you hold a button — is never baked into the framebuffer. If it were, dismissing it would force a full map re-render just to paint it back out. Instead it **composites on the push**: over a static map, the [input plane](../architecture/#staying-responsive-the-two-planes) re-pushes *only* the bulge's narrow right-edge window — it reads those framebuffer columns back (RGB222 → RGB565), draws the bulge over them, and DMAs just that strip. The map underneath is never touched, so the bulge animates at full frame rate while the expensive `render_map` stays asleep. It's [render-on-demand](../architecture/#the-per-frame-loop) taken to its limit: repaint the few hundred pixels that changed, and nothing else.
