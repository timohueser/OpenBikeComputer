...
The panel the device is *designed* for is a reflective **memory-LCD (LS021B7DD02-class MIP)**, eventually driven by the nRF's **FLPR** coprocessor. That reuses the same `Panel` seam — only `flush_band` changes: pack RGB222 to the MIP's line-serial wire bytes and lean on its native **partial-line updates** (a few changed rows cost ~1–3 ms), because a *full* MIP frame is actually slower than the ST7789's. Worth saying plainly: the FLPR is **not** a free scan-out engine — the M33 still converts each line into ping-pong buffers — so the whole design leans on pushing *only what changed*, which is exactly what render-on-demand already delivers.
