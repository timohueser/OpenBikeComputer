...
> coordinated **flat store** cutover is under development. Under the flat store an update package is
> an object like any other — a `PUT` of kind `7` — and installing is a separate authenticated
> [`ARM`](src:specs/FLAT_Store_Protocol.md) command rather than a press on the glass. The card stops
> being user-accessible, so the sideload path below goes with it. The trust model — verify before
> erase, signed images, one trial boot then rollback — is unchanged by that cutover and is where the
...
<figcaption>One build, two homes, one pointer, three clients — and a wall with a single door in it. The workflow is the only thing that signs, and the <code>inspect</code> gate means it cannot publish a container its own firmware would refuse. The <b>release</b> is the archive; the <b>bucket</b> is only where the bytes are served, which is why the manifest names a URL instead of the clients knowing one. The dashed path is the manual one, and it lasts only as long as the card is something a computer can write to: the flat store owns the raw card from block zero and puts no filesystem on it, which retires that arrow along with the sideload row above. Every path converges on a <em>staged</em> file, and the only thing that turns staged into installed is a rider pressing Select — or, under the flat store, an authenticated <code>ARM</code> command that passes the device's own signature, anti-rollback and runtime-safety checks.</figcaption>
</figure>
...
- **Card sideload.** Copy `UPDATE.BIN` onto the card from any computer, put the
  ...
  disabled while a ride is recording, because arming reboots the device. **The flat store retires
  this path outright**: it owns the raw card from block zero, with no partition table and no
  filesystem, so there is nowhere for a computer to drop a file and the card is not user-accessible
  at all. The delivery routes become BLE, USB and the builder
  ([`FLAT_Store_Protocol.md`](src:specs/FLAT_Store_Protocol.md) §4) — the cable being the one that
  covers a device whose radio is unusable.
...
Under the **flat store** staging and installing are still two separate acts, but the second one is a
command rather than a press. Uploading a package is an ordinary `PUT` and does nothing else; a
separate authenticated [`ARM`](src:specs/FLAT_Store_Protocol.md), naming the package by object id
and the revision it expects, may arm and reboot without a physical confirmation — once the device's
own mandatory checks pass: the OBCU structure, the image CRC, the Ed25519 signature, a **version
monotonicity** check that refuses anything not strictly newer, and the runtime-safety refusals (no
ride recording, enough battery). The safety boundary moves off the rider's thumb and onto the things
that were always doing the work: the signature the device verifies itself, the anti-rollback check
it will not let a peer override, and the trial boot with rollback behind it. Over USB the cable *is*
the authorization boundary, which is exactly why the signature and the anti-rollback check are
mandatory rather than advisory. The device's own UI still installs — it does so as a local principal
issuing the same command, so a rider confirming on the glass and an app sending `ARM` travel the
same path.
...
- The flat store's install contract — the `ARM` command, its mandatory checks, the rollback reserve it commits and the order it does everything in: [`FLAT_Store_Protocol.md`](src:specs/FLAT_Store_Protocol.md) §4; the card layout the reserve's extents come out of: [`FLAT_Store_Format.md`](src:specs/FLAT_Store_Format.md) §6
