...
What that buys is the selection people actually ask for. **Baden-Württemberg — 215 cells, 850 MB with its terrain — now assembles in a tab**, at a projected 3.1 GB where the old model said 3.6 GB and refused. A Regierungsbezirk fits a *phone's* tab. But the ceiling itself moved by only about a sixth, to roughly 890 MB of cells, and the reason is worth stating plainly: with the engine down to 45 % of the projection, the majority is now the two copies of the selection the **bridge** holds — the cells copied into linear memory when they arrive, and the shards sitting in memory until the page hands them to a card. Anything around twice a Bundesland is still refused, and DACH by an order of magnitude; those wait on streaming the input and output through origin-private storage, not on any further work in the graph.

That projection deserves a wider error bar than the file-size one above it, and the builder shows it as one. A file's size is *arithmetic* — the cells it carries plus fixed overheads, exact before anything is downloaded. The memory model is a straight line through **two measured runs**, at 90 MB and 296 MB of graph, carried up with a deliberate margin: the harness counts bytes requested rather than pages touched, and wasm's allocator lives in a linear memory that only ever grows, so a freed block is reusable but never returned. Nothing larger than a Bundesland has been measured at all. And the budget the projection is compared against — 3 GiB of the 4 — is a judgement rather than a limit anyone publishes: browsers do not promise how much of the address space a tab gets, and an allocation wasm cannot serve kills the module outright, with no error left to render. So a comfortable "does not fit" is trustworthy, and that is the verdict worth acting on; a "fits" with a few per cent to spare — which is exactly where a Bundesland lands — means *probably*, and reads as a warning rather than a green light.

One of those three terms has since stopped being resident for the whole run. A browser assembly splits at 256 MB and hands each shard to the page the moment its read-back passes, so the *finished set* costs one shard at a time rather than the whole map — the same discipline the manifest-last rule already gives an interrupted transfer, applied to memory instead of to a card. The projection above deliberately still prices the old shape: over-estimating is the safe direction for a threshold whose job is to refuse, and it moves when there is a measurement to move it to.
...
- The browser running exactly that engine — [`obc-web-assemble`](src:apps/obc-web-assemble), the wasm bridge the hosted builder assembles through. It adds no format knowledge: a byte-buffer adapter, a typed error vocabulary, and a progress/abort/hand-off seam built out of the engine's own clock and shard-store traits — the last of those is what lets a finished shard leave wasm memory mid-run instead of at the end. Its output is pinned byte-for-byte against the native CLI's from both sides — natively in `tests/determinism.rs`, and from Node against the wasm build in `bridge.test.ts`
