...
<figure class="fig">
<svg viewBox="0 0 720 256" role="img" aria-label="The navigation-graph section, five parts in file order. A 28-byte nav directory — the graph's resident footprint — is followed by a profile table of one to eight 56-byte bike profiles, then a node quadtree (the same flat u32 encoding as an LOD, over the header bbox), then variable-length junction records bin-packed into 512-byte chunks so distinct leaves may share a chunk, and separately a deduplicated edge pool addressed by byte offset. One junction record is exploded to show it stores its own coordinate and dense id, then a list of 17-byte neighbour entries, each carrying the neighbour's id, its coordinate inline, the connecting edge id, the edge cost in metres, the edge's way-kind byte, and the ascent of riding it in that direction.">
  <defs>
  ...
  <text class="d-tag" x="20" y="22">§8 (v12) — directory · profile table · node quadtree · junction records · edge pool</text>
...
The full byte layout — the 28-byte directory fields, the 56-byte profile records with their `1/16` fixed-point multipliers and climb weight, the 17-byte neighbour entry, the `0xFF` degree sentinel and degree-24 cap, the edge record with its densified `int16` deltas, and how an over-long edge is split at synthetic junctions so no record ever straddles a chunk — is [`OBCM_Spec.md` §8](src:specs/OBCM_Spec.md). What the packer does to *build* this graph from raw highways, and how a profile weights it, is the [extraction stage](../packer-routing/#building-the-navigation-graph); how the device turns it into a route the rest of the system can't tell from a GPX is [the router seam](../architecture/#on-device-routing-the-router-seam).
...
The third term used to be the expensive one, at about 6.4 bytes resident per byte of rebuilt navigation graph. It is the one that got fixed. The merge now keeps a ten-byte handle per edge instead of the record itself and copies the bytes cell → sink at emission, exactly as the geometry graft does; its hash maps became sorted passes; and the read-back walks the written nav section rather than rebuilding the graph a second time in hash maps. Measured phase by phase on two published regions, the working set fell to **about 4.7 bytes per byte of navigation graph, margin included** — a whole Bundesland's merge peaks at 1.0 GB for its 296 MB of graph, where the old engine wanted 1.9 GB and the read-back after it wanted 2.0 GB.

Then the sorted passes left memory too, and that is the change that ends the "bytes per byte of graph" model rather than improving its constant. Every stage of the rewrite — the duplicate check, the island prune, the endpoint joins, the rebuilt adjacency, and the node quadtree over it — is now an **external sort or a merge walk over a host-provided scratch area**, so the working set is a *sort buffer* the caller sizes, not a structure sized by the graph. The last two heap-shaped things went with them: the adjacency stopped being one array of neighbour lists, and the quadtree stopped taking the junctions by value. It is recovered instead from the same records sorted in **tree order** — the quadrant digits of each point's descent, concatenated into one key — which puts every subtree's records in a contiguous run and makes the tree's shape, its bin packing, and where each record lands a single forward pass. The §8 section is then written straight out of that scratch: the index a block at a time, the chunks one 512-byte chunk at a time, each edge record read out of the cell that wrote it, and none of it resident. On a Baden-Württemberg selection the native engine's peak fell from 736 MB to 196 MB for the same 293 MB nav section. The more useful number is the one that *stopped* mattering: the same selection assembled at a 4 MB sort budget peaks at 72 MB. The peak now follows the budget it was given rather than the map it was given, which is the property the ceiling actually needs.

What that buys is the selection people actually ask for. **Baden-Württemberg — 215 cells, 850 MB with its terrain — now assembles in a tab**, where the old model said 3.6 GB and refused. On its own, though, the engine fix moved the ceiling by only about a sixth, to roughly 890 MB of cells, and the reason is worth stating plainly: with the engine down to 45 % of the projection, the majority was the two copies of the selection the **bridge** held — the cells copied into linear memory when they arrived, and the shards sitting in memory until the page handed them to a card. Streaming those two through origin-private storage is what the next two paragraphs describe, and it is what moved the ceiling the rest of the way.

That projection deserves a wider error bar than the file-size one above it, and the builder shows it as one. A file's size is *arithmetic* — the cells it carries plus fixed overheads, exact before anything is downloaded. The memory model is a straight line through **two measured runs**, at 90 MB and 296 MB of graph, carried up with a deliberate margin: the harness counts bytes requested rather than pages touched, and wasm's allocator lives in a linear memory that only ever grows, so a freed block is reusable but never returned. Nothing larger than a Bundesland has been measured at all. And the budget the projection is compared against — 3 GiB of the 4 — is a judgement rather than a limit anyone publishes: browsers do not promise how much of the address space a tab gets, and an allocation wasm cannot serve kills the module outright, with no error left to render. So a comfortable "does not fit" is trustworthy, and that is the verdict worth acting on; a "fits" with a few per cent to spare means *probably*, and reads as a warning rather than a green light.
...
The projection prices the run **it is actually about**, because the two escapes are conditional and their verdicts genuinely differ. On the download path — cells on disk, shards handed out as they pass — the resident terms are constants, the projection is engine-bound, and the budget runs out at roughly **twice a Bundesland** (~1.7 GB of cells at the catalog's density). A direct-to-device send keeps the finished set until its counts have been announced, so it binds earlier, around 1.4× — one selection can honestly download and honestly refuse to stream to a card, and the builder computes and shows the two verdicts separately rather than averaging them. A browser without usable origin-private storage is priced as the old shape, full cells resident, which is what it still runs. And DACH refuses in *every* mode, for the sentence this section opened with: its graph rewrite alone wants ~14 GB of working memory, three times the whole wasm32 address space — a number no residency mode touches, because it is the merge's own, not the bridge's.
