...
The **Display** screen governs the Map's chrome and the auto-return: two toggles for the Map overlays — the small floating top-centre **clock** digits and the bottom-left **scale bar** (the largest round 1/2/5 distance that fits the current zoom) — plus the **idle-return** timeout (15 s … 5 min / Never, default 30 s) that decides how long an untouched UI waits before returning to Home (or, mid-ride, the Map). A screen declares whether idle return is meaningful through its capability row. Modal work that must remain visible — route planning, map transfer, firmware checks/install, pairing and warning cards — is idle-exempt and **suspends** the inactivity clock. When such a flow finishes, the ordinary screen it reveals receives a fresh full timeout; minutes spent uploading or planning are never inherited as already-expired idle time. Deliberate browse and riding views stay exempt without arming an idle wake at all. The Map's other chrome isn't a setting: a bottom-centre **one-slot chip** where "No GPS Fix" outranks "off route", which outranks the calmer waypoint chip and the browse map's one-shot *Press to start a ride* hint; the scale bar steps up above whichever chip is showing, and a warning-red **low-battery** glyph appears top-left below 10 %.
