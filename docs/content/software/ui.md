...
- The **Route overview**'s expiry line — a small muted `Auto-delete · in 3 d` caption above the route's stats. It's deliberately *not* an always-on countdown: it appears **only once the route is within five days of deletion** (a deadline already past, before the hourly pass collects it, reads `soon`), so it reads as a *heads-up that this route is about to go*, not standing chrome. A route that never expires, or whose usage clock hasn't started, shows no line at all.
