...
**Skip ahead** is deliberately a pure navigation skip, not a detour calculation. Opening it replaces the transient ride compass with a north-up map chooser. Each encoder detent adds or removes **500 m** of along-route distance; an orange inner stroke marks the entire selected stretch without hiding the normal magenta route, a ring marks the candidate rejoin point, and the floating bottom panel shows the *actual* clamped distance. The camera continuously fits the rider, the whole selected stretch, and a margin into the map area above that panel, so extending the skip never pushes either end out of view.
