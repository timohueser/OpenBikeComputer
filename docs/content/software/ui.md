...
When the planned path reaches the route earlier than the chosen point — the cheapest approach can legally ride the future route's own road toward the rejoin ring, since only the skipped stretch is blacklisted, not the tail beyond it — the rejoin **advances to that first sustained contact** rather than riding up to the chosen point and doubling straight back; the chosen distance is a *minimum*, and the preview polyline and cost already describe the shortened detour.
