...
screens! {
    ...
    SkipAhead(SkipAheadScreen) => Caps::map().remap(RemapKind::Route), // live route index follows rescans
...
Once a ride is running, `back-hold` opens a second compass with the **same five-detent bezel, amber needle sweep, and label strip**, but ride-scoped stations: **Waypoints**, **Skip ahead**, **POIs**, **Routes**, and **Main menu**. Waypoints starts at north; one counter-clockwise detent reaches Main menu. **Waypoints** opens the route's whole-plan list — names, distance-to-go and climb-to-go — on the next stop; Skip ahead opens the map chooser below; POIs and Routes open their existing browsers. A route-less recording keeps the same five positions: Waypoints and Skip ahead stay dimmed, Waypoints opens *No waypoints / No route loaded*, and Skip ahead cannot commit, so the ring never changes under the rider's hand.

### Skipping ahead without rewriting the route

**Skip ahead** is deliberately a pure navigation skip, not a detour calculation. Opening it replaces the transient ride compass with a north-up map chooser. Each encoder detent adds or removes **200 m** of along-route distance; an orange inner stroke marks the entire selected stretch without hiding the normal magenta route, a ring marks the candidate rejoin point, and the floating bottom panel shows the *actual* clamped distance. The finer step keeps the interaction aimed at a nearby closure or trail problem rather than turning it into a many-kilometre route editor. The camera continuously fits the rider, the whole selected stretch, and a margin into the map area above that panel, so extending the skip never pushes either end out of view.

An encoder **hold** toggles a second, candidate-centred inspection view. The selected rejoin point does not move; `turn` now zooms out/in around its ring and the panel shows both the still-selected distance and magnification. Hold again returns to the fitted overview. `Press` commits from either view and `back` cancels the chooser, so inspection adds precision without adding another confirmation step or changing the existing escape path.

`Press` recomputes the candidate from the latest matched progress and commits it; `back` changes nothing. A commit leaves the OBCR geometry, activity mode, and tracking-session id intact. Instead it advances route progress and installs a persistent lower bound in the route matcher: fixes on the skipped stretch can report off-route, but GPS jitter and the matcher's backward search can never snap navigation behind the chosen rejoin point. The normal matcher takes over again when the rider reaches the route at or beyond that point. Changing/removing the route or starting a new tracking session clears the bound. There is no fabricated straight connector and no weighted-A* road-closure penalty in this first version — finding and presenting a routed detour is a separate feature.

The chooser follows the live progress anchor even while it is open, and `Turn` followed immediately by `Press` cannot commit a stale rendered candidate. A live catalog reorder rekeys both the open chooser and a just-queued commit by the route's durable id; if that route vanished, the action becomes unavailable rather than silently targeting the route that inherited its old index. It is also guarded while the rider is already off-route and inside the final 100 m where there is no useful later rejoin. Both commit and cancel pop back to the exact riding view that opened the compass — Map, Statistics, Climb, or the Paused page.
