...
**The two classes do not reach the same distance up the ladder**, and that is the one place where the styling stops being only about ink. `major` is on the map from the planning tier down; `index` reaches **one tier coarser**. The tier above the planning tier is the band a rider pans across to read a valley, and giving it every 100 m line there would be both illegible and the most expensive tier to fill — but the 500 m rhythm alone is a fifth of the features, and at that tier's coarser simplify it draws the shape of the range without the texture. So the terrain layer *fades*: full contours, then the index rhythm, then nothing, instead of the whole layer disappearing between two zoom steps.
