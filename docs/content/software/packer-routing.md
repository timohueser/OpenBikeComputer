...
The in-memory build — the way-kind classification, the bike-legality filter, junction detection, edge split and dedup, island pruning, bounded-edge split, and great-circle lengths — lives in [`obc-pack/src/nav.rs`](src:host/obc-pack/src/nav.rs), with [`obcm-assemble`](src:host/obcm-assemble/src/nav.rs) applying that bounded split after merged pruning for cell-built maps. Turning the graph into the tiled, chunked [§8 section](../formats/#the-navigation-graph-a-routable-network) (the node quadtree, the inline-adjacency records, the byte-addressed edge pool, and the densification that keeps every record inside one chunk) is the serializer's job, described in [`OBCM_Spec.md` §8](src:specs/OBCM_Spec.md). What the device *does* with it — exact edge snap, virtual-endpoint profile-weighted A\*, clipped emit — is [the router seam](../architecture/#on-device-routing-the-router-seam).
