...
**The elevation rides along with the line** ([OBCM v13](../formats/#features-an-anchor-then-deltas)): a signed `int16` of metres behind the feature header, on contours and on nothing else, and a style-table bit that says which class is the index one. Both exist for the same reason — the device labels index contours, and a number is what makes a contour a map rather than a texture. The packer knows the level at trace time and used to throw it away; the alternative was for the renderer to sample the terrain raster at each label anchor, per frame, which puts an SD read inside the frame budget. Two bytes per contour is about **1 %** of a contoured map: on the alpine extract below, 17,925 contour features and exactly 35,850 bytes against 3.67 MB. The class flag is on the *style* rather than inferred, because the renderer has no way to guess it — the interval and the index step are packer config and never travel in the file.
