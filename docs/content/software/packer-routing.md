...
Both of those limits — *"only what somebody baked"* and *"whole regions only"* — are properties of baking **regions**. Baking **cells** instead removes them: the coverage a rider selects is assembled from a lattice of pre-baked squares in the tab, which is cheap enough to do there because the assembler copies chunk bytes rather than decoding them. Two countries at once, a drawn box, and a corridor around a trip's routes all become the same operation, and a restyle stops being a bake at all. That design is specified but not yet built — the shape of it, and the arithmetic that makes it free, is [cells and assemblies](../formats/#cells-and-assemblies-the-shape-the-catalog-is-moving-to) on the data formats page.
