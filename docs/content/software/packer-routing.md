...
**The two classes do not reach the same distance up the ladder**, and that is the one place where the styling stops being only about ink. `major` is on the map from the planning tier (LOD 3, the tier the renderer selects out to 10 m/px) down; `index` reaches **one tier coarser**. The tier above the planning tier is the band a rider pans across to read a valley, and giving it every 100 m line there would be illegible and barely worth paying for — but the 500 m rhythm alone is a fifth of the features, and at that tier's coarser simplify it draws the shape of the range without the texture. So the terrain layer *fades*: full contours, then the index rhythm, then nothing, instead of the whole layer disappearing between two zoom steps.

The cost is real and worth stating plainly: contours are the most expensive optional thing a map can carry. On the corpus's worst case for them — a 628 km² alpine extract, dense terrain and sparse OSM — 100 m contours from the planning tier down add about **a quarter** to the map. Extending the index class one tier further up costs another **half a percent** on the same extract, which is the whole argument for the split: a fifth of the levels, simplified to that tier's tolerance, is nearly free next to what the full set costs where it already lives. Flat ground adds almost nothing either way, because flat ground crosses almost no levels.
