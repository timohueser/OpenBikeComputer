...
The loop the desktop tier exists to close. A build lands as a real file in a folder you can back up — that is most of the point of having a filesystem — and the device step then offers it directly, so putting a fresh map on the computer is a click rather than a trip through Finder and a card reader.

What makes that more than a shortcut is *where the bytes go*. The map is already on the same disk as the process that owns the USB endpoint, so the window hands over a **path**, not the file: a twelve-byte transfer descriptor crosses the IPC, and the several hundred megabytes go disk → endpoint inside Rust. The browser tier cannot do this and does not pretend to — a page has no paths, so the same surface there streams a scratch file through the tab. One protocol, two ways of feeding it, and the difference is visible only as the memory the app doesn't use.

#### The rides come back the same way

The cable runs both directions, and the return trip is the one an Android or no-phone rider has no other answer for: without a companion app, a recorded ride has nowhere to go. So the app keeps a **managed folder** — a real directory you can relocate, back up and open in Finder — with one small `index.json` and, per ride, two files. A **GPX**, because that is what everything else reads and it is most of the reason this is a folder rather than a browser store; and the device's own **ride object**, byte for byte, because [`track_to_gpx`](src:firmware/obc-route/src/track.rs) still omits `<time>` and a library that quietly dropped every point's timestamp would not be an archive. The second file costs about a fifth of the first, and it is what lets a better exporter be re-run over an old ride.

What makes this more than a download folder is that the device is *told*. A ride the app holds is flagged `synced` there, which is what lets the rider delete it on the device without a warning — and which starts its [auto-delete countdown](../companion-link/#synced-rides-reconciled-state-not-event-inference). That flag is a durability claim, so the app earns it in that order: write, `fsync`, rename, `fsync` the directory, commit the index, *then* ack. It is also why the browser tier keeps a one-shot export and no library at all: a download the rider can cancel at the save dialog is not a durable copy, and the hosted ride panel is handed a device object with no ack on it rather than a rule to remember.

One thing this deliberately is *not* is a build streamed straight into the cable with no file in between. The transfer descriptor announces the whole object's length and its CRC-32 **before the first byte moves**, and neither of those is known until the packer has written its last chunk — so a `.obcm` has to exist first. It is the product rather than an intermediate, which is the same answer from the other direction: you wanted the file anyway.

#### The card, in the open

With a cable that lists as well as writes, the app can treat the device like the storage it is. Its **Device page** shows what the card holds: every route with the facts its list entry already carries — including the retention clock no UI had ever surfaced — and every trip as the group of routes it is, exactly as the device's own menu groups them. Routes can be previewed (the stored polyline read back through the same [`obc-route`](src:firmware/obc-route) reader the device draws from, traced over a map with its elevation profile), renamed, deleted, and gathered into trips; dropping several GPX files at once offers to make the stages of a named trip in one motion. None of it needed a protocol feature: an upload to an existing id **replaces the object atomically**, so a rename is download-patch-replace under the same id (its expiry survives), and a trip edit rewrites an object of a few dozen bytes. Rides are the deliberate exception — over the cable they are read-only, previewable and pullable but never deletable, because deletion is the one act the [sync semantics](../companion-link/#synced-rides-reconciled-state-not-event-inference) reserve for the device itself: a ride that only exists on the card must never be losable from a desk.
...
- The builder's Svelte source and its local FastAPI dev host: [`packer/web_builder/`](src:packer/web_builder); the desktop app that links the packer instead of spawning it: [`obc-desktop`](src:apps/obc-desktop); the browser-side GPX↔OBCR conversion (wasm over `obc-route`): [`obc-web-convert`](src:apps/obc-web-convert)
