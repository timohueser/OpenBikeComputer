...
- **A whole style, not a wall of knobs.** The main page offers the shipped **Bikepacking** style — a full packer config in [`builder/presets/`](src:builder/presets), directly usable with the CLI. It used to offer a shelf of such presets side by side; the [schema/skin split](../formats/#schema-and-skin) ended that, because a second preset is a second *schema* and therefore a second copy of the whole cell store. What varies from here is the **skin**, and a skin is stamped on at assembly rather than picked before a bake. An advanced editor still exposes every field the packer accepts (per-feature styling, LOD tiers, the bike-type routing profiles baked into the map, output settings), so nothing is lost for fine-grained work; exports are, again, plain CLI configs.
- **A style shows what it draws.** Every style card renders a small demo map, packed from *that style's own config*, through the device's own renderer at the panel's 240×320 — a picture of the styling rather than a description of it, and the selected card can be dragged and zoomed. Every demo map is cut from **one source extract at one bounding box** ([`bake-previews.sh`](src:builder/bake-previews.sh)), so cards differ by style and by nothing else; the camera frames that same box on each, never a map's own header bounds, which the packer's complete-ways crop widens by a different amount. This matters most where there is no editor to fall back on: on the hosted site, the shipped styling *is* the styling.
...
Both of those limits — *"only what somebody baked"* and *"whole regions only"* — are properties of baking **regions**. Baking **cells** instead removes them: the coverage a rider selects is assembled from a lattice of pre-baked squares in the tab, which is cheap enough to do there because the assembler copies chunk bytes rather than decoding them. Two countries at once, a drawn box, and a corridor around a trip's routes all become the same operation, and a restyle stops being a bake at all. The machinery behind that is built end to end — the bakery cuts extracts into published cells, the assembler grafts them back into a map and verifies it, and the device mounts the resulting volume set as one logical map. Against a cell catalog the builder's first step becomes a **parts list** (regions, drawn boxes, corridors buffered around routes) composed on the map with a tool rail and priced live from the catalog's own byte counts. A route already saved on a connected OBC can be picked there too: its OBCR polyline is loaded only when selected and becomes the same corridor input as an uploaded GPX. The assembled files can be downloaded or streamed directly to the connected card, one verified shard at a time with the manifest last. The shape of it, and the arithmetic that makes it free, is [cells and assemblies](../formats/#cells-and-assemblies-the-shape-the-catalog-is-moving-to) on the data formats page.
