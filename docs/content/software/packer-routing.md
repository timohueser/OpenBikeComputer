...
Both of those limits — *"only what somebody baked"* and *"whole regions only"* — are properties of baking **regions**. Baking **cells** instead removes them: the coverage a rider selects is assembled from a lattice of pre-baked squares in the tab, which is cheap enough to do there because the assembler copies chunk bytes rather than decoding them. Two countries at once, a drawn box, and a corridor around a trip's routes all become the same operation, and a restyle stops being a bake at all. The machinery behind that is built end to end — the packer cuts an extract into cells, `obc-bake bake --cells europe/germany europe/switzerland` bakes real cells and publishes a real cell catalog, the assembler grafts them back into a map and verifies it, and this picker's replacement exists too: against a cell catalog the builder's first step becomes a **parts list** (regions, drawn boxes, corridors buffered around routes) composed on the map with a tool rail, priced live from the catalog's own byte counts, and assembled in the tab. The builder detects which catalog it was handed and serves the matching flow, so until the hosted catalog cuts over — the device reading a multi-file map is the piece still ahead — the tab still downloads whole regions. The shape of it, and the arithmetic that makes it free, is [cells and assemblies](../formats/#cells-and-assemblies-the-shape-the-catalog-is-moving-to) on the data formats page.
