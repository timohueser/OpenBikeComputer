...
Both of those limits — *"only what somebody baked"* and *"whole regions only"* — are properties of baking **regions**. Baking **cells** instead removes them: the coverage a rider selects is assembled from a lattice of pre-baked squares in the tab, which is cheap enough to do there because the assembler copies chunk bytes rather than decoding them. Two countries at once, a drawn box, and a corridor around a trip's routes all become the same operation, and a restyle stops being a bake at all. The machinery behind that is built — the packer cuts an extract into cells, `obc-bake bake --cells europe/germany europe/switzerland` bakes real cells and publishes a real cell catalog, and the assembler grafts them back into a map and verifies it — while the parts a rider touches (this picker's replacement, and the device reading a multi-file map) are still ahead, so the tab still downloads whole regions. The shape of it, and the arithmetic that makes it free, is [cells and assemblies](../formats/#cells-and-assemblies-the-shape-the-catalog-is-moving-to) on the data formats page.
