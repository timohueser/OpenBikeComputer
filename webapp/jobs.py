"""Background job runner: download selected PBFs, then run obcm_pack.py.
...
PACK_SCRIPT = os.path.join(PROJECT_ROOT, "obcm_pack.py")

# Stage strings emitted by obcm_pack.py / obcm.ingest mapped to a coarse phase.
_STAGE_MARKERS = {
    "Merging": "merging",
    "Pass 1": "ingest",
    "Pass 2": "ingest",
...
def _run(job):
    try:
        ...
        # Write the editor's config to a temp file for obcm_pack.py.
        ...
        if rc == 0 and os.path.exists(out_path):
            ...
            job.status = "failed"
            job.emit({"type": "error", "message": f"obcm_pack.py exited with code {rc}"})
